...
def test_synthetic_shape(null_prices):
    ...
    assert set(prices.columns) == {"SIN", "SPY", "DSI"}
    assert len(prices) == truth["n_days"]
    assert prices.index.name == "date"


def test_synthetic_is_deterministic():
    a, _ = data.synthetic_daily(n_years=5, alpha_bps=0.0, seed=211)
    b, _ = data.synthetic_daily(n_years=5, alpha_bps=0.0, seed=211)
    assert np.allclose(a["SIN"].to_numpy(), b["SIN"].to_numpy())
    c, _ = data.synthetic_daily(n_years=5, alpha_bps=0.0, seed=99)
    assert not np.allclose(a["SIN"].to_numpy(), c["SIN"].to_numpy())
...
def test_alpha_knob_shifts_mean_excess_return():
    ...
    from sin_stocks import strategy as st
    ...
    ew_null = st.build_equal_weight(prices_null[["SIN"]].rename(columns={"SIN": "SIN_EW"}))
    ew_alpha = st.build_equal_weight(prices_alpha[["SIN"]].rename(columns={"SIN": "SIN_EW"}))
    spy_null = st.daily_returns(prices_null)["SPY"].dropna()
    spy_alpha = st.daily_returns(prices_alpha)["SPY"].dropna()
    diff_null = (ew_null - spy_null).mean()
    diff_alpha = (ew_alpha - spy_alpha).mean()
...
def test_synthetic_correlation_reasonable(null_prices):
    """SIN and SPY should be positively correlated in the synthetic tape."""
    from sin_stocks import strategy as st
    prices, truth = null_prices
    ret = st.daily_returns(prices).dropna()
    corr = float(ret.corr().loc["SIN", "SPY"])
    assert corr > 0.50, f"synthetic correlation too low: {corr:.3f}"
...
def test_load_aligned_raises_without_cache(tmp_path):
    with pytest.raises(FileNotFoundError):
        data.load_aligned(fetch=False, cache_dir=str(tmp_path))
...
# ---------------------------------------------------------------------------
@requires_cache
def test_real_mo_loaded():
    df = data.fetch_prices("MO", fetch=False, cache_dir=_CACHE_DIR)
...
def test_real_aligned_no_nans():
    ...
    expected = set(data.ALL_TICKERS)
    assert set(prices.columns) == expected
