"""The Golden Butterfly engine and its honest controls — Study 203.

Tyler's recipe (PortfolioCharts.com, 2015): five equal slices of 20% each —
large-cap blend (SPY), small-cap value (IWN), long Treasuries (TLT), short
Treasuries / cash (SHY), and gold (GLD) — rebalanced annually.

The design intent is an extension of Harry Browne's Permanent Portfolio (Study
144): by splitting the equity leg into large-cap and small-cap *value* and the
cash leg into short *and* long Treasuries, Tyler aimed to add the small-cap-value
premium and reduce the bond-duration concentration while preserving the regime-
proof diversification of the PP.

We pin the Golden Butterfly against four benchmarks that together fully grade the
claim:

- **100% SPY** — the raw-return ceiling.
- **60/40 (SPY/TLT)** — the mainstream diversified alternative.
- **Permanent Portfolio 25/25/25/25 (SPY/TLT/GLD/SHY)** — the parent design.
- **1/N equal-weight across all five legs** — the naive allocation baseline that
  removes any active-weighting insight from the GB.
...
SHY receives realistic total return because we use its ETF total-return price
(which includes coupon income); no synthetic cash yield is needed.
...
- The HAC t-stat is Newey-West on the annual return differences (GB − benchmark);
...
def golden_butterfly(
    ...
    """Tyler's Golden Butterfly: 20% each across five asset classes, annually rebalanced."""
    weights = {lcg: 0.20, scv: 0.20, bond: 0.20, cash: 0.20, gold: 0.20}
...
def permanent_portfolio(
    ...
    """Harry Browne's 25/25/25/25 Permanent Portfolio for comparison (Study 144)."""
...
def equal_weight(
    returns: pd.DataFrame,
    tickers: tuple[str, ...] = ("SPY", "IWN", "TLT", "SHY", "GLD"),
    cost_bps: float = 0.0,
) -> pd.Series:
    """Naive 1/N equal-weight across all five GB legs — the placebo baseline.

    The Golden Butterfly *is* 1/N across five assets, so this is the same
    portfolio.  We include it explicitly so strategy tests can confirm that
    the ``blended_portfolio`` engine produces the correct result.
    """
    n = len(tickers)
...
def hac_tstat_annual(a_daily: pd.Series, b_daily: pd.Series) -> float:
    """Newey-West HAC t-stat for mean(A annual return − B annual return).
...
def bootstrap_sharpe_diff(
    ...
    lo = float(np.nanpercentile(diffs_arr, 2.5))
    hi = float(np.nanpercentile(diffs_arr, 97.5))
    return {
        "point": float(point),
        "ci95": (lo, hi),
...
def equity_drawdowns(
    ...
    For each episode (peak-to-trough in the stock leg) returns the SPY loss and
    the total return of every other column over the same window — the direct test
    of whether the GB's non-stock legs cushioned or piled on.
    """
