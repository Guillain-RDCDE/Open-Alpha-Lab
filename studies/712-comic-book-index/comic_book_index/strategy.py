"""Strategy + inference for Study 712 — "CGC-graded key comics are an asset class".

The claim: buying/flipping CGC-graded key comic books (Action Comics #1, Amazing Fantasy
#15, and the "keys" GoCollect and Heritage headline) is a real *investment* that beats the
S&P. We test the strongest tradable version of that:

1. **The comic price index itself** (hardcoded, cited, approximate) vs SPY on the same
   window — CAGR, vol, max drawdown, and a *t*-stat of the annual excess return. This is
   the "did the comics actually beat stocks?" question, on the index believers quote.
2. **The (nearly non-existent) tradable proxy** (Funko, `FNKO`) vs SPY — the only *listed*
   thing a public investor can buy that is even adjacent to the graded-collectibles trade
   (CGC/PSA parents and Heritage are private/delisted). Monthly returns, Newey-West *t* of
   the alpha vs SPY, Sharpe, max drawdown.
3. **The grading-fee + dealer-spread haircut** — what flipping really costs: CGC grading
   (a flat fee on cheap books, a % of value on expensive keys), the auction/dealer spread
   (buyer's premium + seller's commission, or a dealer buy/sell margin), and the years a
   slab sits insured in a safe. We charge it once on the NAV and show where the "return"
   goes.
...
def annual_excess_t(index_level: pd.Series, bench_level: pd.Series) -> dict:
    """*t*-stat that the comic index's **annual** return beats the benchmark's.
...
# --------------------------------------------------------------------------- #
# The grading-fee + dealer-spread haircut — what flipping actually costs
# --------------------------------------------------------------------------- #
def net_of_costs_cagr(gross_cagr: float, round_trip_spread: float = 0.25,
                      hold_years: float = 3.0, grading_fee_pct: float = 0.02,
                      carry_per_year: float = 0.01) -> dict:
    """Charge the comic-flip's real frictions once on the NAV.

    A buy-high / sell-at-a-dealer-or-auction round trip costs ``round_trip_spread``
    (auction buyer's premium + seller's commission, or a dealer buy/sell margin — one-way
    each side already folded in), amortised over a typical ``hold_years``; plus a one-time
    ``grading_fee_pct`` CGC grading/slabbing cost (a flat fee on cheap books, a % of value
    on expensive keys), also amortised over the hold; plus ``carry_per_year`` annual carry
    (insurance/storage). Returns the net CAGR after these.
    """
    spread_drag_annual = (1 - round_trip_spread) ** (1 / hold_years) - 1  # negative
    grading_drag_annual = (1 - grading_fee_pct) ** (1 / hold_years) - 1  # negative
    net = ((1 + gross_cagr) * (1 + spread_drag_annual)
           * (1 + grading_drag_annual) * (1 - carry_per_year) - 1)
    return {
        "gross_cagr": gross_cagr,
        "spread_drag_annual": spread_drag_annual,
        "grading_drag_annual": grading_drag_annual,
