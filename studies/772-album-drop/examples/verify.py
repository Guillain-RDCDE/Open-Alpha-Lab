"""Reproducible headline run for Study 772 — Album-Drop.
...
print("# Album-Drop — does Spotify (SPOT) move on a blockbuster album release?")

print(f"calendar: {len(dt.EVENTS)} blockbuster album drops {dt.EVENTS[0][1]}->{dt.EVENTS[-1][1]}, "
      f"hardcoded from artist/label press releases (all post SPOT's 2018-04 direct listing)")
...
print(data_stamp("Album-Drop panel (SPOT + SPY, adjusted/total-return)", panel, asof=dt.AS_OF))

ev = st.build_event_table(prices, cost_bps=5.0)
ev10 = st.build_event_table(prices, cost_bps=10.0)
inc, inc10 = ev[ev["included"]], ev10[ev10["included"]]
print(f"\nevents resolved: {len(inc)} of {len(dt.EVENTS)} drops have SPOT+SPY coverage "
      f"around the anchor")
for reason, n in ev[~ev["included"]]["reason"].value_counts().items():
    print(f"  excluded {n:2d}x {reason}")

print("\n# THE RUN-UP (the 'buy the rumour') — abnormal return, K sessions BEFORE the "
      "drop, SPOT minus SPY (gross)")
...
print("\n# THE REACTION (the 'sell the news') — abnormal return, K sessions AFTER the drop")
for label, col in (("2-week reaction (k=10)", "post_s"), ("1-month reaction (k=21)", "post_l")):
    s = st.one_sample_t(inc[col].values)
    hr = st.hit_rate(inc[col].values)
    print(f"  {label:<24s} n={s['n']:2d}  mean={s['mean']*100:+.3f}%  t={s['t']:+.3f}  "
          f"hit {hr['k']}/{hr['n']}={hr['rate']*100:.1f}% "
...
print("\n# Jackknife (leave-one-out) — 2-week reaction t-stat")
x = inc["post_s"].values
...
print("\n# TRADABILITY — the reaction net of costs (calendar-known entry, zero look-ahead)")
for base, label in (("pre_s", "2-week run-up"), ("post_s", "2-week reaction")):
...
for bump in (0.01, 0.02):
    planted = st.synthetic_detect(bump=bump, seed=776, k=10)
    print(f"  planted run-up bump=+{bump*100:.1f}% (seed 776): mean AR {planted['mean']*100:+.3f}%  "
