"""Strategy + inference for Study 772 — Album-Drop.

The claim: **Spotify (SPOT) moves on a blockbuster album release.** Two windows, both
anchored on the real album release date (calendar-known weeks ahead via pre-orders /
teaser singles):

* **The run-up (the "buy the rumour").** SPOT's abnormal return (SPOT total-return minus
  the SPY total-return benchmark) over the K trading sessions ENDING on the last close
  on/before the release. Tested at K = 10 (~2 weeks) and K = 21 (~1 month).

* **The reaction (the "sell the news").** SPOT's abnormal return over the K sessions
  STARTING from the release-day close. Tested at K = 10 and K = 21. If the folklore is
  right the stock pops on the drop; if both windows are noise, the "the album moves the
  stock" story is busted.

Because each album release is a single independent event (not a daily series), the primary
statistic is a **one-sample t** of the abnormal return across events (n = the number of
drops with SPOT+SPY coverage). A random-window placebo (drawing many random, non-drop
K-session windows from SPOT's own history vs SPY) checks whether the observed mean sits
...
POST_SHORT_K = 10   # ~2 weeks reaction
POST_LONG_K = 21    # ~1 month reaction
...
def build_event_table(prices: dict[str, pd.Series], cost_bps: float = COST_BPS
    ...
    """One row per album drop: run-up + post-drop abnormal returns + inclusion.

    A row is INCLUDED only if SPOT and SPY both have cached history covering
    [anchor - PRE_LONG_K .. anchor + POST_LONG_K] for that event. Drops whose window
    ...
    rows = []
    for label, keydate in dt.EVENTS:
        row = dict(album=label, release=keydate)
...
def placebo_pvalue(events: pd.DataFrame, prices: dict[str, pd.Series], col: str,
                   ...
                   n_seeds: int = 20, n_draws_per_seed: int = 200, base_seed: int = 776,
                   tail: str = "right") -> dict:
    """For each INCLUDED event, redraw a random (non-drop) k-session window on SPOT vs
...
def car_path(events: pd.DataFrame, prices: dict[str, pd.Series],
    ...
    relative to the release, normalised so offset 0 = 0%, averaged across all INCLUDED
    events. Negative offsets are the run-up; positive is the post-drop reaction window.
    """
    spot = prices[dt.INSTRUMENT]
    spy = prices[dt.BENCHMARK]
    common = spot.index.intersection(spy.index).sort_values()
...
def synthetic_detect(bump: float, seed: int, k: int = PRE_SHORT_K,
    ...
    pre-drop bump. ``side='pre'`` measures the run-up window [p-k..p); ``side='post'``
    measures [p..p+k)."""
