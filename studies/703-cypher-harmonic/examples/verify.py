"""Reproducible headline run for Study 703 — Cypher-Harmonic.
...
from cypher_harmonic import data, strategy as st  # noqa: E402
...
print("# Cypher-Harmonic — does a 0.786 XC retracement at D predict a reversal?")
...
def pooled(placebo: bool, cost: float = 0.0, seed: int = 703, horizons=HORIZONS):
...
print("\n# THE HEADLINE — Cypher XABC (AB retrace 0.382-0.618 of XA, C overshoots A by "
      "1.13-1.414 of XA, D = C - 0.786*(C-X), the only D in the zoo measured off XC)")
cyp_g, n_piv, n_cand_cyp, cyp_by_ticker = pooled(placebo=False, cost=0.0)
print(f"  pooled confirmed pivots: {n_piv:,}  |  Cypher XABC candidates: {n_cand_cyp:,}  "
      f"|  D-touches: {len(cyp_g):,}")

for h in HORIZONS:
    sf = st.summarize(cyp_g, f"ret_gross_{h}")
...
# --------------------------------------------------------------------------- #
print("\n# SIGNAL AXIS — Cypher D-touch fade vs a random-day BASE RATE "
      f"(matched bullish/bearish mix, pooled over {BASE_SEEDS} seeds), Bonferroni-corrected")

dir_mix_all = float((cyp_g["reversal_dir"] > 0).mean()) if not cyp_g.empty else 0.5
...
base_g = base_rate_pool(cyp_by_ticker)
tests = []  # (label, welch_t, n_cyp, n_base)
for h in HORIZONS:
    sf = st.summarize(cyp_g, f"ret_gross_{h}")
    sb = st.summarize(base_g, f"ret_gross_{h}")
    wt = st.welch_t(cyp_g[f"ret_gross_{h}"], base_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: CYPHER mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
for t in data.BASKET:
    ...
    if n_ev == 0:
        ...
        continue
    mix = float((l_cyp["reversal_dir"] > 0).mean())
    frames = [st.base_rate_ledger(bars[t], n_ev, mix, horizons=HORIZONS, cost_bps=0.0,
                                  seed=st._seed_from(703, t, s)) for s in range(BASE_SEEDS)]
    br_t = pd.concat(frames, ignore_index=True)
    sf = st.summarize(l_cyp, "ret_gross_5")
    sb = st.summarize(br_t, "ret_gross_5")
    wt = st.welch_t(l_cyp["ret_gross_5"], br_t["ret_gross_5"])
    tests.append((f"{t} (5-day)", wt))
    print(f"  {t:5s} CYPHER n={sf['n']:3d} mean={sf['mean_bps']:+7.2f}bps   |   "
...
# --------------------------------------------------------------------------- #
print("\n# THIRD AXIS (myth-check) — does the precise 0.786 XC retracement beat "
      "a placebo retracement zone built off the identical pivots?")
...
for h in HORIZONS:
    ...
    wt = st.welch_t(cyp_g[f"ret_gross_{h}"], plac_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: CYPHER mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
print(f"\n  Cypher beats placebo on {n_beats}/{len(data.BASKET)} tickers (5-day)")
...
null_ts = []
for s_ in range(20):
    ledger = pooled_synth(0.0, 703 + s_)
...
print(f"  planted mean_rev=0.12 (seed 703): n={sp_planted['n']}  mean={sp_planted['mean_bps']:+.2f}bps  "
