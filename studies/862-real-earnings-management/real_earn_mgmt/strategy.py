"""Strategy + inference for Study 862 — Real Earnings Management.

The claim: firms hit consensus by **real actions** — cutting discretionary expense (R&D, SG&A)
and overproducing to lower unit COGS — leaving abnormally LOW discretionary expense and
abnormally HIGH production. We aggregate the two abnormal (residual) pieces into a REM proxy
(``rem = ab_prod − ab_disx``, higher = more manipulation), rank firms at each 10-Q/10-K filing
date (zero look-ahead), and test:

1. **Forward returns — the calendar-time long-short (PRIMARY).** Each month-end, rank the names
   carrying a fresh REM signal into terciles; go long the top (most real management), short the
   bottom, equal-weight; earn the NEXT month's return (one execution lag). The decisive number is
   the **Newey-West (HAC) t** of that monthly long-short series. The subsequent-performance
   literature (Cohen-Zarowin 2010; Gunny 2010) predicts high-REM firms *under*-perform, so a
   negative, significant long-short would *support* the claim (buy the clean firms, short the
   manipulators) and a positive, significant one would be a wrong-sign refutation.

2. **Forward returns — the pooled event drift (CROSS-CHECK).** Bucket all (ticker, filing) events
   by REM, measure top-minus-bottom forward drift over 1m/1q/2q horizons, one-sample t + a
   label-shuffle placebo.

3. **Does REM predict an operating reversal? (3rd axis).** Overproduction inflates *current* gross
   margin (fixed cost spread thin) and must reverse; cutting R&D/SG&A borrows from the future.
   Pooled OLS of the **forward change in gross margin** on REM — the mechanism check.

Honesty rails: one documented execution lag (signal at month/filing t, return of t+1); costs =
one-way × NAV × turnover, shorts pay borrow; survivorship named on the Signal axis
(current-survivors manufacturer basket). Given the thin, uneven EDGAR panel, expect a modest,
likely-uncertifiable answer.
"""
...
def calendar_ls(prices: pd.DataFrame, events: pd.DataFrame, signal_col: str = "rem",
    ...
    top − bottom), long, short, n (cross-section size), turnover (one-way, fraction replaced).
    """
...
def calendar_ls_net(ls: pd.DataFrame, cost_bps: float = 20.0, borrow_bps_ann: float = 100.0,
    ...
    ``borrow_bps_ann`` annualised (÷12 monthly). The friction is charged against the *magnitude*
    of the gross edge (its sign is preserved), so a costed |edge| always shrinks. Net series →
    NW t / Sharpe.
    """
    c = cost_bps / 1e4
    borrow_m = (borrow_bps_ann / 1e4) / 12.0
    gross = ls["ls"].to_numpy()
    drag = 2.0 * c * ls["turnover"].to_numpy() + borrow_m
    net = gross - np.sign(gross) * drag       # friction always shrinks the |edge|
...
def event_drift_frame(prices: pd.DataFrame, events: pd.DataFrame, horizon: int,
                      lag: int = 1, signal_col: str = "rem") -> pd.DataFrame:
...
def placebo_pvalue(frame: pd.DataFrame, n_draws: int = 10_000, n_buckets: int = 3,
    ...
    ``p`` = P[|shuffled long-short| ≥ |observed|] (two-sided — the real-tape sign is not assumed)."""
    s = frame["signal"].to_numpy(); d = frame["drift"].to_numpy()
...
# --------------------------------------------------------------------------- #
# 3rd axis — does REM predict a forward operating (gross-margin) reversal?
# --------------------------------------------------------------------------- #
def leads_operating(events: pd.DataFrame) -> dict:
    """Does high REM predict a DROP in the next quarter's gross margin (the reversal)?

    Overproduction inflates current gross margin (fixed overhead spread over more units) and must
    reverse; cutting R&D/SG&A borrows from the future. Pooled OLS of the forward gross-margin
    change ``Δgm = next_gm − gm`` on REM. Reports the slope b, its (iid) t, R², Pearson
    correlation, and the forward-Δgm spread between the top and bottom REM terciles. A negative,
    significant b is the operating reversal the value-destruction story predicts. (t is iid-pooled
    — events cluster by quarter, so read it as suggestive, not a calendar-robust HAC statistic.)
    """
    fr = events.dropna(subset=["rem", "gm", "next_gm"]).copy()
    x = fr["rem"].to_numpy()
    y = (fr["next_gm"].to_numpy() - fr["gm"].to_numpy())
    ok = np.isfinite(x) & np.isfinite(y)
    x, y = x[ok], y[ok]
    n = len(x)
    if n < 10:
        return {"n": n, "slope": float("nan"), "t": float("nan"), "r2": float("nan"),
                "corr": float("nan"), "top_dgm": float("nan"), "bot_dgm": float("nan"),
                "spread": float("nan")}
    ...
    b = _bucketize(x, 3)
    top_dgm = float(y[b == 2].mean()); bot_dgm = float(y[b == 0].mean())
    return {"n": n, "slope": float(beta[1]), "t": t, "r2": r2, "corr": corr,
            "top_dgm": top_dgm, "bot_dgm": bot_dgm, "spread": top_dgm - bot_dgm}
...
def synthetic_detect(prices: pd.DataFrame, events: pd.DataFrame) -> dict:
    ...
    ls = calendar_ls(prices, events, signal_col="rem", n_buckets=3, min_names=6,
                     staleness_days=120)
