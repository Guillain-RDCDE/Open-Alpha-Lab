"""Strategy + inference for Study 786 — Flu-Season.

The claim: **the pharmacy tape (CVS) rallies into flu season.** Two windows, both
anchored on the fixed October-1 flu-season start (a CDC calendar convention known years
ahead):

* **The run-up (the "rally into the season").** CVS's abnormal return (CVS total-return
  minus the SPY total-return benchmark) over the K trading sessions ENDING on the last
  close on/before the October-1 season start. Tested at K = 10 (~2 weeks) and K = 21
  (~1 month). If the folklore is right this is positive.

* **The in-season leg (does the anticipation give back once the season is here?).** CVS's
  abnormal return over the K sessions STARTING from the season-start close. Tested at
  K = 10 and K = 21. If "rally into it then fade" is the story, the run-up is positive and
  the in-season leg is negative.

Because each flu-season year is a single independent event (not a daily series), the
primary statistic is a **one-sample t** of the abnormal return across events (n = the
number of years with CVS+SPY coverage). A random-window placebo (drawing many random,
non-season K-session windows from CVS's own history vs SPY) checks whether the observed
...
POST_SHORT_K = 10   # ~2 weeks in-season
POST_LONG_K = 21    # ~1 month in-season
...
def build_event_table(prices: dict[str, pd.Series], cost_bps: float = COST_BPS
    ...
    """One row per flu-season year: run-up + in-season abnormal returns + inclusion.

    A row is INCLUDED only if CVS and SPY both have cached history covering
    ...
    for year, keydate in dt.EVENTS:
        row = dict(year=year, season_start=keydate)
        anchor_ts = pd.Timestamp(keydate)
        on_or_before = common[common <= anchor_ts]
        if len(on_or_before) == 0:
            row.update(included=False, reason="no CVS/SPY coverage at the anchor")
            rows.append(row)
            continue
        ...
        def ar(i_start, i_end):
            r_a = cvs.loc[common[i_end]] / cvs.loc[common[i_start]] - 1.0
...
def placebo_pvalue(events: pd.DataFrame, prices: dict[str, pd.Series], col: str,
                   ...
                   n_seeds: int = 20, n_draws_per_seed: int = 200, base_seed: int = 813,
                   tail: str = "right") -> dict:
    """For each INCLUDED event, redraw a random (non-season) k-session window on CVS vs
    ...
    "left" (a claim of NEGATIVE mean, e.g. an in-season give-back -> p = share of null
    means <= observed).
    """
    cvs = prices[dt.INSTRUMENT]
    spy = prices[dt.BENCHMARK]
    ...
    for s in range(n_seeds):
        ...
        for _ in range(n_draws_per_seed):
            ...
            for _e in range(n_events):
                ...
                r_a = cvs.loc[d_end] / cvs.loc[d_start] - 1.0
...
def car_path(events: pd.DataFrame, prices: dict[str, pd.Series],
    ...
    """Mean cumulative abnormal return (CVS - SPY) at each offset from -pre..+post
    relative to the season start, normalised so offset 0 = 0%, averaged across all
    INCLUDED events. Negative offsets are the run-up; positive is the in-season window.
    """
    cvs = prices[dt.INSTRUMENT]
    spy = prices[dt.BENCHMARK]
    common = cvs.index.intersection(spy.index).sort_values()
    ...
    for _, row in inc.iterrows():
        ...
        base_a, base_s = cvs.loc[common[p]], spy.loc[common[p]]
        vals = []
        for o in offsets:
            d = common[p + o]
            r_a = cvs.loc[d] / base_a - 1.0
...
def synthetic_detect(bump: float, seed: int, k: int = PRE_SHORT_K,
    ...
    pre-season bump. ``side='pre'`` measures the run-up window [p-k..p); ``side='post'``
    measures the in-season window [p..p+k)."""
