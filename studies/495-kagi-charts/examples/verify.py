"""Reproducible headline run for Study 495 — Kagi Charts.
...
from kagi_charts import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch switch-vs-random test
    HAVE_SCIPY = True
except Exception:  # noqa: BLE001
    HAVE_SCIPY = False

ASOF = "2026-05-31"
REV = st.DEFAULT_REVERSAL
...
print("# Kagi charts — mechanical yang-switch (buy thick line) on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    span = (spy.index[-1] - spy.index[0]).days / 365.25
    print(f"window   : {spy.index[0].date()} -> {spy.index[-1].date()}  ({span:.1f} years)")
    print(f"SPY fp   : {data.fingerprint(spy)}   tickers: {data.DEFAULT_TICKERS}   reversal={REV:.0%}")

    print("\n# Pooled yang-switch vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'sw_bps':>8} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.yang_switch_entries(c, reversal=REV)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>8.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total yang switches (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: yang-switch one-sample t and switch-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.yang_switch_entries(c, reversal=REV)
        re = st.random_entries(c, max(len(e), 50), seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} switch={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Threshold-scramble GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.threshold_scramble_placebo(_load("SPY")["close"], 20, reversal=REV, n_draws=500, seed=495)
...
for edge in (0.0, 0.25):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.yang_switch_entries(c, reversal=REV), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  switch={s['mean_bps']:+7.1f}bps  "
