"""Data layer for Study 983 — The Weekend Oracle.

Bitcoin's daily bars from Yahoo! Finance are **UTC days**: the day-*t* bar closes at
00:00 UTC, which is 20:00 New York time — three hours *after* the equity close. Every alignment
in this study is built with that fact in front of it, because it is the single easiest way to
turn a clock artefact into a fake signal.
...
# Crypto trades 24/7; equities do not. That asymmetry is the whole study.
CRYPTO = "BTC-USD"
CRYPTO_2 = "ETH-USD"
EQUITY = "SPY"
TECH = "QQQ"
GOLD = "GLD"                 # the other "trades-a-lot" asset, as a control
CASH = "BIL"
TICKERS = (CRYPTO, CRYPTO_2, EQUITY, TECH, GOLD, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call weekendoracle.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 983) -> tuple[pd.DataFrame, dict]:
