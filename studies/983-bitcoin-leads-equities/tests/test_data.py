"""Data-layer tests for Study 983 — synthetic determinism offline, cache-gated on tape."""
...
from weekendoracle import data  # noqa: E402


def test_synthetic_panel_is_deterministic():
    a, ca, _ = data.synthetic_panel(n_assets=5, n_years=4, seed=983)
    b, cb, _ = data.synthetic_panel(n_assets=5, n_years=4, seed=983)
    assert np.allclose(a.to_numpy(), b.to_numpy())
    assert np.allclose(ca.to_numpy(), cb.to_numpy())


def test_synthetic_panel_seed_sensitive():
    a, _, _ = data.synthetic_panel(n_assets=5, n_years=4, seed=983)
    b, _, _ = data.synthetic_panel(n_assets=5, n_years=4, seed=983 + 1)
    assert not np.allclose(a.to_numpy(), b.to_numpy())


def test_synthetic_panel_shape_and_index():
    prices, cash, truth = data.synthetic_panel(n_assets=7, n_years=6, seed=983)
...
def test_signal_strength_scales_the_plant():
    _, _, t1 = data.synthetic_panel(signal_strength=1.0, seed=983)
    _, _, th = data.synthetic_panel(signal_strength=0.5, seed=983)
    _, _, t0 = data.synthetic_panel(signal_strength=0.0, seed=983)
    assert t0["alpha_vol_eff"] == 0.0
    assert t1["alpha_vol_eff"] > th["alpha_vol_eff"] > 0.0


def test_synthetic_cash_is_monotone():
    _, cash, truth = data.synthetic_panel(n_years=5, seed=983)
    assert (cash.diff().dropna() > 0).all()
    assert truth["cash_rate_ann"] > 0


def test_fingerprint_stable_and_sensitive():
    a, _, _ = data.synthetic_panel(n_assets=4, n_years=3, seed=983)
    b, _, _ = data.synthetic_panel(n_assets=4, n_years=3, seed=983 + 1)
