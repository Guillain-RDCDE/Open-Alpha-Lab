"""Data layer for Study 990 — Counting the Breaks.

SPY's history reaches back to 1993, which gives roughly 8,300 sessions — and at a
99% confidence level that is only about 83 expected breaches. The whole study is an exercise in
what can and cannot be concluded from 83 events, which is less than most risk reports assume.
...
# A long equity history, plus assets whose tails are shaped very differently.
EQUITY = "SPY"
NASDAQ = "QQQ"
BONDS = "TLT"
GOLD = "GLD"
...
CRYPTO = "BTC-USD"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, BONDS, GOLD, EMERGING, CRYPTO, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call breaks.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 990) -> tuple[pd.DataFrame, dict]:
