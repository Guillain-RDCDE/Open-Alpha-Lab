"""Data layer for Study 698 — ABCD-Harmonic.

Two ingredients:

* **Real tape.** Daily OHLC for SPY + a five-stock basket (QQQ, AAPL, MSFT, TSLA,
  NVDA — the identical basket used by sibling study
  `77-golden-mean <../77-golden-mean/>`_, for apples-to-apples comparison), from
...
* **Synthetic world.** A deterministic, seeded daily price path with a TUNABLE
  mean-reversion knob (``mean_rev``): at ``mean_rev = 0`` price is a pure random
  walk (the null the ABCD detector must NOT manufacture significance from); at
  ``mean_rev > 0`` price is nudged back toward a slow moving average, giving *any*
  well-formed pullback-and-continuation structure (including a completed
  AB=CD leg) genuine forecastable reversion. This is the "faithful engine / power
  check" the desk's harness runs on every pattern-recognition study — never cited
  in support of the real-tape stamp.

Pure numpy + pandas + stdlib on the offline path. ``fetch()`` (network) runs once
per ticker to build the cache and is never imported by the notebooks' offline cells.
"""
...
def _cache_path(ticker: str) -> str:
    return os.path.join(CACHE_DIR, f"abcd_{ticker.lower()}.csv")
...
def synthetic_world(mean_rev: float = 0.0, n_days: int = 2500, annual_vol: float = 0.22,
                    start: str = "2010-01-04", seed: int = 698) -> pd.DataFrame:
    """A deterministic daily OHLC(ish) tape with a TUNABLE mean-reversion tendency.

    Log returns follow ``r_t = -mean_rev * (price_t / ema_t - 1) + eps_t`` with a
    slow (span-50) EMA anchor — the same generic "does price reliably pull back
    toward an anchor" mechanism that would make ANY equal-legged reversal
    projection (Fibonacci-labelled or not) show a genuine edge once
    ``mean_rev > 0``. ``mean_rev = 0`` is the null: the ABCD detector's Welch/HAC
    split must NOT reach significance on it, across many seeds.
