...
def build_curious():
    cells = [
        md(
            ...
            "this way in a year. So we asked the one question the thread skips — **how long "
            "does the free nickel actually sit there before someone faster takes it?**\n\n"
            ...
            "every chart below is generated by the code beside it. The reproducible core "
            "runs on a **synthetic** market with a *baked-in* 6-minute half-life (the cached "
            "real prices are git-ignored) — on purpose: that's where our stopwatch *works*, "
            "proving the code, so the real-world result (quoted from "
            "[`../docs/results.md`](../docs/results.md)) is a fact about the market, not a "
            "bug. House style in [METHODOLOGY.md](../../../METHODOLOGY.md)."
        ),
        code(BOOT),
...
def build_quants():
    cells = [
        md(
            "# The Vanishing Penny — a quantitative teardown 🔬\n"
            "### First-passage hypothesis · episodes · two half-life estimators · resolution sweep · capture integral\n\n"
            "![Signal: Real](https://img.shields.io/badge/Signal-Real-2ea44f?style=flat-square)\n"
            "![Tradability: Mirage](https://img.shields.io/badge/Tradability-Mirage-c0392b?style=flat-square)\n"
            "![Execution--moat: Confirmed](https://img.shields.io/badge/Execution--moat-Confirmed-8b949e?style=flat-square)\n\n"
            "The deep companion to the "
            "[notebook for the curious](01_for_the_curious.ipynb) — *same seven beats, every "
            "claim now carrying its standard error.* Thesis: the `YES+NO` arbitrage is "
            "**real** (risk-free, ~\\$40M documented) but its **half-life is below the public "
            "minute tape**, so realised retail capture `½^(latency/H)` ≈ 0 — a mirage that is "
            "entirely an execution moat, not a signal failure.\n\n"
            "> ⚠️ **Not investment advice.** Executed on the **synthetic book** (baked-in "
            "6-min half-life), where the estimator works — this validates the stopwatch "
            "end-to-end; the real verdict is quoted from "
            "[`../docs/results.md`](../docs/results.md) (`examples/verify_real.py`). Fixed "
            "seeds; no network. Method & literature map: "
