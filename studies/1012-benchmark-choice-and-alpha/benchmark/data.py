"""Data layer for Study 1012 — Choose Your Benchmark.

Nine candidate benchmarks spanning size, style and geography, and seventeen
"funds" — sector and style products with genuine, known tilts. Using products whose exposures
are documented rather than opaque active funds is deliberate: when the true factor loading is
knowable, a mis-specified benchmark's damage can be attributed rather than guessed at.
...
# Candidate benchmarks, from the crudest to the most specific.
MARKET = "SPY"
TOTAL = "VTI"
LARGE_VALUE = "IWD"
LARGE_GROWTH = "IWF"
...
CASH = "BIL"
BILLS = "SHY"
BENCHMARKS = (MARKET, TOTAL, LARGE_VALUE, LARGE_GROWTH, SMALL, SMALL_VALUE,
              SMALL_GROWTH, MID, EAFE)
# "Funds" whose alpha we will measure: sector and style products with real tilts.
FUNDS = ("XLK", "XLF", "XLE", "XLV", "XLI", "XLP", "XLY", "XLU", "XLB", "VNQ",
         "QQQ", "IWM", "IWD", "IWF", "EFA", "GLD", "TLT")
TICKERS = tuple(dict.fromkeys(BENCHMARKS + FUNDS + (CASH, BILLS)))
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call benchmark.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1012) -> tuple[pd.DataFrame, dict]:
