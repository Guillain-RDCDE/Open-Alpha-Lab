"""Data layer for Study 928 — Odd-Lot Priority (the odd-lot leg of an issuer self-tender).

Three inputs, one shape (date-indexed daily total-return closes plus an event table):

- ``EVENTS`` — the **same** hardcoded, publicly-verifiable table of 180 issuer *modified
  Dutch auction* self-tender offers, 2010-01-01 → 2025-12-31, that Study 927 uses. It is
  ...
  **earliest SC TO-I filing date of each cluster** is taken as the event date — the
  offer's *commencement*, the day the tender documents become public, so it is
  point-in-time clean by construction. Sharing the list with 927 is deliberate: the two
  studies ask different questions of the *same* events, and any difference in answer is
  therefore about the question, not about the sample.

- ``fetch`` / ``load_prices`` — daily **total-return** closes from Yahoo! Finance
  (``yfinance``, ``auto_adjust=True``) for every event ticker plus **SPY** (the market leg
  the round trip is measured against) and **BIL** (the cash leg for the excess-of-cash
  race). ``fetch`` touches the network and caches parquet in the **shared**
  ``studies/_cache`` (retry up to 4×); ``load_prices`` reads that cache **offline** and
  never imports yfinance.

- ``synthetic_panel`` / ``synthetic_daily`` — *deterministic, offline* generators. The
  panel plants an announcement-day run-up and a post-expiry give-back whose sizes scale
  with ``signal_strength``; at ``signal_strength=0`` the world is a pure null and the
  odd-lot machinery must stay quiet. Seeds are fixed → tests are deterministic.

**What the tape cannot give us.** A tender's *terms* — the price range, the final clearing
price, the expiry date, the proration factor — live in the SC TO-I/A and the closing 8-K,
not in a price series. This study therefore treats four inputs as declared
**PROXIES / ASSUMPTIONS**, each swept in ``strategy.py`` and named in the README:

1. ``PREMIUM_DEFAULT`` — the clearing price as a premium over the pre-announcement close
   (13%, the mid of the Comment-Jarrell / Bagwell self-tender range). Swept 5–25%.
2. ``EXPIRY_DAYS_DEFAULT`` — 21 trading days from commencement, the Rule 14e-1 statutory
   minimum of 20 business days plus one. Swept 20–40.
3. ``PRORATION_DEFAULT`` — the fraction of a *round-lot* holder's tendered shares actually
   bought in an oversubscribed offer (0.35). Swept 0.15–1.00; at 1.00 the offer is
   under-subscribed and odd-lot priority is worth exactly nothing.
4. ``TENDER_FEE_USD_DEFAULT`` — the flat broker fee for processing a voluntary corporate
   action ($30). Swept $0–$50. On an odd lot this is the dominant cost.

**Two named biases.** (1) *Survivorship / ticker recovery*: the CIK→ticker map comes from
SEC's current ``company_tickers.json``, so issuers acquired, taken private or liquidated
carry no ticker and drop out — and an issuer that went private *because* of the tender is
exactly the case where the post-expiry price would have been most informative. (2) *Price
tape*: yfinance serves a ticker's history under its **current** symbol, so issuers whose
symbol was recycled after a bankruptcy are excluded by hand (``EXCLUDED_TICKERS``).

No look-ahead is baked in here — that discipline lives in ``strategy.py`` (the offer is
public at the close of day ``t``; the odd lot is bought at the close of ``t+1``).
"""
...
# The exact, re-runnable EDGAR full-text-search query behind EVENTS (shared with 927).
EDGAR_QUERY = (
...
# The market leg (the round trip is measured against SPY) and the cash leg (BIL).
MARKET = "SPY"
CASH = "BIL"

# --------------------------------------------------------------------------- #
# Declared PROXIES / ASSUMPTIONS — none of these is on the price tape.
# Every one is swept in strategy.py and named in the README.
# --------------------------------------------------------------------------- #
PREMIUM_DEFAULT = 0.13          # clearing price / pre-announcement close − 1
PREMIUM_GRID = (0.05, 0.09, 0.13, 0.17, 0.21, 0.25)
EXPIRY_DAYS_DEFAULT = 21        # trading days from commencement to expiry
EXPIRY_GRID = (20, 25, 30, 40)
PRORATION_DEFAULT = 0.35        # round-lot fill fraction when oversubscribed
PRORATION_GRID = (0.15, 0.25, 0.35, 0.50, 0.75, 1.00)
TENDER_FEE_USD_DEFAULT = 30.0   # flat broker corporate-action fee, per event
TENDER_FEE_GRID = (0.0, 15.0, 30.0, 50.0)
COST_BPS_DEFAULT = 15.0         # one-way × NAV on each market execution
# The bps grid runs to 100 because a third of these issuers are micro-caps (SCKT, OPXS,
# MTEX, BMNM, IZEA, WHLR...) where the touch on a 99-share lot is far wider than 15 bps,
# and cost_bps is charged three times per hedged round trip (buy + SPY round trip).
COST_BPS_GRID = (0.0, 15.0, 40.0, 100.0)
ODD_LOT_POSITION_USD = 5000.0  # 99 shares of a ~$50 stock: the hard capacity ceiling
...
# Identical to studies/927-dutch-auction-buyback (same tender list, different question).
# --------------------------------------------------------------------------- #
EVENTS = (
...
def usable_events() -> tuple:
    """``EVENTS`` with the recycled-symbol names dropped, in date order."""
    rows = [e for e in EVENTS if e[0] not in EXCLUDED_TICKERS]
...
def fetch(
    ...
    issuers paid large special dividends inside the measurement windows and a *round trip*
    that ignored them would mis-state the give-back.
...
def have_real(cache_dir: str = DEFAULT_CACHE) -> bool:
    """True iff the market and cash legs plus at least 50 issuer tapes are cached."""
...
def load_prices(
    ...
    always incomplete, and the drop-outs are named as survivorship in the README — but a
    missing **SPY** or **BIL** raises, because neither the market-adjusted round trip nor
    the cash race is defined without them. Never touches the network.
    """
    if tickers is None:
        tickers = all_tickers()
    out = {}
        ...
        if not os.path.exists(path):
            if require_market and tk in (MARKET, CASH):
                raise FileNotFoundError(
                    ...
                    f"Call odd_lot.data.fetch() once to populate the shared cache."
                )
            continue
...
def synthetic_daily(
    ...
    A market factor with beta 1.0 and an idiosyncratic sleeve — the minimum structure a
    market-adjusted round-trip measurement needs. Deterministic given ``seed``.
    """
    rng = np.random.default_rng(seed)
    dates = pd.bdate_range(start=start, periods=n_days)
    d = drift_ann / TRADING_DAYS_PER_YEAR
    s = vol_ann / np.sqrt(TRADING_DAYS_PER_YEAR)
    mkt = rng.normal(0.07 / TRADING_DAYS_PER_YEAR,
...
def synthetic_panel(
    ...
    runup_bps: float = 500.0,        # planted announcement run-up, close(t-6) -> close(t+1)
    giveback_bps: float = 400.0,     # planted post-expiry give-back off the expiry price
    expiry_days: int = EXPIRY_DAYS_DEFAULT,
    vol_ann: float = 0.34,
    start: str = "2012-01-02",
    seed: int = 928,
) -> tuple:
    """A panel of synthetic issuers with a *planted* self-tender in each.

    Every name gets an independent idiosyncratic tape on top of one shared market factor
    (so the panel carries the same cross-sectional correlation problem as the real one).
    Two effects are planted, both scaled by ``signal_strength``:

    - a **run-up**: ``runup_bps`` of abnormal return spread over the six sessions from
      ``t-5`` to ``t+1``, i.e. exactly the move that eats the tender premium before an
      outside buyer can act;
    - a **give-back**: ``giveback_bps`` of abnormal return *lost* over the five sessions
      after expiry, i.e. exactly the move that makes odd-lot priority worth something.

    At ``signal_strength = 0`` neither is planted and the odd-lot machinery must stay
    quiet. Returns ``(panel, events, truth)`` where ``panel`` is ``{name: close Series}``
    including a ``"MKT"`` market leg and a ``"CASH"`` leg, ``events`` is a list of
    ...
    panel = {"MKT": pd.Series(100.0 * np.exp(np.cumsum(mkt_r)), index=idx)}
    panel["CASH"] = pd.Series(
        np.cumprod(np.full(n_days, (1.0 + 0.02) ** (1.0 / TRADING_DAYS_PER_YEAR))),
        index=idx)

    lo, hi = 60, n_days - (expiry_days + 40)
    events = []
    runup = signal_strength * runup_bps * 1e-4
    giveback = signal_strength * giveback_bps * 1e-4
    for i in range(n_events):
        name = f"SYN{i:03d}"
        idio = rng.normal(0.0, s, n_days)
        p = int(rng.integers(lo, hi))
        idio[p - 4: p + 2] += runup / 6.0            # the (t-5, t+1) run-up
        idio[p + expiry_days + 1: p + expiry_days + 6] -= giveback / 5.0   # the give-back
        panel[name] = pd.Series(50.0 * np.exp(np.cumsum(mkt_r + idio)), index=idx)
        events.append((name, str(dates[p].date()), f"SYN-{i:04d}", f"Synthetic Issuer {i}"))

    truth = {
        "signal_strength": signal_strength,
        "runup_bps": runup_bps, "giveback_bps": giveback_bps,
        "runup_planted_bps": float(runup * 1e4),
        "giveback_planted_bps": float(giveback * 1e4),
        "n_events": n_events, "n_days": n_days, "expiry_days": expiry_days,
        "vol_ann": vol_ann, "seed": seed, "start": start,
    }
