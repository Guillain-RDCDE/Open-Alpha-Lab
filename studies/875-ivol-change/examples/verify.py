"""Reproducible headline run for Study 875 — Idiosyncratic-Vol Change.
...
print("# Idiosyncratic-Vol Change — does RISING residual vol precede lower returns?")
...
sp = st.delta_spreads(ret, mkt, window=21, frac=0.3)
h = st.delta_stats(sp)
print(f"\nsort: delta-IVOL (recent-21d minus prior-21d residual vol), long bottom30% "
      f"(falling) / short top30% (rising), {h['n_days']} days "
      f"(median {int(sp['n'].median())} names/day)")
print("# THE HEADLINE — long-falling-idio-vol / short-rising-idio-vol spread")
print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
      f"one-sample t = {h['t_1s']:+.2f}")
print(f"  books : falling-ivol {h['lo_bps']:+.2f} vs rising-ivol {h['hi_bps']:+.2f} bps  "
...
sig_off = (pl['obs_bps'] - pl['placebo_mean_bps']) / pl['placebo_sd_bps']
print(f"  observed sits {sig_off:+.2f} sigma from the placebo mean")

print("\n# ADDITIVITY — is the idio-vol CHANGE anything beyond the idio-vol LEVEL (501)?")
ad = st.additivity(ret, window=21, frac=0.3)
print(f"  idio-vol LEVEL spread: {ad['level_bps']:+.2f} bps/day (NW t = {ad['level_t_nw']:+.2f})")
print(f"  corr(change, level) = {ad['corr']:+.3f}  beta = {ad['beta']:+.3f}")
print(f"  change alpha vs level: {ad['alpha_bps']:+.2f} bps/day (NW t = {ad['alpha_t_nw']:+.2f})")
...
for lo, hi, lbl in [("2010-01-01", "2018-01-01", "2010-2017"),
    ...
    ts = st.delta_stats(sub)
    print(f"  {lbl}: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-falling-idio-vol / short-rising-idio-vol, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=875 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.002, seed=875, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.002, seed 875): spread NW t = {sy['t_nw']:+.2f}, "
