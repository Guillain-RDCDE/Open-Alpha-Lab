"""Data layer for Study 129 (Heikin-Ashi).

Two tapes, one shape (a daily OHLCV frame):

- ``synthetic_daily`` — a *deterministic, offline* generator.  A bar-level AR(1) drift
  knob (``momentum``) lets us dial the only thing the HA colour-flip rule can possibly
  harvest: return persistence.  ``momentum=0`` is a pure random walk — a fair coin —
  so a test can assert the HA flip beats random *only* when we plant a trend, and not
  otherwise.  This is the study's null in a bottle.
- ``fetch_daily`` — the real Yahoo! daily tape (``yfinance``), cache-only by default so
  ...
  long (years of data), giving HA enough warmup bars and enough independent flips for
  meaningful inference.

Heikin-Ashi construction (recursive, causal, no look-ahead):
...
The ``colour`` is green (HA_close >= HA_open) or red (HA_close < HA_open).  A
``colour flip`` is a change from green to red or vice versa; we enter at the *next*
...
def synthetic_daily(
    ...
    - ``momentum = 0``   -> a martingale; the HA colour flip is a fair coin.
...
def fetch_daily(
    ...
    a parquet under ``_cache/``).  The daily tape is long — 15 years of history by
    default — giving HA enough warmup bars and enough independent colour flips for a
    meaningful inference.
    """
