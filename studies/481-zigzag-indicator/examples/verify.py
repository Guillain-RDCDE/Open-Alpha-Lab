"""Reproducible headline run for Study 481 — ZigZag Indicator.
...
from zigzag_indicator import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch up-leg-vs-random test
    HAVE_SCIPY = True
...
print("# ZigZag indicator — mechanical confirmed-up-leg long on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    span = (spy.index[-1] - spy.index[0]).days / 365.25
    print(f"window   : {spy.index[0].date()} -> {spy.index[-1].date()}  ({span:.1f} years)")
    print(f"SPY fp   : {data.fingerprint(spy)}   tickers: {data.DEFAULT_TICKERS}   pct={PCT}")

    print("\n# Pooled confirmed-up-leg vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'upleg_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.confirmed_uppleg_entries(c, pct=PCT)
            re = st.random_entries(c, max(len(e), 50), pct=PCT, seed=7)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total confirmed up-legs (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: up-leg one-sample t and up-leg-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.confirmed_uppleg_entries(c, pct=PCT)
        re = st.random_entries(c, max(len(e), 50), pct=PCT, seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} upleg={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Shuffled-leg GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_leg_placebo(_load("SPY")["close"], 20, pct=PCT, n_draws=500, seed=481)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => up/down geometry not load-bearing)")
else:
...
print("  edge=0 must NOT reach significance; planted post-turn drift must light up.")
for edge in (0.0, 0.40):
    px, _ = data.synthetic_panel(edge=edge, seed=481, n_days=4000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.confirmed_uppleg_entries(c, pct=PCT), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  upleg={s['mean_bps']:+7.1f}bps  "
