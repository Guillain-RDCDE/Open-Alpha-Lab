...
the cost-discipline long-short lights up *only when a link is planted*."""
...
from sga_stickiness import strategy as st  # noqa: E402
...
def test_calendar_ls_null_is_quiet(null_panel):
    ...
    assert abs(stt["t_nw"]) < 3.0                # no planted link -> no strong HAC t


def test_calendar_ls_recovers_planted_link(planted_panel):
    prices, ev = planted_panel
    ls = st.calendar_ls(prices, ev, staleness_days=120)
    stt = st.calendar_ls_stats(ls)
    assert stt["mean_bps"] > 0                    # high-discipline beats sticky
...
# --------------------------------------------------------------------------- #
# 3rd axis — stickiness predicts weaker future profitability
# --------------------------------------------------------------------------- #
def test_predicts_profitability_planted_negative():
    rng = np.random.default_rng(5)
    n = 400
    sticky = rng.normal(0.2, 0.15, n)
    roa_chg = -0.10 * sticky + rng.normal(0, 0.01, n)   # stickier -> ROA falls more
    ev = pd.DataFrame({"stickiness": sticky, "roa_chg": roa_chg})
    out = st.predicts_profitability(ev)
    assert out["slope"] < 0 and out["t"] < -5.0
    assert out["spread"] < 0                              # top-stickiness tercile under-earns


def test_predicts_profitability_no_relation():
    rng = np.random.default_rng(6)
    n = 400
    ev = pd.DataFrame({"stickiness": rng.normal(0.2, 0.15, n),
                       "roa_chg": rng.normal(0, 0.02, n)})
    out = st.predicts_profitability(ev)
