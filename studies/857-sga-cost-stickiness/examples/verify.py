"""Reproducible headline run for Study 857 — SG&A Cost Stickiness.
...
from sga_stickiness import data, strategy as st  # noqa: E402

print("# SG&A Cost Stickiness — do sticky-cost firms under-earn?")
...
print(f"[data] stickiness (-β2): mean {ev['stickiness'].mean():+.3f}  "
      f"sd {ev['stickiness'].std():.3f}  "
      f"share sticky (β2<0) {(ev['stickiness'] > 0).mean()*100:.0f}%")

print("\n# PRIMARY — calendar-time tercile long-short on cost DISCIPLINE (disc = -stickiness)")
print("#   (rank fresh signals monthly, long leanest / short stickiest, earn next month; one lag)")
...
print("\n# CROSS-CHECK — pooled event drift (tercile sort by disc, 1-day-lag entry)")
...
print(f"  monotonicity (63d disc terciles low->high): {[f'{b*100:+.2f}%' for b in bm]}")

print("\n# ERA SPLIT — calendar long-short pre/post the 2020 sales shock")
for lbl, lo, hi in [("pre-2020H2", "2000-01-01", data.ERA_SPLIT),
                    ("2020H2-2026", data.ERA_SPLIT, "2100-01-01")]:
    sub = ls[(ls.index >= lo) & (ls.index < hi)]["ls"].to_numpy()
    if len(sub) >= 3:
        print(f"  {lbl}: n={len(sub)}  mean {sub.mean()*1e4:+.1f} bps  NW t = {st.newey_west_t(sub):+.2f}")

print("\n# THIRD AXIS — does stickiness predict WEAKER future profitability (ROA change)?")
q = st.predicts_profitability(ev)
print(f"  n={q['n']}  slope={q['slope']:+.3f}  t={q['t']:+.2f}  R2={q['r2']:.3f}  corr={q['corr']:+.3f}")
print(f"  next-year ROA change: top stickiness tercile {q['top_roa_chg']*100:+.2f}pp  "
      f"vs bottom {q['bot_roa_chg']*100:+.2f}pp  ->  spread {q['spread']*100:+.2f} pp")
...
print("  planted link. Null checked over 10 seeds (never a single stream).")
null_ts = []
for s_ in range(10):
    p0, e0 = data.synthetic_panel(edge=0.0, seed=857 + s_)
    null_ts.append(st.synthetic_detect(p0, e0)["t_nw"])
null_ts = np.asarray(null_ts)
print(f"  null (edge=0), 10 seeds: mean NW t = {null_ts.mean():+.2f} (sd {null_ts.std(ddof=1):.2f}), "
      f"|t|>=2 in {(abs(null_ts) >= 2).sum()}/10 seeds")
p1, e1 = data.synthetic_panel(edge=0.35, seed=857)
pl = st.synthetic_detect(p1, e1)
print(f"  planted edge=0.35 (seed 857): mean {pl['mean_bps']:+.1f} bps/mo, NW t = {pl['t_nw']:+.2f}")
