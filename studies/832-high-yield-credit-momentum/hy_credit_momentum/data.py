"""Data layer for Study 832 — High-Yield Credit Momentum.

The claim under test: the **trend** (trailing total return) of high-yield credit,
measured *duration-hedged* as **HYG in excess of IEF**, is a risk-on/off **timing**
signal for equities. When the credit trend is positive (HY out-earning duration-matched
Treasuries → risk appetite rising) you go **risk-on → long SPY**; when it turns negative
you **de-risk → hold IEF**. This is the credit **TREND** as a timing switch, distinct from
[115-credit-spreads](../../115-credit-spreads/) which reads the credit-spread **LEVEL** as
a warning gauge.

Two tapes, one shape (a tz-naive daily adjusted-close frame for four ETFs):
...
* **Synthetic world — ``synthetic_panel`` — the positive control.** A deterministic,
  seeded generator with a TUNABLE knob ``edge``: a persistent latent risk-on factor
  drives the HY-vs-IEF credit trend and — only when ``edge > 0`` — the *forward* SPY
  excess-over-IEF return. ``edge = 0`` is the null (credit trend still moves, but carries
  **no** information about forward equity), so the timing test must find nothing.

The four tickers:

* **HYG** — iShares iBoxx $ High Yield Corporate Bond ETF (high-yield credit).
* **IEF** — iShares 7-10 Year Treasury Bond ETF (the duration hedge / de-risk leg).
* **LQD** — iShares iBoxx $ Investment Grade Corporate Bond ETF (an IG cross-check).
* **SPY** — SPDR S&P 500 ETF (the equity leg the timer switches into).
...
def fetch(start: str = START, end: str | None = None, retries: int = 4) -> str:
    ...
    for attempt in range(retries):
        try:
            ...
            # Extract the total-return Close for each ticker into a flat frame.
...
# --------------------------------------------------------------------------- #
# Synthetic world — planted credit-trend -> forward-equity timing (the control)
# --------------------------------------------------------------------------- #
def synthetic_panel(
    edge: float = 0.0,
    seed: int = 832,
    ...
    credit-trend → forward-equity timing relation.

    A persistent latent **risk-on factor** ``f[t]`` (AR(1), autocorrelation
    ``factor_rho``) drives the *duration-hedged credit* excess return and — only when
    ``edge > 0`` — the **forward** SPY excess-over-IEF return::
        ...
        spy_r[t]  = ief_r[t] + equity_drift + edge * f[t-1] + equity noise

    So a high, persistent ``f`` makes the trailing **HYG-minus-IEF trend** positive AND
    (with ``edge > 0``) raises the SPY *forward* excess over IEF — exactly the risk-on
    timing signal under test. ``edge = 0`` is the null: the credit trend still varies but
    predicts nothing about forward equity, so the timer must find no edge.
    ...
    f_lag = np.concatenate([[0.0], f[:-1]])   # forward relation: f[t-1] -> spy[t]
    spy_r = ief_r + equity_drift + edge * f_lag + rng.normal(0.0, equity_vol, n_days)
