"""Data layer for Study 988 — The Taming.

Bitcoin trades every day, so its year has roughly 365 observations rather than 252.
Every annualisation in this study uses the asset's own calendar — mixing the two is a 20% error
in reported volatility, which is larger than the trend being measured.
...
# Bitcoin, its usual comparisons, and the equity market as a yardstick for what
# "maturing" would even look like.
CRYPTO = "BTC-USD"
CRYPTO_2 = "ETH-USD"
EQUITY = "SPY"
NASDAQ = "QQQ"
GOLD = "GLD"
SINGLE_STOCK = "TSLA"     # a famously volatile equity, for scale
CASH = "BIL"
TICKERS = (CRYPTO, CRYPTO_2, EQUITY, NASDAQ, GOLD, SINGLE_STOCK, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call taming.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 988) -> tuple[pd.DataFrame, dict]:
