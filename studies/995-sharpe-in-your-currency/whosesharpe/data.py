"""Data layer for Study 995 — Whose Sharpe Is It?.

The FX legs are currency ETFs (FXE, FXB, …), each of which holds a deposit in the
foreign currency. Their total return is therefore the currency's move against the dollar **plus
the foreign deposit rate**, which is exactly what a foreign investor's cash leg earns — a
convenient accident that makes the arithmetic in ``strategy`` honest rather than approximate.
Section 1 of the results checks that property rather than assuming it.
...
# One asset, four viewpoints. FX ETFs stand in for the exchange rates because they
# are total-return instruments with the same calendar as the equity tape.
ASSET = "SPY"
ALT_ASSETS = ("QQQ", "TLT", "GLD", "EFA")
# Each of these is "one unit of foreign currency, priced in dollars", so its return IS the
# currency's appreciation against the dollar.
FX = {"EUR": "FXE", "GBP": "FXB", "JPY": "FXY", "CHF": "FXF", "AUD": "FXA", "CAD": "FXC"}
# A currency-hedged twin, to price the hedge directly rather than assuming its cost.
HEDGED = "HEDJ"
CASH = "BIL"
TICKERS = (ASSET,) + ALT_ASSETS + tuple(FX.values()) + (HEDGED, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call whosesharpe.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 995) -> tuple[pd.DataFrame, dict]:
