"""Reproducible headline run for Study 464 — Pennant.
...
from pennant import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch pennant-vs-random test
    HAVE_SCIPY = True
...
print("# Pennant — mechanical pole+triangle breakout (pole-direction) on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled pennant breakout (pole-direction) vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'penn_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            ...
            rs = st.random_entries(b["close"], sg, seed=7)
            tt.append(st.forward_returns(b["close"], sg, h))
            rr.append(st.forward_returns(b["close"], rs, h))
            ne.append(st.forward_returns(b["close"], sg, h, cost_bps=1.0))
            if h == st.HORIZONS[0]:
                total += len(sg)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total pennant breakouts (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: pennant one-sample t and pennant-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        sg = st.detect_pennants(b)
        rs = st.random_entries(b["close"], sg, seed=7)
        s = st.summarize(st.forward_returns(b["close"], sg, 20))
        sr = st.summarize(st.forward_returns(b["close"], rs, 20))
        nlong = int((sg["dir"] > 0).sum()) if len(sg) else 0
        print(f"  {t}: ent={len(sg):>3} (long {nlong}) penn={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Direction-scramble placebo (SPY, H=20, 500 draws)")
    b = _load("SPY")
    pl = st.direction_placebo(b["close"], st.detect_pennants(b), 20, n_draws=500, seed=464)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => trading-in-pole-direction not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    ...
    sg = st.detect_pennants(px)
    s = st.summarize(st.forward_returns(px["close"], sg, 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  penn={s['mean_bps']:+7.1f}bps  "
