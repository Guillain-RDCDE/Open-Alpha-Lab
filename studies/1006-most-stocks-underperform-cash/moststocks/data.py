"""Data layer for Study 1006 — Most Stocks Lose.

Fifty large-capitalisation US names, the cap- and equal-weighted index products
they aggregate towards, and short Treasuries as the cash benchmark.

**These fifty all survived**, which is the exact opposite of the selection that produces
Bessembinder's result — his universe includes every company that ever listed, most of which
delisted. So this basket is the *hardest possible* place to find the effect: if the median
survivor still lags bills over long horizons, the finding is not an artefact of dead companies.
The study measures the direction and size of that bias explicitly rather than claiming to
replicate a number it cannot.
...
CASH = "BIL"
BILLS = "SHY"
# A cross-section wide enough for a distribution, across sectors and outcomes.
NAMES = ("AAPL", "MSFT", "JNJ", "XOM", "JPM", "PG", "KO", "PFE", "WMT", "CSCO",
         "INTC", "MRK", "T", "VZ", "HD", "MCD", "BA", "CAT", "MMM", "IBM",
         "GE", "F", "DIS", "NKE", "ORCL", "QCOM", "TXN", "AMGN", "GILD", "LOW",
         "UNH", "CVX", "COST", "SBUX", "ADBE", "AMD", "MU", "SCHW", "USB", "AXP",
         "BAC", "C", "WFC", "GS", "MS", "HON", "UPS", "LMT", "DE", "EMR")
TICKERS = (MARKET, EQUAL_WEIGHT, CASH, BILLS) + NAMES
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call moststocks.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1006) -> tuple[pd.DataFrame, dict]:
