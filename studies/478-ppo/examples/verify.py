"""Reproducible headline run for Study 478 — Percentage Price Oscillator (PPO).
...
from ppo import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch crossover-vs-random test
    HAVE_SCIPY = True
...
print("# PPO — mechanical bullish-crossover on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled PPO crossover vs drift-matched random baseline (+ raw-MACD comparator)")
    print(f"  {'H':>3} {'n':>5} {'cross_bps':>10} {'win%':>5} {'1samp_t':>8} "
          f"{'rnd_bps':>8} {'delta':>7} {'net_bps':>8} {'welch_t':>8} {'p':>6} "
          f"{'macd_bps':>9} {'macd_d':>7}")
    total = 0
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            ...
            mm.append(st.forward_returns(c, me, h))
            if h == st.HORIZONS[0]:
                total += len(e)
        tt = np.concatenate(tt); rr = np.concatenate(rr)
        ne = np.concatenate(ne); mm = np.concatenate(mm)
        s = st.summarize(tt); sr = st.summarize(rr); sn = st.summarize(ne); sm = st.summarize(mm)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f} {sm['mean_bps']:>9.1f} {sm['mean_bps']-sr['mean_bps']:>7.1f}")
    print(f"  total PPO crossovers (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: PPO one-sample t and crossover-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.ppo_cross_entries(c)
        re = st.random_entries(c, max(len(e), 50), seed=7)
    ...
    print("\n# Shuffled-sign crossover placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_sign_placebo(_load("SPY")["close"], 20, n_draws=500, seed=478)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => crossover structure not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.ppo_cross_entries(c), 20))
