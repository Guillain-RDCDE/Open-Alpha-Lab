"""Data layer for Study 987 — Gold's Loud Cousin.

SLV and GLD both launched in the mid-2000s, so the common window is about twenty
years — long enough to contain two full metals cycles, which is more than most commodity studies
get, and still short enough that the beta estimates below carry real uncertainty.
...
# The two metals, the miners that lever them again, the dollar and real rates
# (gold's two textbook drivers), and industrial demand — silver's alleged extra ingredient.
GOLD = "GLD"
SILVER = "SLV"
GOLD_MINERS = "GDX"
SILVER_MINERS = "SIL"
DOLLAR = "UUP"
TIPS = "TIP"            # a real-rate proxy
INDUSTRIAL = "XLI"      # industrial activity, silver's supposed second driver
COPPER = "CPER"
CASH = "BIL"
TICKERS = (GOLD, SILVER, GOLD_MINERS, SILVER_MINERS, DOLLAR, TIPS, INDUSTRIAL, COPPER, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call loudcousin.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 987) -> tuple[pd.DataFrame, dict]:
