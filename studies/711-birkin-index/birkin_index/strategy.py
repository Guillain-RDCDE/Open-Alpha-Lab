"""Strategy + inference for Study 711 — "a Birkin beats the S&P (and even gold)".

The claim (Baghunter 2016, recycled by luxury-investment media): buying an Hermès Birkin
is a real *investment* that beats the S&P and gold, appreciating ~14%/yr with no down
years. We test the strongest tradable version of that:

1. **The resale index itself** (hardcoded, cited, approximate) vs SPY *and* GLD on the same
   window — CAGR, vol, max drawdown, and a *t*-stat of the annual excess return. This is
   the "did the bag actually beat stocks/gold?" question, on the level believers quote.
2. **The tradable equity proxies** (Hermès RMS.PA, LVMH MC.PA, Kering KER.PA) vs SPY — the
   only thing a public investor can actually buy that is *about* the luxury-bag trade.
   Monthly returns, Newey-West *t* of the alpha vs SPY, Sharpe, max drawdown.
3. **The consignment/illiquidity haircut** — what a real flip costs: the ~30% dealer /
   consignment spread (buy at boutique-plus or grey-market premium, sell at a consignment
   discount), authentication, and the months a bag sits unsold. Charged once on NAV.
...
def annual_excess_t(index_level: pd.Series, bench_level: pd.Series) -> dict:
    ...
    paired excess (index - bench) against 0. Small-sample (≈10 years), so this is a
...
# --------------------------------------------------------------------------- #
# The consignment / illiquidity haircut — what a Birkin flip actually costs
# --------------------------------------------------------------------------- #
def net_of_carry_cagr(gross_cagr: float, round_trip_spread: float = 0.30,
                      hold_years: float = 3.0, insure_per_year: float = 0.005) -> dict:
    """Charge the Birkin flip's real frictions once on the NAV.

    A buy-at-a-premium / sell-through-consignment round trip costs ``round_trip_spread``
    (a specialist reseller / auction house takes ~25–33% consignment + the buy-side
    grey-market premium is already folded in), amortised over a typical ``hold_years``;
    plus ``insure_per_year`` annual carry (insurance/storage/authentication). Returns the
