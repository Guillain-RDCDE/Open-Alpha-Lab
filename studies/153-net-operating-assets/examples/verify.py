"""Real-tape verification — Study 153 (Net-Operating-Assets). Regenerates docs/results.md numbers.

Reads the EDGAR cache, computes the NOA signal, runs quintile sorts and the random-portfolio
control, and prints the headline statistics. No network access required — reads from the
shared ``_cache/`` parquet files.

    python studies/153-net-operating-assets/examples/verify.py
"""
...
def main() -> None:
    ...
    print(f"Fingerprint (NOA): {data.fingerprint(sig)}")
    print()

    h = st.quintile_returns(sig, fwd, q=0.20)

    print("=== quintile returns (NOA low->high, forward 1yr) ===")
    for q in ["q1", "q2", "q3", "q4", "q5", "market"]:
        s = st.summary(h[q])
        label = "low-NOA" if q == "q1" else ("high-NOA" if q == "q5" else
    ...
    print()
    print(f"=== long low-NOA / short high-NOA hedge ===")
    ...
    print(f"  Low-NOA (q1) excess: mean={lo_excess['mean']*100:+.1f}%/yr  "
          f"HAC t={lo_excess['tstat']:+.2f}")
    print(f"  High-NOA (q5) excess: mean={hi_excess['mean']*100:+.1f}%/yr  "
