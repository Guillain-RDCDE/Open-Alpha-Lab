...
from fifty_two_week_high import data  # noqa: E402
...
def test_synthetic_is_deterministic():
    a, _ = data.synthetic_panel(n_stocks=5, n_days=300, momentum=0.1, seed=42)
    b, _ = data.synthetic_panel(n_stocks=5, n_days=300, momentum=0.1, seed=42)
    assert np.allclose(a.to_numpy(), b.to_numpy())
    c, _ = data.synthetic_panel(n_stocks=5, n_days=300, momentum=0.1, seed=99)
...
def test_momentum_induces_positive_autocorrelation():
    """The momentum knob plants positive autocorrelation in returns (as intended)."""
    flat, _ = data.synthetic_panel(n_stocks=5, n_days=800, momentum=0.0, seed=236)
    mom, _ = data.synthetic_panel(n_stocks=5, n_days=800, momentum=0.30, seed=236)
    ...
    ac_mom = mean_lag1_ac(mom)
    # Flat should have near-zero AC; momentum should have positive AC
...
def test_fingerprint_is_stable_and_content_sensitive(coin_panel):
    ...
    other, _ = data.synthetic_panel(n_stocks=10, n_days=600, momentum=0.0, seed=99)
