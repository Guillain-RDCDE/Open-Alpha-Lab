"""Data layer for Study 1003 — The 1% Allocation.

Bitcoin's daily history against a conventional 60/40 built from SPY and AGG, with
gold and long bonds as alternative diversifiers.

The panel returned here is **raw**: the union of every ticker's calendar, so bitcoin has rows on
weekends where the equity funds are NaN. That is the faithful record and the data layer does not
editorialise. The calendar decision — bitcoin trades 365 days a year and the rest of a portfolio
does not — is an analytical choice, so it lives visibly in
``strategy.align_to_equity_calendar`` rather than being applied silently on load. It matters:
annualising bitcoin over 365 observations while everything else uses 252 inflates every
risk-adjusted comparison in bitcoin's favour, and the study measures exactly how much.
...
GOLD = "GLD"
CASH = "BIL"
TICKERS = (BTC, EQUITY, BONDS, LONG_BONDS, GOLD, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call onepercent.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1003) -> tuple[pd.DataFrame, dict]:
