"""Diamond Top as a falsifiable mechanical rule — Study 466.

A **diamond top** is a classic reversal chart pattern: after an advance, the swing range
first **broadens** (a megaphone — successively higher highs and lower lows) and then
**narrows** (a symmetrical triangle — lower highs and higher lows), tracing a diamond. The
folklore (every chart-pattern site, Bulkowski, StockCharts) says the diamond **calls the
turn**: when price finally breaks **down** out of the narrowing apex, it starts a real
decline, so you **short the breakdown**.
...
2. **Diamond geometry** — over a window of the most-recent confirmed alternating pivots we
   ...
   (amplitudes decreasing), i.e. the range expands then contracts — a diamond.
3. **Downside break** — once a diamond is confirmed, a short fires on the first close
   **below** the lower edge of the narrowing apex (the breakdown). Entry is at the **next**
   close (one documented lag); we then measure the forward H-day return of a *short* (we
   short, so the trade return is the **negative** of the price move).
4. **Controls.** (a) a **random-entry** baseline (same instrument, same epoch, same hold,
   same short sign) that captures the tape's drift, and (b) a **shuffled-pivot placebo** that
   rebuilds diamonds from a permutation of the pivot *prices*, destroying the broaden-then-
   narrow geometry while keeping the marginal distribution — the honest "is the diamond
   shape doing anything?" null.

No look-ahead: pivots carry a ``k``-bar confirmation lag, the breakdown is read on the close
...
def diamond_breakdowns(close: pd.Series, k: int = 5, n_piv: int = 6,
                        tol: float = 0.08) -> pd.DatetimeIndex:
    """Bars whose close breaks **down** out of a confirmed diamond top.
    ...
    pivot sits in a rising context (a *top* — the diamond formed after an advance), we arm a
    short. The short fires on the first close **below** the lowest of the narrowing-apex
    pivots (the downside break). Only the first bar of each consecutive run is kept.

    Entry is executed at the next close by :func:`forward_returns` (with ``short=True``).
    """
    piv = _alternating(find_pivots(close, k=k))
    n = len(close)
    cl = close.to_numpy(dtype=float)
    idx = close.index
    armed_floor = np.full(n, np.nan)   # the breakdown level active at each bar
    if len(piv) >= n_piv:
        ...
        for t in range(n):
            ...
            if not is_diamond(seg_prices, tol=tol):
                continue
            # a *top*: the diamond should follow an advance -> the start pivot is below the peak
            peak_high = seg_prices.max()
            if seg_prices[0] >= peak_high:      # already at the high -> not a clean top context
                continue
            # breakdown level = lowest pivot of the narrowing apex (last 3 pivots)
            armed_floor[t] = float(seg_prices[-3:].min())
    floor = pd.Series(armed_floor, index=idx)
    mask = (close < floor) & floor.notna()
...
def forward_returns(close: pd.Series, entries, horizon: int, cost_bps: float = 0.0,
    ...
    The diamond-top rule is a **short** (``short=True``): the trade return is the *negative*
    of the price move, so a post-breakdown decline is a profit. ``cost_bps`` is a one-way cost
...
def shuffled_pivot_placebo(close: pd.Series, horizon: int, k: int = 5, n_piv: int = 6,
                           tol: float = 0.08, n_draws: int = 1000, seed: int = 466) -> dict:
    ...
    share of placebo runs whose mean breakdown forward (short) return **beats** the real one —
    the honest "is the diamond shape adding anything?" p-value, plus the observed mean.
    """
    obs = float(np.mean(forward_returns(close, diamond_breakdowns(close, k=k, n_piv=n_piv,
                                                                  tol=tol), horizon)))
    piv = _alternating(find_pivots(close, k=k))
    if len(piv) < n_piv:
    ...
    for _ in range(n_draws):
        ...
        floor = pd.Series(armed, index=idx)
        mask = (close < floor) & floor.notna()
        first = mask & ~mask.shift(1, fill_value=False)
...
def run_experiment(close: pd.Series, k: int = 5, n_piv: int = 6, tol: float = 0.08,
    ...
    """Run the full gauntlet on one tape: breakdown short vs random-entry baseline, all H.

    Returns a dict keyed by horizon with the breakdown summary (gross + net), the
    drift-matched random-entry baseline (same short sign), and the breakdown-minus-random
    delta.
    """
    ent = diamond_breakdowns(close, k=k, n_piv=n_piv, tol=tol)
