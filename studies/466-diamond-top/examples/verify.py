"""Reproducible headline run for Study 466 — Diamond Top.
...
from diamond_top import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch breakdown-vs-random test
    HAVE_SCIPY = True
...
print("# Diamond top — mechanical downside-break short on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled diamond breakdown (SHORT) vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'brk_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.diamond_breakdowns(c, k=K, n_piv=N_PIV, tol=TOL)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total diamond breakdowns (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: breakdown one-sample t and breakdown-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.diamond_breakdowns(c, k=K, n_piv=N_PIV, tol=TOL)
        re = st.random_entries(c, max(len(e), 50), k=K, seed=7)
    ...
    pl = st.shuffled_pivot_placebo(_load("SPY")["close"], 20, k=K, n_piv=N_PIV, tol=TOL,
                                   n_draws=500, seed=466)
...
for edge in (0.0, 0.60):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.diamond_breakdowns(c, k=K, n_piv=N_PIV, tol=TOL), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  brk={s['mean_bps']:+7.1f}bps  "
