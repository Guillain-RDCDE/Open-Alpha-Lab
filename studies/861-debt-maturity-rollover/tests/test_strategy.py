...
the claim long-short (long low-share / short high-share) lights up *only when a rollover penalty
is planted* — and with the right sign."""
...
from debt_maturity import strategy as st  # noqa: E402
...
def test_calendar_ls_columns(null_panel):
    ...
    ls = st.calendar_ls(prices, ev, staleness_days=120, long_high=False)
...
def test_calendar_ls_null_is_quiet(null_panel):
    ...
    assert abs(stt["t_nw"]) < 3.0            # no planted penalty -> no strong HAC t


def test_calendar_ls_recovers_planted_penalty(planted_panel):
    prices, ev = planted_panel
    ls = st.calendar_ls(prices, ev, staleness_days=120, long_high=False)
    stt = st.calendar_ls_stats(ls)
    assert stt["mean_bps"] > 0               # long low-share beats short high-share
    assert stt["t_nw"] > 2.0                 # and it clears the bar (claim confirmed)


def test_long_high_flips_sign(planted_panel):
    prices, ev = planted_panel
    ls_lo = st.calendar_ls(prices, ev, staleness_days=120, long_high=False)
    ls_hi = st.calendar_ls(prices, ev, staleness_days=120, long_high=True)
    # long-short of long-top vs long-bottom is the exact negative of the reverse
    assert np.allclose(ls_lo["ls"].to_numpy(), -ls_hi["ls"].to_numpy())
...
# era split
# --------------------------------------------------------------------------- #
def test_era_split_partitions(planted_panel):
    prices, ev = planted_panel
    ls = st.calendar_ls(prices, ev, staleness_days=120, long_high=False)
    e = st.era_split(ls, split="2015-01-01")
    assert e["early_n"] + e["late_n"] == len(ls)
    assert e["early_n"] > 0 and e["late_n"] > 0
...
def test_event_summary_null_placebo_not_extreme(null_panel):
    prices, ev = null_panel
    s = st.event_summary(prices, ev, horizon=63, n_draws=1000, long_high=False)
...
def test_placebo_reproducible(planted_panel):
    ...
    a = st.placebo_pvalue(fr, n_draws=500, seed=7, long_high=False)["draws"]
    b = st.placebo_pvalue(fr, n_draws=500, seed=7, long_high=False)["draws"]
