"""Reproducible headline run for Study 817 — Realized-Volatility Trend.
...
print("# Realized-Volatility Trend — do rising-vol names keep de-rating?")
...
sp = st.vol_spreads(ret, short_w=21, long_w=63, frac=0.3)
h = st.vol_stats(sp)
print(f"\nsort: vol trend (21d/63d - 1), long bottom30% (falling) / short top30% (rising), "
      f"{h['n_days']} days (median {int(sp['n'].median())} names/day)")
print("# THE HEADLINE — long-falling-vol / short-rising-vol spread")
print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
      f"one-sample t = {h['t_1s']:+.2f}")
print(f"  books : falling-vol {h['lo_bps']:+.2f} vs rising-vol {h['hi_bps']:+.2f} bps  "
...
ad = st.additivity(ret, short_w=21, long_w=63, frac=0.3)
print(f"  low-vol LEVEL spread: {ad['level_bps']:+.2f} bps/day (NW t = {ad['level_t_nw']:+.2f})")
print(f"  corr(trend, level) = {ad['corr']:+.3f}  beta = {ad['beta']:+.3f}")
print(f"  trend alpha vs level: {ad['alpha_bps']:+.2f} bps/day (NW t = {ad['alpha_t_nw']:+.2f})")
...
print("\n# THE TIMER — long-falling-vol / short-rising-vol, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=817 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0015, seed=817, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0015, seed 817): spread NW t = {sy['t_nw']:+.2f}, "
