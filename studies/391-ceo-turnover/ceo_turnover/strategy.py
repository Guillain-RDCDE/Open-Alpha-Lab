"""Strategy + inference for Study 391 — CEO-Turnover (event-study abnormal returns).

The claim: *firing the CEO* moves the stock — and the believers can't agree which way
("the underperformer is gone, rally!" vs "leadership chaos, sell!"). We pin it down with
a textbook short-window **event study**:

    For each dated CEO-change announcement, fit a **market model** (stock = α + β·SPY)
    on a clean PRE-event estimation window [−est−gap, −gap], then compute the
    **abnormal return** AR_t = r_stock,t − (α + β·r_SPY,t) on the event window and
    cumulate it into a **CAR** (cumulative abnormal return).

We then ask, with a 1-day execution lag for any tradable variant:

  * Is the **forced** CAR different from zero? the **planned** CAR? (Welch / t vs 0)
  * Is **forced − planned** different from zero? (the believers' core claim)
  ...
  * **one-way costs** applied to a "buy on the announcement, hold the window" variant.

The decisive number is not the sign of any point estimate (the buckets disagree, as the
folklore does) but the **sample size**: with ~a dozen events per bucket the
forced-minus-planned CAR is statistically indistinguishable from noise — an event-study
power problem, not a directional discovery.
"""
...
# --------------------------------------------------------------------------- #
def _log_simple_returns(prices: pd.Series) -> pd.Series:
...
def event_car(prices: pd.DataFrame, ticker: str, announce_date: pd.Timestamp,
              ...
              lag: int = 0) -> float:
    """Market-model cumulative abnormal return (CAR) for one CEO-change event.
    ...
    if ticker not in prices.columns or "SPY" not in prices.columns:
        return float("nan")
    rs = _log_simple_returns(prices[ticker]).dropna()
    rm = _log_simple_returns(prices["SPY"]).dropna()
...
def car_panel(prices: pd.DataFrame, events: list[dict],
    ...
    """CAR for every event in the table; returns a tidy frame with ``forced`` flag."""
    rows = []
    for e in events:
...
def placebo_car_dist(prices: pd.DataFrame, tickers: list[str], k: int,
                     window: tuple[int, int] = DEFAULT_WINDOW,
                     n_draws: int = 20_000, seed: int = 391) -> np.ndarray:
    ...
    # pre-compute simple returns aligned to SPY
    rm = _log_simple_returns(prices["SPY"]).dropna()
    rets = {}
    for t in avail:
        rs = _log_simple_returns(prices[t]).dropna()
        common = rs.index.intersection(rm.index)
...
def summarize(panel: pd.DataFrame, prices: pd.DataFrame | None = None,
    ...
    """Headline event-study stats: forced / planned / all buckets, the forced−planned
    difference (Welch t), and a placebo p-value for the forced bucket if prices given."""
    forced = panel.loc[panel["forced"], "car"].to_numpy(float)
    planned = panel.loc[~panel["forced"], "car"].to_numpy(float)
    allc = panel["car"].to_numpy(float)
    out = {
        "forced": summarize_bucket(forced),
        "planned": summarize_bucket(planned),
        "all": summarize_bucket(allc),
        "diff_pct": float(np.nanmean(forced) - np.nanmean(planned)) * 100
        if len(forced) and len(planned) else float("nan"),
        "diff_t": welch_t(forced, planned),
    }
    if prices is not None and tickers is not None and len(forced):
        null = placebo_car_dist(prices, tickers, k=len(forced), window=window,
                                n_draws=n_draws)
        out["forced_placebo_p"] = placebo_pvalue(float(np.nanmean(forced)), null)
        out["null_mean_pct"] = float(np.nanmean(null) * 100)
    return out


def net_of_costs(car_mean: float, cost_bps: float = 10.0) -> dict:
    """One-way round-trip cost on a 'buy on announcement, hold the window' trade.

    A short event-window trade is one entry + one exit; 10 bps round-trip covers an
    institutional equity desk. Returns gross/net mean CAR. (The point is the same as
    the rest of the study: cost is not the binding constraint — the event count is.)"""
    c = cost_bps / 1e4
