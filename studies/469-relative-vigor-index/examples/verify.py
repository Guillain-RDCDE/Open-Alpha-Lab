"""Reproducible headline run for Study 469 — Relative Vigor Index (RVI).
...
from relative_vigor_index import data, strategy as st  # noqa: E402
...
print("# Relative Vigor Index — mechanical RVI cross-up on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled RVI cross-up vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.cross_up_entries(b, period=P)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total RVI cross-ups (H={st.HORIZONS[0]}): {total}")
    ...
    pl = st.phase_scramble_placebo(_load("SPY"), 20, period=P, n_draws=500, seed=469)
...
print("  edge=0 must NOT reach significance; planted regime momentum must light up.")
for edge in (0.0, 0.60):
    px, _ = data.synthetic_panel(edge=edge, seed=469, n_days=4000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.cross_up_entries(px, period=P), 20))
