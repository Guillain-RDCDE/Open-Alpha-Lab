"""Data layer for Study 1005 — Beta Has a Half-Life.

Forty large-cap names, nine sector funds and the market proxy they are all
regressed against. Sector funds are included deliberately: if beta instability is mostly
estimation noise then it should shrink sharply for a diversified portfolio, and if it is genuine
time variation in exposure it should not. Having both in one panel is what separates the two.
...
# Sector funds: a portfolio's beta should be far steadier than a single name's.
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call betahalflife.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1005) -> tuple[pd.DataFrame, dict]:
