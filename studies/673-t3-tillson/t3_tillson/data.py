"""Data layer for Study 673 — T3 (Tillson).
...
* **Real tape.** Daily total-return bars (`auto_adjust=True`) for a five-ticker basket
  (SPY, QQQ, AAPL, MSFT, XLE — the same liquid basket as sibling studies 432/434/672) via
  ``yfinance`` (no key), **cache-first**: a cached parquet under the study's own
  ``_cache/`` is read offline; the network is touched only on a cache miss or an explicit
  ``fetch=True``.
...
* **Synthetic world.** A deterministic, seeded tape with a TUNABLE planted trend
  ...
  coin for any moving-average timing rule; ``edge > 0`` plants a slow, persistent trend a
  trend-follower can ride. This is the study's faithful-engine / power check: the harness
  must NOT manufacture an edge from the null and must recover a real one when planted.

Tim Tillson's "T3" ("Better Moving Averages", *Technical Analysis of Stocks & Commodities*,
Jan 1998) stacks six exponential moving averages of the same nominal length and recombines
them with a "volume factor" v (Tillson's own term for the smoothing-vs-lag knob, nothing
to do with traded volume)::

    GD(x, v) = EMA(x)*(1+v) - EMA(EMA(x))*v          # "generalized DEMA"
    T3 = GD(GD(GD(price)))                            # GD nested three times = 6 EMAs

The pitch, repeated on every charting site since: because T3 is a *generalized* DEMA/TEMA
that adds curve-fitting smoothness (the v knob) on top of DEMA's lag reduction, it "hugs
price" with less lag than a plain EMA of the same length AND is smoother, so its
crossovers are both earlier and cleaner ("virtually eliminates lag while smoothing").
...
def _cache_path(ticker: str, cache_dir: str = DEFAULT_CACHE) -> str:
    ...
    return os.path.join(cache_dir, f"t3_{safe}_1d.parquet")
...
def synthetic_panel(
    ...
    The only structure any moving-average trend rule (T3 included) can possibly harvest
    is *persistent* drift. The drift sign flips every ``regime_len`` bars; its magnitude
    is ``edge * daily_vol``.
