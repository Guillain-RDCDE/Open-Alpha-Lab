"""Reproducible headline run for Study 486 — Gann Hi-Lo Activator.
...
from gann_hilo_activator import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch flip-vs-random test
    HAVE_SCIPY = True
...
print("# Gann Hi-Lo Activator — mechanical flip-up entry on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled flip-up vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'flip_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.flip_up_entries(b, period=P)
            re = st.random_entries(b, max(len(e), 50), period=P, seed=7)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total flip-up entries (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: flip one-sample t and flip-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        c = b["close"]
        e = st.flip_up_entries(b, period=P)
        ...
        print(f"  {t}: ent={len(e):>3} flip={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Shuffled-flip TIMING placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_flip_placebo(_load("SPY"), 20, period=P, n_draws=500, seed=486)
...
for edge in (0.0, 0.60):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.flip_up_entries(px, period=P), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  flip={s['mean_bps']:+7.1f}bps  "
