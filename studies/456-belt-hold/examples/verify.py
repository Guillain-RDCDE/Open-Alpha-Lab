"""Reproducible headline run for Study 456 — Belt-Hold (opening marubozu).
...
from belt_hold import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch belt-hold-vs-random test
    HAVE_SCIPY = True
...
print("# Belt-hold — mechanical bullish belt-hold on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled belt-hold vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'bh_bps':>8} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>8.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total belt-hold signals (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: belt-hold one-sample t and belt-hold-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        e = st.belt_hold_entries(b)
        re = st.random_entries(b, max(len(e), 50), seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} bh={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Shape-scramble GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shape_scramble_placebo(_load("SPY"), 20, n_draws=500, seed=456)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => candle shape not load-bearing)")
else:
...
print("  edge=0 must NOT reach significance; planted reversal must light up.")
for edge in (0.0, 0.40):
    px, _ = data.synthetic_panel(edge=edge, seed=456, n_days=4000)
    s = st.summarize(st.forward_returns(px, st.belt_hold_entries(px), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  bh={s['mean_bps']:+7.1f}bps  "
