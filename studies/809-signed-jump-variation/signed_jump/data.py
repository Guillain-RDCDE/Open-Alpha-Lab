"""Data layer for Study 809 — Signed Jump Variation.

The claim under test (Barndorff-Nielsen, Kinnebrock & Shephard 2010 on realized
**semivariance**; Bollerslev, Li & Zhao 2020, *"Good Volatility, Bad Volatility, and the
Cross Section of Stock Returns"*): split a name's recent realized variance into an
**upside** piece ``RS+ = Σ r² · 1(r>0)`` and a **downside** piece ``RS- = Σ r² · 1(r<0)``.
The **signed jump variation** ``SJ = (RS+ − RS-) / RV`` (``RV = RS+ + RS-``) is priced
**negatively**: names whose recent variance is **downside-dominated** (negative signed
jump — "bad" volatility) carry a **premium** and out-earn, while the upside-dominated
("good" volatility) names under-earn. A long **low-SJ** / short **high-SJ** book should
...
* **Synthetic world — the positive control.** A deterministic, seeded panel
  ...
  "asymmetry tilt" ``c_i`` that both (a) shapes whether its daily variance is upside- or
  downside-dominated (so a trailing signed-jump sort proxies ``c_i``) and (b) — only when
  ``edge > 0`` — depresses its forward mean return. ``edge = 0`` is the null world: the
  signed jump still varies across names but carries **no** information about forward
  returns, and the sort must find nothing. ``edge > 0`` plants the Bollerslev-Li-Zhao
  negative signed-jump → return relation (downside names out-earn).
...
# --------------------------------------------------------------------------- #
# Synthetic world — planted negative signed-jump->return relation (positive control)
# --------------------------------------------------------------------------- #
def synthetic_panel(
    edge: float = 0.0,
    seed: int = 809,
    ...
    """Deterministic seeded OHLC panel with a TUNABLE planted signed-jump->return relation.

    Each asset ``i`` carries a persistent latent "asymmetry tilt" ``c_i[t]`` — an AR(1)
    with autocorrelation ``factor_rho``. The tilt controls whether the daily variance is
    **upside**- or **downside**-dominated via a mean-zero quadratic-in-shock term (so a
    trailing signed-jump sort — ``(RS+ − RS-)/RV`` — proxies ``c_i``), and — only when
    ``edge > 0`` — depresses the **forward mean**:

        z ~ N(0,1)
        asym_shock = daily_vol * (z + asym_gain * c_i[t] * (z**2 - 1))
        r[i,t] = drift - edge * c_i[t] + asym_shock

    A high positive tilt makes the big moves land on the **upside** (RS+ > RS-, a positive
    signed jump — "good" volatility) *and* (with ``edge > 0``) lowers the forward mean — the
    Bollerslev-Li-Zhao pattern: high signed jump (upside-dominated), low forward return; the
    downside-dominated (negative-SJ, "bad" volatility) names carry the premium. The
    ``(z**2 - 1)`` term is mean-zero, so ``edge = 0`` is a clean null: the signed jump still
    varies across names but predicts nothing.
    ...
    for i in range(n_assets):
        ...
        asym = daily_vol * (z + asym_gain * c * (z ** 2 - 1.0))
        r = drift - edge * c + asym
