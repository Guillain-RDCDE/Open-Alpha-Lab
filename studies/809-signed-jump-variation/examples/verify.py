"""Reproducible headline run for Study 809 — Signed Jump Variation.
...
print("# Signed Jump Variation — do downside-dominated names carry a premium?")
...
for w in (21,):
    sp = st.signed_jump_spreads(ret, window=w, frac=0.3)
    h = st.signed_jump_stats(sp)
    print(f"\nsort: trailing-{w}d signed jump (RS+ - RS-)/RV, long bottom30% (low SJ) / "
          f"short top30% (high SJ), {h['n_days']} days (median {int(sp['n'].median())} names/day)")
    print("# THE HEADLINE — long-low-SJ / short-high-SJ spread")
    print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
          f"one-sample t = {h['t_1s']:+.2f}")
    print(f"  books : low-SJ {h['lo_bps']:+.2f} vs high-SJ {h['hi_bps']:+.2f} bps  "
          f"(Welch t = {h['welch_t']:+.2f})")
    sps = sp["spread"].to_numpy()
    print(f"  gross spread Sharpe (no cost): "
          f"{sps.mean() / sps.std(ddof=1) * np.sqrt(252):.2f}")

sp = st.signed_jump_spreads(ret, window=21, frac=0.3)
...
for lo, hi, lbl in [("2010-01-01", "2018-01-01", "2010-2017"),
    ...
    ts = st.signed_jump_stats(sub)
    print(f"  {lbl}: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-low-SJ / short-high-SJ, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=809 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0024, seed=809, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0024, seed 809): spread NW t = {sy['t_nw']:+.2f}, "
