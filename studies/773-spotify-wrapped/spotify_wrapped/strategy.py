"""Strategy + inference for Study 773 — Spotify-Wrapped.

The claim: **Spotify (SPOT) rallies into its December Wrapped launch.** Two windows, both
anchored on the real Wrapped launch date (calendar-known — Wrapped ships in the same
late-Nov/early-Dec slot every year):

* **The run-up (the "buy the buzz").** SPOT's abnormal return (SPOT total-return minus the
  SPY total-return benchmark) over the K trading sessions ENDING on the last close
  on/before the launch. Tested at K = 10 (~2 weeks) and K = 21 (~1 month).

* **The fade (the "sell the news").** SPOT's abnormal return over the K sessions STARTING
  from the launch-day close. Tested at K = 10 and K = 21. If the folklore is right the
  run-up is positive; if it's a "buy the buzz, sell the news" the fade is negative; if both
  are noise, the whole Wrapped-rally story is busted.

Because each Wrapped launch is a single independent event (not a daily series), the primary
statistic is a **one-sample t** of the abnormal return across events (n = the number of
years with SPOT+SPY coverage — only ~8, since SPOT lists in April 2018). A random-window
placebo (drawing many random, non-launch K-session windows from SPOT's own history vs SPY)
checks whether the observed mean sits inside or outside the stock's ordinary tracking noise.
"""
...
def build_event_table(prices: dict[str, pd.Series], cost_bps: float = COST_BPS
    ...
    """One row per Wrapped year: run-up + post-event abnormal returns + inclusion.

    A row is INCLUDED only if SPOT and SPY both have cached history covering
    [anchor - PRE_LONG_K .. anchor + POST_LONG_K] for that year. Years whose window
    falls outside coverage (e.g. 2016-2017, before the SPOT direct listing) are excluded
    with a reason, so the funnel is auditable.
    """
    spot = prices[dt.INSTRUMENT]
    ...
    for year, keydate in dt.EVENTS:
        row = dict(year=year, launch=keydate)
        anchor_ts = pd.Timestamp(keydate)
        on_or_before = common[common <= anchor_ts]
        if len(on_or_before) == 0:
            row.update(included=False, reason="no SPOT/SPY coverage at the anchor")
            rows.append(row)
            continue
        ...
        def ar(i_start, i_end):
            r_a = spot.loc[common[i_end]] / spot.loc[common[i_start]] - 1.0
...
def placebo_pvalue(events: pd.DataFrame, prices: dict[str, pd.Series], col: str,
                   ...
                   n_seeds: int = 20, n_draws_per_seed: int = 200, base_seed: int = 777,
                   tail: str = "right") -> dict:
    """For each INCLUDED event, redraw a random (non-launch) k-session window on SPOT vs
    ...
    for s in range(n_seeds):
        ...
        for _ in range(n_draws_per_seed):
            ...
            for _e in range(n_events):
                ...
                r_a = spot.loc[d_end] / spot.loc[d_start] - 1.0
...
def car_path(events: pd.DataFrame, prices: dict[str, pd.Series],
    ...
    """Mean cumulative abnormal return (SPOT - SPY) at each offset from -pre..+post
    relative to the launch, normalised so offset 0 = 0%, averaged across all INCLUDED
    events. Negative offsets are the run-up; positive is the post-event fade window.
    """
    spot = prices[dt.INSTRUMENT]
    spy = prices[dt.BENCHMARK]
    common = spot.index.intersection(spy.index).sort_values()
    ...
    for _, row in inc.iterrows():
        ...
        base_a, base_s = spot.loc[common[p]], spy.loc[common[p]]
        vals = []
        for o in offsets:
            d = common[p + o]
            r_a = spot.loc[d] / base_a - 1.0
...
def synthetic_detect(bump: float, seed: int, k: int = PRE_SHORT_K,
    ...
    pre-launch bump. ``side='pre'`` measures the run-up window [p-k..p); ``side='post'``
    measures [p..p+k)."""
