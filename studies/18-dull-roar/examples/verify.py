...
rebalanced monthly, decile books, {COST_BPS:.0f} bp slippage. **Window caveat:** the shared panel
starts in {lo.year}, so the sample *excludes 2008-09* — the very crash where defensive low-vol pays
best; a window chosen by data availability, not by us, but one that reads against the anomaly.
**Survivorship caveat:** current membership excludes delisted names. That cut is *structurally
hostile* to this anomaly: the high-vol blow-ups that should populate the short leg delisted out of
the sample, leaving only the wild names that won — the ranking is upper-bounded for them and
biased against low-minus-high.*

## The verdict, earned — Signal `{signal}` · Tradability `{trad}` · Free alpha? `BETA-TILT`

The most-cited anomaly in finance — and on this panel it doesn't just fade, it **inverts**. The
...
zero. Per leg, the story is starker: the calm decile's alpha is **{legs['low_leg_alpha_pct']:+.1f}%/yr**
(*t* {legs['low_leg_alpha_t']:+.1f}) while the *wild* decile's is **{legs['high_leg_alpha_pct']:+.1f}%/yr**
(*t* {legs['high_leg_alpha_t']:+.1f}) — the low-minus-high alpha spread is
**{legs['long_short_alpha_pct']:+.1f}%/yr**. On this 2010-2026 survivor large-cap panel **the wild
stocks won**; the textbook trade (long calm, short wild) would have been shorting the decade's
winners. Only *after* that headline does the borrow point land: even in a window where the spread
had worked, the short leg is the expensive-to-borrow lottery corner, and here the dollar-neutral
book crosses zero at a borrow fee of **{be['breakeven_bps']:.0f} bps/yr** — under water before the
borrow even starts. What's left without shorting is a **long-only low-vol tilt** that is mostly
*lower beta* (beta **{tilt['low_beta']:.2f}**, alpha *t* = {tilt['alpha_t']:+.1f}). Real in the
long-run literature, inverted on the sample a practitioner faces today — a defensive posture, not
free alpha.

*(Why `WEAK` not `NONE`: the long-history academic effect and our synthetic control are unambiguously
real; this modern window — survivorship-biased against the short leg and excluding the crash years
where low-vol shines — is where it fails to show, and where its sign even flips. A finding about the
anomaly's fragility, on a sample built to be hostile to it; not a clean refutation.)*
...
- **Beta-neutral BAB**: alpha **{bab['alpha_ann_pct']:+.1f}%/yr**, HAC *t* = **{bab['alpha_t']:+.1f}**,
  ...
  {bab['beta_low_leg']:.2f} / {bab['beta_high_leg']:.2f} — full-sample, in-sample betas: a
  construction check on the anomaly's clean form, not a tradable book).
...
- **Where the effect lives — and which way it points**: low (calm) leg alpha
  **{legs['low_leg_alpha_pct']:+.1f}%** (*t* {legs['low_leg_alpha_t']:+.1f}); high (wild) leg alpha
  **{legs['high_leg_alpha_pct']:+.1f}%** (*t* {legs['high_leg_alpha_t']:+.1f}); low-minus-high spread
  **{legs['long_short_alpha_pct']:+.1f}%/yr**. The action is in the high-vol leg
  ({legs['share_in_short_leg']:.0%} of the combined magnitude) but with the **wrong sign for the
  anomaly**: the wild decile carried *positive* alpha, so the textbook short would have paid that
  alpha away before a single bp of borrow. The anomaly doesn't hide in the unshortable leg here —
  it inverts there.
...
under water before the borrow even starts, because on this panel the spread itself ran the wrong way
(the wild leg's alpha was positive). The borrow argument is the *second* wall, not the first: lottery-
stock borrows routinely run into the hundreds of bps, so even in a window where the spread *had*
worked, this is the friction that would have eaten it. What a no-shorting investor is left with is the
**long-only low-vol book**: a small, statistically thin defensive tilt (CAPM alpha
**{tilt['alpha_ann_pct']:+.1f}%/yr**, *t* = {tilt['alpha_t']:+.1f}; levered to beta 1,
**{tilt['excess_cagr_at_beta1_pct']:+.1f}%/yr** excess CAGR) — bounded, beta-driven, and nothing like
