...
def build_curious():
    cells = [
        ...
        md(
            ...
            "and per-leg the effect actually ran *backwards*: the wild stocks won. |\n"
            "| Is the clean version tradable? | 🚫 **No.** On this sample the trade was wrong on "
            "*direction* (shorting the decade's winners) — and even when the spread works, the proper, "
            "beta-neutral form needs you to **short** the wild stocks, the very names that are "
        ...
        md(
            ...
            f"- And here's the part to say plainly: the anomaly **inverted**. The *wild* decile's alpha "
            f"was **+{R['leg_high'].lstrip('+')}%/yr** (t {R['leg_high_t']}) against the calm decile's "
            f"**{R['leg_low']}%** (t {R['leg_low_t']}) — a calm-minus-wild spread of "
            f"**{R['leg_spread']}%/yr**. On this survivor large-cap panel the exciting stocks *won*; "
            "the textbook trade would have spent the decade short the winners.\n\n"
            "The celebrated anomaly didn't just fade on the modern large-cap tape — it pointed the "
            "other way."
        ),
        ...
        md(
            ...
            f"- **But on the real S&P 500 it's absent — and per leg, inverted** — flat SML, a "
            f"{R['bab_alpha']}%/yr beta-neutral alpha (t = {R['bab_t']}), and a calm-minus-wild alpha "
            f"spread of **{R['leg_spread']}%/yr** (the wild stocks won). Real in the long-run "
            "literature, pointing the other way in the sample you'd actually trade.\n"
        ...
        md(
            ...
            f"- **The trade ran backwards.** On this tape the wild leg's alpha was *positive* "
            f"(+{R['leg_high'].lstrip('+')}%/yr, t {R['leg_high_t']}), so the textbook short paid that "
            "alpha away — the dollar-neutral book is under water before the borrow even costs anything "
            f"(break-even **{R['breakeven']} bps**). And even in a window where the spread *had* "
            "worked, the names you'd short — small, lottery-like, heavily shorted — are exactly the "
            "expensive-or-impossible-to-borrow ones.\n"
...
def build_quants():
    cells = [
        ...
        md(
            ...
            "| **Tradability** | 🔴 `MIRAGE` | On this panel the anomaly **inverts**: the wild leg's "
            f"alpha is **+{R['leg_high'].lstrip('+')}%/yr** (*t* {R['leg_high_t']}) vs the calm leg's "
            f"**{R['leg_low']}%** (*t* {R['leg_low_t']}) — low-minus-high **{R['leg_spread']}%/yr**. "
            "The textbook short was short the winners; the borrow wall (break-even "
            f"**{R['breakeven']} bps/yr**) is only the second killer. |\n"
            ...
            "and on our control, but on the modern investable cross-section the wild stocks won "
            f"(low-minus-high alpha {R['leg_spread']}%/yr) — what remains is a low-beta defensive tilt, "
            "and a textbook long-short that fails on direction before the unaffordable short-leg "
            "borrow even bites.\n\n"
        ...
        md(
            "### 4d · Leg attribution — where does the alpha live, and which way does it point?\n"
            ...
            "Mind the *signs*, not just the shares: a magnitude split says nothing about direction, and "
            "on the real tape the wild leg's alpha comes out **positive** — an inversion, not a "
            "concentration."
        ),
        code(
        ...
        md(
            ...
            f"- **Real tape disagrees — and inverts** (`../docs/results.md`): SML slope {R['sml_slope']}, "
            f"beta-neutral alpha {R['bab_alpha']}%/yr (*t* = {R['bab_t']}), the calm book *trailing* the "
            f"market — and a wild leg whose own alpha was **+{R['leg_high'].lstrip('+')}%/yr** "
            f"(*t* {R['leg_high_t']}) against the calm leg's {R['leg_low']}% (*t* {R['leg_low_t']}): "
            f"low-minus-high **{R['leg_spread']}%/yr**. The wild stocks won.\n"
        ...
        md(
            ...
            f"- **Direction first, borrow second.** On the real tape the high-vol leg's alpha was "
            f"*positive* (+{R['leg_high'].lstrip('+')}%/yr, *t* {R['leg_high_t']}) — the short leg "
            "didn't merely cost borrow, it shorted the winners. The dollar-neutral book's break-even "
            f"borrow is **{R['breakeven']} bps/yr** (under water at a free short), and the names it "
            "must short — small, lottery-like, heavily-shorted — are the expensive-or-impossible-to-"
            "borrow ones in any window.\n"
