"""Reproducible headline run for Study 819 — Abnormal-Volume Shock.
...
print("# Abnormal-Volume Shock — do abnormally heavy names go on to drift up?")
...
vol = st.volume_panel(panel)
LOOKBACK, FORM, FRAC = 60, 5, 0.3
sp = st.avol_spreads(ret, vol, lookback=LOOKBACK, form=FORM, frac=FRAC)
h = st.avol_stats(sp)
print(f"\nsort: trailing-{FORM}d mean of ({LOOKBACK}d) standardised abnormal volume, "
      f"long top30% / short bottom30%, {h['n_days']} days "
      f"(median {int(sp['n'].median())} names/day)")
print("# THE HEADLINE — long-high-avol / short-low-avol spread")
print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
      f"one-sample t = {h['t_1s']:+.2f}")
print(f"  books : high-avol {h['hi_bps']:+.2f} vs low-avol {h['lo_bps']:+.2f} bps  "
...
pl = st.placebo_pvalue(ret, vol, lookback=LOOKBACK, form=FORM, frac=FRAC,
                       n_seeds=20, n_draws_per_seed=50)
...
for lo, hi, lbl in [("2010-01-01", "2018-01-01", "2010-2017"),
    ...
    ts = st.avol_stats(sub)
    print(f"  {lbl}: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-high-avol / short-low-avol, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=819 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0020, seed=819, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0020, seed 819): spread NW t = {sy['t_nw']:+.2f}, "
