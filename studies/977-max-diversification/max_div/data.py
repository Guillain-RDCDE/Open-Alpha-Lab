"""Data layer for Study 977 — Maximum Diversification.

Both panels are daily total-return closes. The multi-asset panel spans volatilities
from roughly 4% (IEF) to 25% (EEM), which matters: the diversification ratio is a ratio of a
*weighted average volatility* to a *portfolio volatility*, so a universe whose assets all have
the same volatility gives it very little to say.
...
# Two panels, matching this desk's other allocation studies so the results line up:
# a sector sleeve and a multi-asset sleeve. The multi-asset one is where a diversification
# ratio has the most to work with, because its assets genuinely differ in volatility.
SECTORS = ("XLB", "XLC", "XLE", "XLF", "XLI", "XLK", "XLP", "XLRE", "XLU", "XLV", "XLY")
MULTI = ("SPY", "IWM", "EFA", "EEM", "TLT", "IEF", "LQD", "HYG", "GLD", "DBC")
BENCH = "SPY"
CASH = "BIL"
TICKERS = SECTORS + MULTI + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call max_div.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 977) -> tuple[pd.DataFrame, dict]:
