"""Reproducible headline run for Study 497 — Woodie's Pivot Points.
...
from woodie_pivots import data, strategy as st  # noqa: E402
...
print("# Woodie's pivots — mechanical S1-support touch on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled S1-support touch vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'touch_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total S1 touches (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: touch one-sample t and touch-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        e = st.s1_touch_entries(b)
        re = st.random_entries(b, max(len(e), 50), seed=7)
        ...
        print(f"  {t}: ent={len(e):>4} touch={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Random-LEVEL placebo (SPY, H=20, 500 draws)")
    pl = st.random_level_placebo(_load("SPY"), 20, n_draws=500, seed=497)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => the specific Woodie S1 not load-bearing)")
else:
...
for edge in (0.0, 0.40):
    ...
    e = st.s1_touch_entries(px)
    s = st.summarize(st.forward_returns(px, e, 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  touch={s['mean_bps']:+7.1f}bps  "
