"""Data layer for Study 964 — All-Time High.

Total-return closes, not price closes, and the distinction is load-bearing: a
**record high** measured on a price series is a different set of days from a record high
measured on a wealth index, and dividends alone will drag a price chart below its old peak
for years. This study asks what happens to *money*, so the running maximum is computed on
the total-return tape and the point is made explicitly in the notebooks.
...
# Six tapes with different personalities: US large cap, US tech, developed and emerging
# international, long Treasuries, gold — plus BIL as the cash leg the "wait for a dip"
# rule parks in. A record-high rule that only works on the one index that went up in a
# straight line is not a finding.
RISKY = ("SPY", "QQQ", "EFA", "EEM", "TLT", "GLD")
CASH = "BIL"
TICKERS = RISKY + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call ath_buy.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 964) -> tuple[pd.DataFrame, dict]:
