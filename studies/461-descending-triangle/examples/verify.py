"""Reproducible headline run for Study 461 — Descending Triangle.
...
from descending_triangle import data, strategy as st  # noqa: E402
...
print("# Descending triangle — mechanical support-break SHORT on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled support-break SHORT vs drift-matched random baseline (short P&L, bps)")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.support_break_entries(c, k=K, lookback=LOOKBACK)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total support breaks (H={st.HORIZONS[0]}): {total}")
    ...
    print("\n# Scrambled-highs GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.scrambled_highs_placebo(_load("SPY")["close"], 20, k=K, lookback=LOOKBACK,
                                    n_draws=500, seed=461)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => descending-highs geometry not load-bearing)")
else:
...
for edge in (0.0, 0.50):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.support_break_entries(c, k=K, lookback=LOOKBACK), 20))
