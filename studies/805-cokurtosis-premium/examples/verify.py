"""Reproducible headline run for Study 805 — Cokurtosis Premium.
...
print("# Cokurtosis Premium — do high-market-cokurtosis names earn a positive risk premium?")
...
sp = st.cokurt_spreads(ret, window=WINDOW, frac=0.3)
h = st.cokurt_stats(sp)
print(f"\nsort: trailing-{WINDOW}d cokurtosis w/ equal-weight market, long top30% / "
      f"short bottom30%, {h['n_days']} days (median {int(sp['n'].median())} names/day)")
print("# THE HEADLINE — long-high-cokurt / short-low-cokurt spread")
print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
      f"one-sample t = {h['t_1s']:+.2f}")
print(f"  books : high-cokurt {h['hi_bps']:+.2f} vs low-cokurt {h['lo_bps']:+.2f} bps  "
...
pl = st.placebo_pvalue(ret, window=WINDOW, n_seeds=20, n_draws_per_seed=50)
sig_pos = (pl["obs_bps"] - pl["placebo_mean_bps"]) / pl["placebo_sd_bps"]
...
print(f"  observed sits {sig_pos:+.2f} sd from the permutation null")
...
for lo, hi, lbl in [("2010-01-01", "2018-01-01", "2010-2017"),
    ...
    ts = st.cokurt_stats(sub)
    print(f"  {lbl}: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-high-cokurt / short-low-cokurt, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(knob=0.0, seed=805 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(knob=0.009, seed=805, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1, window=WINDOW)
print(f"  planted (knob=0.009, seed 805): spread NW t = {sy['t_nw']:+.2f}, "
