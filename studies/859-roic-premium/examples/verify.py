"""Reproducible headline run for Study 859 — Return-on-Invested-Capital Premium.
...
from roic_premium import data, strategy as st  # noqa: E402

print("# ROIC Premium — does return on invested capital sort future returns, "
      "and beat plain ROE / gross profitability?")
...
print(f"[data] prices {px.shape}  events {ev.shape}  names={ev['ticker'].nunique()}  "
      ...
      f"as-of {data.AS_OF}  prices_fp={data.fingerprint(px)}")

print("\n# PRIMARY — calendar-time tercile long-short on ROIC level")
...
ls120 = st.calendar_ls(px, ev, signal_col="roic", n_buckets=3, min_names=6, staleness_days=120)
...
print(f"  ROIC-change variant (roic_chg): mean {s_chg['mean_bps']:+.1f} bps, "
      f"NW t = {s_chg['t_nw']:+.2f}, Sharpe {s_chg['sharpe']:.2f}, n={s_chg['n_months']}")

print("\n# CROSS-CHECK — pooled event drift (tercile sort by ROIC, 1-day-lag entry)")
...
print("\n# ERA SPLIT — calendar long-short pre/post 2018 (TCJA statutory-rate change)")
for lbl, lo, hi in [("2009-2017", "2000-01-01", "2018-01-01"),
...
for col in ("roic", "roic_chg", "roe", "gp"):
    d = c[col]
    print(f"  {col:>9}: LS {d['mean_bps']:+.1f} bps/mo  NW t = {d['t_nw']:+.2f}  "
          f"Sharpe {d['sharpe']:.2f}  n={d['n_months']}")
print(f"  ROIC vs ROE cross-sectional rank correlation = {c['roic_roe_rank_corr']:+.3f}")
...
print("  planted premium. Null checked over 12 seeds (never a single stream).")
null_ts = []
for s_ in range(12):
...
print(f"  planted edge=0.15 (seed 859): mean {pl['mean_bps']:+.1f} bps/mo, NW t = {pl['t_nw']:+.2f}")
