"""Data layer for Study 1010 — Mostly Noise.

Fifty large-cap names plus nine sector funds and the market proxy.

Fifty assets is chosen deliberately: it makes N/T a binding constraint at every estimation
window a practitioner would actually use. With one year of daily data, N/T is roughly 0.2 and
random-matrix theory says a large share of the eigenvalue spectrum is indistinguishable from
noise. The point of the study is that this is calculable in advance, from N and T alone.
...
MARKET = "SPY"
CASH = "BIL"
# A cross-section large enough that N/T is a real constraint, which is the point.
...
TICKERS = (MARKET, CASH) + NAMES + SECTORS
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call corrnoise.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1010) -> tuple[pd.DataFrame, dict]:
