"""Strategy + inference for Study 900 — Quality-Income.

We build two long-only, equal-weight, **monthly-rebalanced** dividend sleeves from live
ETFs and race them (and SPY) on **excess-of-cash** terms:

* **Quality sleeve** = equal weight SCHD + NOBL (durable / growing payers).
* **Yield sleeve**   = equal weight SPHD + VYM (raw high-yield screens).

The question — *does screening dividends for quality beat chasing yield?* — is answered
on three quantities:

* **Excess-of-cash Sharpe race.** Each sleeve's monthly return minus BIL (the realized
  cash return); annualised Sharpe of quality, yield and SPY, plus the **Sharpe gap**
  quality − yield. Because cash cancels in the difference, the quality-minus-yield
  monthly spread is cash-independent — we put a Newey-West (HAC) *t* on its mean and a
  **paired moving-block bootstrap** CI on the Sharpe gap itself.
* **Drawdown & calendar table.** Max drawdown per sleeve and a calendar-year return
  table — is the quality ride genuinely shallower?
...
Reuses ``quantlab.stats`` / ``quantlab.analytics`` where they fit.
"""
...
def sleeve_returns(monthly: pd.DataFrame, members: list[str]) -> pd.Series:
    ...
    is simply the equal-weight mean of its members' month-``t`` returns. Rows where any
    member is missing (a not-yet-listed ETF) are dropped so the sleeve starts only once
    ALL members trade — no partial-basket look-ahead.
    """
...
def sleeve_turnover(monthly: pd.DataFrame, members: list[str]) -> pd.Series:
    ...
    Over a month the two members drift away from 50/50; at month-end we trade back.
    Turnover_t = 0.5 * sum_i |w_drift_i - w_target_i| (one-way fraction of NAV traded).
    The FIRST in-sample month is the initial build (turnover = target weight sum / 2).
    Uses only within-month drift (returns known at ``t``) — no future information.
    """
    sub = monthly[members].dropna(how="any")
    n = len(members)
    w_t = 1.0 / n
...
# --------------------------------------------------------------------------- #
# The Sharpe race + the quality-minus-yield test
# --------------------------------------------------------------------------- #
...
def sharpe_gap_test(quality: pd.Series, yld: pd.Series, cash: pd.Series,
                    lags: int = 6) -> dict:
    """Excess-of-cash Sharpe race quality vs yield + HAC *t* on the monthly difference.

    The quality-minus-yield monthly spread ``d = quality - yield`` is cash-independent
    (cash cancels), so its mean/HAC *t* is the clean "does quality out-earn yield?"
    statistic. Reports both sleeves' excess Sharpe, the Sharpe gap, and the spread's
    mean / one-sample *t* / Newey-West *t*.
    """
    df = pd.concat([quality, yld, cash], axis=1).dropna()
    q, y, c = df.iloc[:, 0], df.iloc[:, 1], df.iloc[:, 2]
    d = (q - y).to_numpy(dtype=float)
    sh_q = excess_sharpe(q, c)
    sh_y = excess_sharpe(y, c)
    return {
        "n_months": len(df),
        "sharpe_q": sh_q,
        "sharpe_y": sh_y,
        "sharpe_gap": sh_q - sh_y,
...
def sharpe_gap_bootstrap(quality: pd.Series, yld: pd.Series, cash: pd.Series,
                         n_draws: int = 4000, block: int = 6, seed: int = 900) -> dict:
    """Paired moving-block bootstrap CI for the excess-of-cash Sharpe GAP (quality-yield).

    Joint monthly rows (quality, yield, cash) are resampled in blocks so serial and
    cross correlation survive; the Sharpe gap is recomputed per draw. Reports the 95% CI
    and the share of draws with a NEGATIVE gap (a blunt "could quality be no better?").
    """
    df = pd.concat([quality, yld, cash], axis=1).dropna()
    ...
    for i in range(n_draws):
        ...
        qe, ye = q[idx] - c[idx], y[idx] - c[idx]
        sdq, sdy = qe.std(ddof=1), ye.std(ddof=1)
        gq = qe.mean() / sdq * ann if sdq > 0 else np.nan
        gy = ye.mean() / sdy * ann if sdy > 0 else np.nan
        draws[i] = gq - gy
...
def era_cut(quality: pd.Series, yld: pd.Series, cash: pd.Series,
            split: str = "2020-01-01", lags: int = 6) -> dict:
    """Split the quality-vs-yield race into two eras at ``split``; report each half."""
    out = {}
    df = pd.concat([quality, yld, cash], axis=1).dropna()
    for lbl, lo, hi in [("early", df.index.min(), pd.Timestamp(split)),
        ...
        if len(sub) < 6:
            ...
            continue
        r = sharpe_gap_test(sub.iloc[:, 0], sub.iloc[:, 1], sub.iloc[:, 2], lags)
        out[lbl] = r
    return out
...
def synthetic_detect(world: pd.DataFrame, lags: int = 6) -> dict:
    """Run the quality-vs-yield Sharpe-gap test on a synthetic world (excess-of-cash).

    ``world`` columns ``quality`` / ``yield`` are already excess of cash, so a zero cash
    series recovers the same statistics the real race computes.
    """
    zero_cash = pd.Series(0.0, index=world.index, name="cash")
    q = world["quality"].rename("quality")
    y = world["yield"].rename("yield")
    r = sharpe_gap_test(q, y, zero_cash, lags)
    return {"sharpe_gap": r["sharpe_gap"], "t_nw": r["t_nw"],
            "diff_ann_pct": r["diff_ann_pct"], "n_months": r["n_months"]}
