"""Data layer for Study 978 — The Resampled Frontier.

Daily total-return closes for two panels. Resampling is defended most strongly for
mean-variance problems with dispersed expected returns, so the multi-asset panel — bonds,
credit, equities, gold, commodities — is the headline and the sector sleeve is the check that
the conclusion is not an artefact of one cross-section.
...
# Ten multi-asset sleeves — the setting Michaud resampling was sold for — plus the
# eleven sector SPDRs as a second, more homogeneous cross-section.
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call resampled.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 978) -> tuple[pd.DataFrame, dict]:
