...
def test_synthetic_shape_and_ohlc(random_walk):
    ...
    # OHLC sanity: high brackets the bar range.
...
def test_mean_rev_creates_negative_autocorrelation():
    """The mean_rev knob really induces negative serial dependence."""
    flat, _ = data.synthetic_daily(n_days=2000, mean_rev=0.0, seed=181)
    rev, _ = data.synthetic_daily(n_days=2000, mean_rev=0.30, seed=181)
    ...
    assert ac(r_rev) < -0.10  # planted mean reversion
