"""Data layer for Study 683 — the Evening-Star candlestick pattern.
...
* **Real tape.** Daily OHLCV (yfinance, ``auto_adjust=True``, no key) for a fixed basket
  ...
  still trading) — survivorship is named on the Signal axis. Same 30-name basket used by
  the desk's other candlestick-pattern studies (402/408), for a like-for-like comparison.
...
* **Synthetic.** ``synthetic_panel`` — a deterministic, fixed-seed generator with a
  **planted-edge knob**. With ``edge = 0`` the post-pattern path is pure noise (a fair
  ...
  so that the day(s) *after* a real evening star keep falling — the positive control
  proves the harness *can* bank a planted top/reversal.
...
# the same 30-name basket the desk's sibling candlestick studies (402-engulfing-pattern,
# 408-three-black-crows) use, for a like-for-like comparison. This is a *survivors*
# basket (every name still trades in 2026) — named on the Signal axis: a fixed
# surviving-names basket cannot include firms that were delisted after a bad run, a mild
# bias we reason about explicitly in docs/results.md.
BASKET = [
...
def _cache_path(ticker: str, cache_dir: str) -> str:
    ...
    return os.path.join(cache_dir, f"es_{safe}_1d.parquet")
...
def synthetic_panel(n_names: int = 30, n_days: int = 2200, edge: float = 0.0,
                    seed: int = 683, daily_vol: float = 0.013,
    ...
    Each name is a random walk in log-price with a mild upward drift (so evening stars —
    which need a tall UP candle — actually occur). We build proper OHLC bars: the open
    gaps from the previous close, the close is the random-walk close, and wicks extend
    the high/low. After each bar we test whether the last three bars formed a textbook
    **evening star** (see ``strategy.is_evening_star``); if ``edge != 0`` we add an
    *extra* drift of ``-edge`` to the **next** day's return — i.e. a real day-after
    continuation down in the direction the bearish pattern predicts.
    ...
    for k in range(n_names):
        eps = rng.normal(0.0004, daily_vol, n_days)   # mild up-drift feeds tall up candles
        gap = rng.normal(0.0, daily_vol * 0.5, n_days)
        ...
        prev_close = 100.0
        pending = 0      # -1 if an evening star just formed (applies to next bar)
        for i in range(n_days):
            ...
            hi[i] = max(o, c) + w
            lo[i] = min(o, c) - w
            # detect evening star on bars (i-2, i-1, i) -> set pending for i+1
            pending = 0
            if i >= 2:
                trip = _es_from_arrays(open_, close, hi, lo, i)
                if trip == -1:
                    pending = -1
            prev_close = c
...
def _es_from_arrays(o: np.ndarray, c: np.ndarray, h: np.ndarray, l: np.ndarray,
                    i: int, body_large_frac: float = 0.6, body_star_frac: float = 0.3,
                    gap_tol_frac: float = 0.25, penetration: float = 0.5) -> int:
    """Inline evening-star test on bars (i-2, i-1, i); -1 if present else 0.

    Mirrors ``strategy.is_evening_star`` so the synthetic generator stays causal and
    self-contained (no circular import at module build time). Same default parameters.
    """
