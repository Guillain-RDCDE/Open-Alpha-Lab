"""Reproducible headline run for Study 683 — Evening-Star.
...
from evening_star import data, strategy as st  # noqa: E402

NDRAWS = 5_000
AS_OF = "2026-06-30"        # last complete calendar month at publication (2026-07-10)

print("# Evening Star — signed-SHORT event study on a 30-name large-cap + SPY basket")
if data.have_real():
    ...
    print("\n# Signed-SHORT forward return after evening star (enter next open, 1-day lag)")
    ...
    print("\n# Myth check #1 — the STRICT textbook gap (star doesn't touch candle-1's body)")
    for h in st.HORIZONS:
        s = st.summarize(panel, h, n_draws=NDRAWS, gap_tol_frac=0.0)
    ...
    print("(no _cache/es_*.parquet — run data.load_real() with network once to build the cache)")
...
for edge in (0.0, 0.004, 0.008):
    ...
    print(f"  planted edge={edge:+.3f}/day (seed 683): planted_days={truth['n_planted_days']:>4}  "
