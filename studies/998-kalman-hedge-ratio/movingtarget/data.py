"""Data layer for Study 998 — The Moving Target.

Seven pairs spanning the spectrum from "should be a constant" (GLD/IAU, two funds
holding the same metal) to "drifts continuously" (GDX/GLD). Including the near-constant pairs is
essential: an adaptive estimator that improves on those too would be improving on noise.
...
# Pairs whose relationship is known to drift, plus one that should be nearly
# constant as a control.
PAIRS = (("GLD", "IAU"),        # two gold trackers: the ratio should barely move
         ("SPY", "IVV"),        # two S&P trackers: same
         ("XLE", "XOP"),        # energy majors vs producers: drifts a lot
         ("GDX", "GLD"),        # miners vs metal: drifts with costs and leverage
         ("EWA", "EWC"),        # the textbook pairs-trading example
         ("KO", "PEP"),         # the other textbook example
         ("TLT", "IEF"))        # two durations: drifts with the curve
CASH = "BIL"
TICKERS = tuple(sorted({t for p in PAIRS for t in p})) + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call movingtarget.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 998) -> tuple[pd.DataFrame, dict]:
