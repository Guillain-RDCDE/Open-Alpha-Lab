"""Reproducible headline run for Study 706 — Diamond Bottom.
...
from diamond_bottom import data, strategy as st  # noqa: E402
...
print("# Diamond bottom — mechanical upside-break long on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled diamond breakout (LONG) vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.diamond_breakouts(c, k=K, n_piv=N_PIV, tol=TOL)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total diamond breakouts (H={st.HORIZONS[0]}): {total}")
...
print("  edge=0 must NOT reach significance (checked over 20 seeds); planted reversal must light up.")
null_ts = []
for s_ in range(20):
    px, _ = data.synthetic_panel(edge=0.0, seed=706 + s_, n_days=8000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.diamond_breakouts(c, k=K, n_piv=N_PIV, tol=TOL), 20))
    null_ts.append(s["t"])
...
for edge in (0.0, 0.60):
    px, _ = data.synthetic_panel(edge=edge, seed=706, n_days=8000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.diamond_breakouts(c, k=K, n_piv=N_PIV, tol=TOL), 20))
    print(f"  edge={edge:.2f} (seed 706): n={s['n']:>4}  brk={s['mean_bps']:+7.1f}bps  "
