"""Data layer for Study 75 (Knee-Jerk).

Two tapes, one shape (a tz-naive daily OHLCV frame):

- ``synthetic_daily`` — a *deterministic, offline* generator. A day-level AR(1) drift
  knob (``reversion``) lets us dial the only thing the RSI(2) mean-reversion rule can
  possibly harvest: very-short-term *anti*-persistence (mean reversion). ``reversion=0``
  is a pure random walk — a fair coin — so a test can assert the rule beats random *only*
...
- ``fetch_daily`` — the real Yahoo! daily tape (``yfinance``), cache-only by default so
  the test-suite and the reproducible core never touch the network. Long history available
  for daily bars (decades), but we also test the pre/post 2009 split as the brief asks.
...
def synthetic_daily(
    ...
    structure the RSI(2) oversold rule needs to harvest.

    - ``reversion = 0``   → a martingale; the RSI(2) signal is a fair coin.
...
def fetch_daily(
    ...
    is exactly what we need for the pre/post publication split test.
    """
