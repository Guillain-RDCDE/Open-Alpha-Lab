"""Data layer for Study 1000 — The Cycle Hunt.

UNG is in the universe as a **positive control**: natural gas has a genuine annual
demand cycle, so any spectral method that cannot detect a periodicity there is failing to detect
rather than correctly reporting absence. A study that only ever finds nothing has not
demonstrated that nothing is there.
...
# A long tape, plus one series with a genuine known cycle as a positive control.
EQUITY = "SPY"
NASDAQ = "QQQ"
SMALL = "IWM"
BONDS = "TLT"
GOLD = "GLD"
CRYPTO = "BTC-USD"
# A real seasonal: natural gas has a genuine annual demand cycle, so a spectral method
# that cannot find THAT is broken rather than merely conservative.
SEASONAL = "UNG"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, SMALL, BONDS, GOLD, CRYPTO, SEASONAL, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call cyclehunt.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1000) -> tuple[pd.DataFrame, dict]:
