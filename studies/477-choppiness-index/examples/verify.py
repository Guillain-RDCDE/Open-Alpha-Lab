"""Reproducible headline run for Study 477 — Choppiness Index.
...
from choppiness_index import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch low-CI-vs-random test
    HAVE_SCIPY = True
...
print("# Choppiness Index — mechanical low-CI-onset long on 5 indices/ETFs (yfinance daily)")
print(f"# CI window={W}, low-CI threshold={LOW}")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled low-CI onset vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'lowci_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.low_ci_entries(b, window=W, low=LOW)
            re = st.random_entries(c, max(len(e), 50), window=W, seed=7)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total low-CI onsets (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: low-CI one-sample t and low-CI-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        c = b["close"]
        e = st.low_ci_entries(b, window=W, low=LOW)
        ...
        print(f"  {t}: ent={len(e):>3} lowci={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Return-shuffled GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_returns_placebo(_load("SPY"), 20, window=W, low=LOW, n_draws=500, seed=477)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => CI structure not load-bearing)")
else:
...
for edge in (0.0, 0.35):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.low_ci_entries(px, window=W, low=LOW), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  lowci={s['mean_bps']:+7.1f}bps  "
