"""Data layer for Study 993 — Down Hurts More.

Gold, Bitcoin and the euro are in this universe for a specific reason: none of them
has a balance sheet, so the debt-to-equity mechanism that gives the "leverage effect" its name
cannot operate on them. If the asymmetry shows up there too, the name is wrong.
...
# Equities (where the effect is famous), plus asset classes with no leverage story
# at all — the controls that decide between the two competing explanations.
EQUITY = "SPY"
NASDAQ = "QQQ"
SMALL = "IWM"
GOLD = "GLD"            # no balance sheet, so no "leverage" mechanism possible
BONDS = "TLT"
CRYPTO = "BTC-USD"      # no balance sheet either
CURRENCY = "FXE"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, SMALL, GOLD, BONDS, CRYPTO, CURRENCY, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call downhurts.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 993) -> tuple[pd.DataFrame, dict]:
