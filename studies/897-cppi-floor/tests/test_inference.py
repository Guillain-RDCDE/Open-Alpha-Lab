"""Offline tests for the inference primitives — HAC t, spanning alpha, bootstrap, Wilson, DD."""
import os
import sys
...
def test_bootstrap_ci_brackets_point(bear_world):
    frame, _ = bear_world
    ret = st.to_returns(frame)
    r = st.race(ret, multiplier=5.0, floor_frac=0.80)
    bs = st.bootstrap_sharpe_diff(r["cppi_net"], r["bh_net"], r["cash_net"], n_boot=500)
    lo, hi = bs["ci95"]
