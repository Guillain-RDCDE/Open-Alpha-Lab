"""Strategy/analysis-engine tests for Study 224 (Monday Effect), including spine tests."""
...
from monday_effect import strategy  # noqa: E402
...
def test_weekday_means_shape(null_tape):
    ...
    # Per-weekday counts add up to the number of return bars.
    assert int(wm["n"].sum()) == len(close) - 1


def test_weekday_position_calendar_known(null_tape):
    close = null_tape[0]["close"]
    pos = strategy.monday_only_position(close)
    expected = (close.index.dayofweek == 0).astype(float)
...
def test_contrast_sign_matches_means(null_tape):
    close = null_tape[0]["close"]
    c = strategy.contrast(close, 0)
    wm = strategy.weekday_means(close)
    assert abs(c["in_bps"] - wm.loc["Mon", "mean_bps"]) < 1e-6
...
def test_subperiod_recovers_planted_in_both_halves(planted_tape):
    """The planted effect is stationary so the change across the split is near zero."""
    close = planted_tape[0]["close"]
    cut = str(close.index[len(close) // 2].date())
    sp = strategy.subperiod_effect(close, 1, cut=cut)
