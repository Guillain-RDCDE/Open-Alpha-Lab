"""Data layer for Study 994 — The Rounding Tax.

Share prices matter here, not just returns — a $600 share and a $30 share behave
completely differently under whole-share constraints. The cached closes are total-return
adjusted, so the *level* is an adjusted level rather than the historical trade price; section 1
of the results quantifies how much that matters and why the conclusion survives it.
...
# Deliberately spans the share-price spectrum: a $600 index fund, a $40 one, and
# names whose prices have wandered by an order of magnitude over the sample.
CORE = "SPY"            # expensive per share
CHEAP_CORE = "SCHB"     # a cheap-per-share total-market fund
BONDS = "BND"
INTL = "VXUS"
SMALL = "IJR"
REIT = "VNQ"
GOLD = "GLD"
CASH = "BIL"
TICKERS = (CORE, CHEAP_CORE, BONDS, INTL, SMALL, REIT, GOLD, CASH)

# The target allocation the study rebalances back to.
TARGET = {CORE: 0.40, INTL: 0.20, BONDS: 0.25, SMALL: 0.05, REIT: 0.05, GOLD: 0.05}
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call roundingtax.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 994) -> tuple[pd.DataFrame, dict]:
