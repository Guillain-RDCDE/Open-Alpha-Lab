"""Data layer for Study 702 — Shark-Harmonic.
...
* **Synthetic world.** A deterministic, seeded daily price path with a TUNABLE
  ...
  (the null the Shark detector must NOT manufacture significance from); at
  ``mean_rev > 0`` price is nudged back toward a slow moving average, giving *any*
  well-formed extension-and-reversal structure (Shark-labelled or not) genuine
...
def _cache_path(ticker: str) -> str:
    return os.path.join(CACHE_DIR, f"shark_{ticker.lower()}.csv")
...
def synthetic_world(mean_rev: float = 0.0, n_days: int = 2500, annual_vol: float = 0.22,
                    start: str = "2010-01-04", seed: int = 702) -> pd.DataFrame:
    ...
    (Shark-labelled or not) show a genuine edge once ``mean_rev > 0``.
    ``mean_rev = 0`` is the null: the Shark detector's split must NOT reach
