"""Reproducible headline run for Study 702 — Shark-Harmonic.
...
from shark_harmonic import data, strategy as st  # noqa: E402
...
print("# Shark-Harmonic — does the '5-0' 0.886-1.13 completion zone predict a reversal?")
...
def pooled(placebo: bool, cost: float = 0.0, seed: int = 702, horizons=HORIZONS):
...
print("\n# THE HEADLINE — Shark XABCD (AB extends XA 1.13-1.618x past X, BC extends AB "
      "1.618-2.24x past A, D = a ZONE at [0.886, 1.13] x the original XA leg)")
shark_g, n_piv, n_cand_shark, shark_by_ticker = pooled(placebo=False, cost=0.0)
print(f"  pooled confirmed pivots: {n_piv:,}  |  Shark XABCD candidates: {n_cand_shark:,}  "
      f"|  D-zone touches: {len(shark_g):,}")

for h in HORIZONS:
    sf = st.summarize(shark_g, f"ret_gross_{h}")
...
# --------------------------------------------------------------------------- #
print("\n# SIGNAL AXIS — Shark D-zone fade vs a random-day BASE RATE "
...
base_g = base_rate_pool(shark_by_ticker)
tests = []  # (label, welch_t, n_shark, n_base)
for h in HORIZONS:
    sf = st.summarize(shark_g, f"ret_gross_{h}")
    sb = st.summarize(base_g, f"ret_gross_{h}")
    wt = st.welch_t(shark_g[f"ret_gross_{h}"], base_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: SHARK mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
for t in data.BASKET:
    ...
    n_ev = len(l_shark)
    if n_ev == 0:
        print(f"  {t:5s} n=0 (no D-zone touches)")
        continue
    mix = float((l_shark["reversal_dir"] > 0).mean())
    frames = [st.base_rate_ledger(bars[t], n_ev, mix, horizons=HORIZONS, cost_bps=0.0,
                                  seed=st._seed_from(702, t, s)) for s in range(BASE_SEEDS)]
    br_t = pd.concat(frames, ignore_index=True)
    sf = st.summarize(l_shark, "ret_gross_5")
    sb = st.summarize(br_t, "ret_gross_5")
    wt = st.welch_t(l_shark["ret_gross_5"], br_t["ret_gross_5"])
    tests.append((f"{t} (5-day)", wt))
    print(f"  {t:5s} SHARK n={sf['n']:3d} mean={sf['mean_bps']:+7.2f}bps   |   "
...
# --------------------------------------------------------------------------- #
# Third axis (myth-check) — does the precise 0.886-1.13 zone beat a placebo?
# --------------------------------------------------------------------------- #
print("\n# THIRD AXIS (myth-check) — does the precise 0.886-1.13 XA completion zone beat "
      "a placebo zone built off the identical pivots?")
plac_g, _, n_cand_plac, plac_by_ticker = pooled(placebo=True, cost=0.0)
print(f"  placebo candidates: {n_cand_plac:,}  |  placebo D-zone touches: {len(plac_g):,}")
for h in HORIZONS:
    sf = st.summarize(shark_g, f"ret_gross_{h}")
    sp = st.summarize(plac_g, f"ret_gross_{h}")
    wt = st.welch_t(shark_g[f"ret_gross_{h}"], plac_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: SHARK mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
print(f"\n  Shark beats placebo on {n_beats}/{len(data.BASKET)} tickers (5-day)")
...
null_ts = []
for s_ in range(20):
    ledger = pooled_synth(0.0, 702 + s_)
...
planted = pooled_synth(0.70, 702)
sp_planted = st.summarize(planted, "ret_gross_5")
print(f"  planted mean_rev=0.70 (seed 702): n={sp_planted['n']}  mean={sp_planted['mean_bps']:+.2f}bps  "
      f"HAC t={sp_planted['t']:+.2f}  (a strong reversion tendency is needed for this "
      "structure's 5-day fade to light up — the sign runs opposite the Shark's own "
      "'resume-the-trend' story, expected: this is a generic power check, not a validation "
      "of the directional narrative)")
