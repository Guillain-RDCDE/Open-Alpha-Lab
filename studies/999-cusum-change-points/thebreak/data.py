"""Data layer for Study 999 — The Break.

A long daily tape with several agreed-upon regime changes in it — 1998, 2000, 2008,
2020, 2022 — which gives the detectors real events to be graded against, rather than only
simulated ones.
...
# A long equity tape plus assets with very different regime structure.
EQUITY = "SPY"
NASDAQ = "QQQ"
BONDS = "TLT"
GOLD = "GLD"
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call thebreak.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 999) -> tuple[pd.DataFrame, dict]:
