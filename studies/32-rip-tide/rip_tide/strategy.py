...
The deliberate mirror of Study 31's trend book. For each market independently:
  1. **Signal** — the *negative* sign of a blended trailing short return (1/3/5-day), lagged. SHORT a
     market that just rose, LONG one that just fell. This is the only line that differs from TSMOM.
...
def reversal_signal(returns: pd.DataFrame, lookbacks=(1, 3, 5)) -> pd.DataFrame:
    ...
    cumulative returns over each (short) lookback in days, lagged one day so it's tradable. Short a
    market that has just risen, long one that has just fallen."""
...
def book_returns(returns: pd.DataFrame, cost_bps: float = 2.0, **kw) -> pd.Series:
    """Net daily return of the contrarian book: positions applied to next-day returns, minus turnover
