...
finds none in the null; the signal is causal (lagged) and is the negative of the trend signal; turnover
is high; the cost wall bites; and the holding-period rescue behaves."""
...
def test_signal_is_causal_and_contrarian():
    """reversal_signal is lagged AND the exact negative of the momentum sign of the same window."""
    ...
    # flipping the LAST day's return must not change any earlier signal (causal / lagged)
    ...
    mom = np.sign(sum(np.sign(prices / prices.shift(lb) - 1.0) for lb in (1, 3, 5)) / 3).shift(1).fillna(0.0)
