...
the divergence long-short lights up *only when an effect is planted*."""
...
from margin_inventory import strategy as st  # noqa: E402
...
# leads-margin axis
# --------------------------------------------------------------------------- #
def test_leads_margin_planted_correlation():
    import pandas as pd
    rng = np.random.default_rng(5)
    n = 400
    g = rng.normal(0, 0.15, n)
    fut = 0.6 * g + rng.normal(0, 0.03, n)          # divergence truly leads future margin change
    ev = pd.DataFrame({"divergence": g, "next_d_gross_margin": fut})
    out = st.leads_margin(ev)
    assert out["slope"] > 0 and out["t"] > 5.0
    assert out["spread"] > 0


def test_leads_margin_no_relation():
    import pandas as pd
    rng = np.random.default_rng(6)
    n = 400
    ev = pd.DataFrame({"divergence": rng.normal(0, 0.15, n),
                       "next_d_gross_margin": rng.normal(0, 0.05, n)})
    out = st.leads_margin(ev)
