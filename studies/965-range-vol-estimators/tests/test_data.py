"""Data-layer tests for Study 965 — synthetic determinism offline, cache-gated on tape."""
...
def test_synthetic_ohlc_is_deterministic():
    a, _ = data.synthetic_ohlc(n_years=4, seed=965)
    b, _ = data.synthetic_ohlc(n_years=4, seed=965)
    assert np.allclose(a.to_numpy(), b.to_numpy())


def test_synthetic_ohlc_seed_sensitive():
    a, _ = data.synthetic_ohlc(n_years=4, seed=965)
    b, _ = data.synthetic_ohlc(n_years=4, seed=965 + 1)
    assert not np.allclose(a.to_numpy(), b.to_numpy())


def test_bars_are_internally_consistent():
    bars, truth = data.synthetic_ohlc(n_years=5, seed=965)
...
def test_range_is_never_degenerate():
    ...
    bars, _ = data.synthetic_ohlc(n_years=5, seed=965)
    assert (np.log(bars["high"] / bars["low"]) > 0).all()


def test_signal_strength_switches_clustering_off():
    _, t1 = data.synthetic_ohlc(n_years=8, signal_strength=1.0, seed=965)
    _, t0 = data.synthetic_ohlc(n_years=8, signal_strength=0.0, seed=965)
    assert t1["sigma"].std() > 0
    assert np.isclose(t0["sigma"].std(), 0.0, atol=1e-15)


def test_fingerprint_stable_and_sensitive():
    a, _ = data.synthetic_ohlc(n_years=3, seed=965)
    b, _ = data.synthetic_ohlc(n_years=3, seed=965 + 1)
