"""Strategy + inference for Study 689 — the Upside Gap Two Crows candlestick pattern.

The claim (an obscure but famous-in-the-books bearish-reversal candle): **upside gap two
crows** — a bullish (white) candle, then a black candle that **gaps up** away from it (the
"upside gap"), then a second black candle that opens even higher and closes lower than
the first crow, engulfing its body but *not quite* closing the gap — marks a top. The
folk recipe: after a rally, a gap-up that two "crows" (black candles) fail to fully
retrace warns that the rally is exhausted and a top is forming; short it, or get out.

Precise detector (a textbook three-candle upside-gap-two-crows), on bars (t-2, t-1, t):

    Let (O0,C0),(O1,C1),(O2,C2) be the three bars oldest -> newest, with H/L for wicks.
    1. candle t-2 is a bullish (white) real body:         C0 > O0
    2. candle t-1 GAPS UP from candle t-2 and is bearish (black):
           min(O1,C1) > max(O0,C0)     (a genuine gap between the real bodies — the
                                        default, LOOSE gap; the strict myth-check
                                        tightens this to a full range gap, L1 > H0)
           C1 < O1
    3. candle t is bearish (black), opens ABOVE candle t-1's open and closes BELOW
       candle t-1's close (its body engulfs candle t-1's body from above — the second
       "crow" digs further into the gap), but the gap is NOT fully filled:
           C2 < O2,  O2 >= O1,  C2 <= C1,  C2 > C0
    (Optional myth-check filters: a strict full-range gap (``true_range_gap=True``)
     and/or a genuine prior UPTREND context so the "reversal" is reversing something.)

The pattern is confirmed at the **close** of bar t. Signed by direction, upside gap two
...
Measurement (no look-ahead, one documented execution lag):
    ...
    **unconditional** base rate: the same forward return computed on every bar of the
    same basket (not just the ones that follow an upside-gap-two-crows), since the
    basket carries the market's own positive drift and a naive vs-zero test is
    contaminated by it.

Inference, the desk's shared spirit:

  * a one-sample **Newey-West (HAC) t** of the signed forward return vs 0 (informational
    — contaminated by the basket's own drift) and a **Welch t** vs the unconditional
    base rate (the decisive, drift-neutral comparison);
  * a **hit rate** (share of events where shorting paid) vs the unconditional base
    rate's hit rate, both with Wilson intervals;
...
The decisive number is the signed forward return, net, with its Welch t vs the
unconditional base — and whether it clears t >= 2, basket-wide AND ticker-by-ticker
after Bonferroni.
"""
...
HORIZONS = (1, 5, 10)               # trading-day forward-return horizons


# --------------------------------------------------------------------------- #
# Detector
# --------------------------------------------------------------------------- #
def is_upside_gap_two_crows(o0: float, c0: float, h0: float, l0: float,
                            o1: float, c1: float, h1: float, l1: float,
                            o2: float, c2: float, h2: float, l2: float,
                            true_range_gap: bool = False) -> int:
    """Return -1 (upside gap two crows confirmed) or 0 for the bar triple (oldest -> newest).

    Bars are passed oldest first: (0) = t-2 (the bullish candle), (1) = t-1 (the first
    "crow", gapping up), (2) = t (the second, confirming "crow", engulfing bar 1's body
    from above but not fully filling the gap). ``true_range_gap=True`` enforces the
    textbook strict gap (bar 1's entire wick range clears bar 0's high, ``l1 > h0``); the
    default is the looser, realistic body-level gap (bar 1's real body clears bar 0's
    real body — liquid US equities rarely gap by their whole daily range).
    """
    # 1. bar0: bullish (white) real body
    if not (c0 > o0):
        return 0
    # 2. gap up bar0 -> bar1
    if true_range_gap:
        ...
        if not (min(o1, c1) > max(o0, c0)):
            return 0
    # 3. bar1: bearish (black) real body — the first crow
    if not (c1 < o1):
        return 0
    # 4. bar2: bearish (black) real body, engulfing bar1's body from above — the second crow
    if not (c2 < o2):
        return 0
    if not (o2 >= o1 and c2 <= c1):
        return 0
    # 5. the gap is NOT fully filled: bar2's close stays above bar0's close
    if not (c2 > c0):
        return 0
    return -1


def ugtc_signals(bars: pd.DataFrame, true_range_gap: bool = False) -> pd.Series:
    """-1 / 0 per bar: -1 where the bar CONFIRMS an upside gap two crows (uses t-2, t-1, t)."""
...
def _trend_up(bars: pd.DataFrame, lookback: int = 10) -> np.ndarray:
    ...
    An upside gap two crows is meant to be a *reversal* of a rally; we look at the close
...
def collect_events(panel: dict[str, pd.DataFrame], horizon: int,
    ...
    """Pool upside-gap-two-crows events across the basket with their SIGNED forward return.
    ...
    (a top-of-rally signal: signed_ret = -1 * forward move), drop events whose window
    ...
    for tk, bars in panel.items():
        if bars is None or len(bars) < horizon + 15:
            continue
        sig = ugtc_signals(bars, true_range_gap=true_range_gap).to_numpy()
...
def unconditional_base(panel: dict[str, pd.DataFrame], horizon: int) -> np.ndarray:
    ...
    Because the event return is signed SHORT, we compare it against the raw (long)
    unconditional pool carefully (a short of nothing earns minus the market's up-drift).
    """
    vals = []
...
def placebo_pvalue(panel: dict[str, pd.DataFrame], horizon: int, n_events: int,
                   obs_mean: float, n_draws: int = 5000, seed: int = 689) -> dict:
...
def per_ticker_stats(panel: dict[str, pd.DataFrame], horizon: int,
    ...
    """One row per ticker with >= ``min_events`` UGTC events: n, mean, HAC t, Welch t.
