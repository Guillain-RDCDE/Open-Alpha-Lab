"""Reproducible headline run for Study 450 — Andrews' Pitchfork.
...
from andrews_pitchfork import data, strategy as st  # noqa: E402
...
print("# Andrews' pitchfork — mechanical lower-tine-touch on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled lower-tine-touch vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total lower-tine touches (H={st.HORIZONS[0]}): {total}")
    ...
    pl = st.shuffled_pivot_placebo(_load("SPY")["close"], 20, k=K, n_draws=500, seed=450)
...
for edge in (0.0, 0.40):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.tine_touch_entries(c, k=K), 20))
