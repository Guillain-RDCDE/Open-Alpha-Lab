"""Data layer for Study 126 (Parabolic-SAR).

Two tapes, one shape (a tz-naive daily OHLCV frame):

- ``synthetic_daily`` — a *deterministic, offline* generator.  A bar-level AR(1)
  drift knob (``momentum``) lets us dial the only structure the Parabolic SAR can
  possibly harvest: directional return persistence.  ``momentum=0`` is a pure random
  walk — a fair coin — so a test can assert the SAR flip beats random *only* when we
  plant a trend, and not otherwise.  This is the study's null in a bottle.
- ``fetch_daily`` — the real Yahoo! daily tape (``yfinance``), cache-only by default so
  ...
  long (years of data), giving the SAR's acceleration-factor ramp enough warmup and
  enough independent flip events for meaningful inference.

Parabolic SAR uses Wilder's classic defaults: initial acceleration factor AF = 0.02,
step = 0.02 per bar the extreme price extends, cap = 0.20.

No look-ahead is baked in here — that discipline lives in ``strategy.py`` (signals are
formed on closes / price extremes up to *t*, positions entered at *t+1*'s open).
"""
...
# Wilder's canonical SAR defaults
SAR_AF_INIT = 0.02
SAR_AF_STEP = 0.02
SAR_AF_MAX  = 0.20
...
def synthetic_daily(
    ...
    """A reproducible daily OHLCV tape with a known amount of directional persistence.
    ...
    - ``momentum = 0``   → a martingale; the SAR flip is a fair coin.
    - ``momentum > 0``   → return persistence the trend indicator can ride.
    - ``momentum < 0``   → mean reversion the SAR is on the *wrong* side of.
...
def fetch_daily(
    ...
    default — giving the Parabolic SAR's acceleration-factor ramp enough warmup bars
    and enough independent flip events for a meaningful inference.
    """
