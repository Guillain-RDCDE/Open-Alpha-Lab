"""Data layer for Study 672 — McGinley Dynamic.
...
* **Real tape.** Daily total-return bars (`auto_adjust=True`) for a five-ticker basket
  (SPY, QQQ, AAPL, MSFT, XLE — the same liquid basket as sibling studies 432/434) via
...
John McGinley's "Dynamic" (Technically Speaking, 1997) is a moving average whose update
speeds up or slows down automatically with how far price has run from the line::

    MD(t) = MD(t-1) + (P(t) - MD(t-1)) / (N * (P(t) / MD(t-1))^4)

N is the nominal "period" (McGinley's own default is 14 for daily data — the knob is a
speed constant, not a lookback window). The claim: because the ``^4`` term makes the line
speed up in fast markets and slow down (hug tightly) in slow ones, it "auto-adjusts" and
gives cleaner crossovers than a fixed SMA/EMA of the same nominal length.

Pure numpy + pandas + stdlib on the offline path. ``fetch()``/``load_real(fetch=True)``
...
def _cache_path(ticker: str, cache_dir: str = DEFAULT_CACHE) -> str:
    ...
    return os.path.join(cache_dir, f"mcd_{safe}_1d.parquet")
...
def synthetic_panel(
    ...
    The only structure any moving-average trend rule (McGinley's Dynamic included) can
    possibly harvest is *persistent* drift. The drift sign flips every ``regime_len``
    bars; its magnitude is ``edge * daily_vol``.
