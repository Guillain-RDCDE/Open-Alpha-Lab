"""Data layer for Study 1007 — Time Does Not Diversify.

Daily total returns for US large and small caps, developed international,
aggregate and long bonds, inflation-linked bonds and short Treasuries.

Thirty-three years contains only three genuinely independent thirty-year periods, so the long
horizons here rest on overlapping windows and the honest analysis has to say what that costs.
The block bootstrap and the synthetic control exist for exactly that reason.
...
CASH = "BIL"
BILLS = "SHY"
TICKERS = (EQUITY, SMALL, EAFE, BONDS, LONG_BONDS, TIPS, CASH, BILLS)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call timediv.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1007) -> tuple[pd.DataFrame, dict]:
