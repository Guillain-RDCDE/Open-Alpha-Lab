...
def _turnover_per_month(macro: pd.DataFrame, kind: str, smooth: int) -> float:
    w = macro_momentum_weights(macro, smooth=smooth) if kind == "macro_momentum" \
        else inflation_tilt_weights(macro, smooth=smooth)
    return float(w.diff().abs().sum(axis=1).mean())


def cost_sweep(returns: pd.DataFrame, macro: pd.DataFrame, kind: str = "macro_momentum",
               roundtrip_bps=(0, 5, 10, 25, 50), smooth: int = 3) -> pd.DataFrame:
...
def breakeven_cost_bps(returns: pd.DataFrame, macro: pd.DataFrame, kind: str = "macro_momentum",
                       hi: float = 500.0, smooth: int = 3) -> float:
    ...
    gross_mean = book_returns(returns, macro, kind=kind, cost_bps=0.0, smooth=smooth).mean()
    tpm = _turnover_per_month(macro, kind, smooth)
