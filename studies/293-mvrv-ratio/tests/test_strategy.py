...
the regression recovers a contrarian MVRV->price link only when one is planted."""
...
# Signal construction
# ---------------------------------------------------------------------------
def test_mvrv_stretch_sign(null_series):
    """log(mvrv/center): positive when mvrv>center, negative when below."""
    df, _ = null_series
    z = st.mvrv_stretch(df, center=1.0)
    m = df["mvrv"]
    assert (z[m > 1.0] > 0).all()
    assert (z[m < 1.0] < 0).all()


def test_mvrv_stretch_deterministic(null_series):
    df, _ = null_series
    a = st.mvrv_stretch(df, center=1.8)
    b = st.mvrv_stretch(df, center=1.8)
...
# ---------------------------------------------------------------------------
def test_regression_recovers_planted_contrarian(live_series):
    """With a planted contrarian link, the slope is NEGATIVE and |t|>2."""
    df, _ = live_series
    reg = st.predictive_regression(df)
    assert reg["slope_mvrv"] < 0.0, f"Expected negative (contrarian) slope, got {reg['slope_mvrv']:.3f}"
    assert reg["t_mvrv"] < -2.0, f"Planted contrarian signal should give t<-2, got {reg['t_mvrv']:.2f}"
...
def test_regression_null_near_zero(null_series):
    ...
    df, _ = null_series
    reg = st.predictive_regression(df)
    assert np.isfinite(reg["t_mvrv"])
    assert abs(reg["t_mvrv"]) < 2.5, f"Null |t| too large: {reg['t_mvrv']:.2f}"
...
# Band classification
# ---------------------------------------------------------------------------
def test_mvrv_state_values(live_series):
    df, _ = live_series
    state = st.mvrv_state(df, high=3.5, low=1.0)
...
def test_state_forward_stats_labels(live_series):
    ...
    assert set(tab.index) == {"over-heated", "neutral", "under-valued"}
