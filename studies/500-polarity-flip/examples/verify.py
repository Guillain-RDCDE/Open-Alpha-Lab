"""Reproducible headline run for Study 500 — Polarity-Flip (role reversal).
...
from polarity_flip import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch retest-vs-random test
    HAVE_SCIPY = True
...
print("# Polarity-flip — mechanical broken-resistance retest on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled polarity-retest vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'retest_bps':>11} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>11.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              f"{sr['mean_bps']:>8.1f} {s['mean_bps']-sr['mean_bps']:>7.1f} {sn['mean_bps']:>8.1f} "
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total polarity retests (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: retest one-sample t and retest-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.polarity_entries(c, k=K, band=BAND, break_buf=BREAK_BUF)
        re = st.random_entries(c, max(len(e), 50), k=K, seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} retest={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Scrambled-level placebo (SPY, H=20, 500 draws)")
    pl = st.scrambled_level_placebo(_load("SPY")["close"], 20, k=K, band=BAND,
                                    break_buf=BREAK_BUF, n_draws=500, seed=500)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => the specific broken level not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.polarity_entries(c, k=K, band=BAND, break_buf=BREAK_BUF), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  retest={s['mean_bps']:+7.1f}bps  "
