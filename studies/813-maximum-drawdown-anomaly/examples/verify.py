"""Reproducible headline run for Study 813 — the Maximum-Drawdown Anomaly.
...
print("# Maximum-Drawdown Anomaly — do deep-drawdown names under-earn or rebound?")
...
print("  SURVIVORSHIP: current-membership mega-cap panel — magnitudes are an upper "
      "bound (delisted deep-drawdown names absent). Named on the Signal axis.")

for w in (252,):
    sp = st.maxdd_spreads(closes, window=w, frac=0.3)
    h = st.maxdd_stats(sp)
    print(f"\nsort: trailing-{w}d max drawdown, long bottom30% (calm) / short top30% "
          f"(distressed), {h['n_days']} days (median {int(sp['n'].median())} names/day)")
    print("# THE HEADLINE — long-calm / short-distressed spread (spread = calm - distressed)")
    print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
          f"one-sample t = {h['t_1s']:+.2f}")
    print(f"  books : calm {h['lo_bps']:+.2f} vs distressed {h['hi_bps']:+.2f} bps  "
          f"(Welch t = {h['welch_t']:+.2f})")
    sps = sp["spread"].to_numpy()
    print(f"  gross spread Sharpe (no cost): "
          f"{sps.mean() / sps.std(ddof=1) * np.sqrt(252):.2f}")

sp = st.maxdd_spreads(closes, window=252, frac=0.3)
print("\n# PLACEBO — column-permute the forward returns (1,000 permutations)")
pl = st.placebo_pvalue(closes, n_seeds=20, n_draws_per_seed=50)
...
print(f"  observed is {sigma:+.2f} sigma from the permutation null centre")
...
for lo, hi, lbl in [("2010-01-01", "2018-01-01", "2010-2017"),
    ...
    ts = st.maxdd_stats(sub)
    print(f"  {lbl}: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-calm / short-distressed, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=813 + s_, n_assets=40, n_days=1500)
...
p1 = data.synthetic_panel(edge=0.004, seed=813, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.004, seed 813): spread NW t = {sy['t_nw']:+.2f}, "
