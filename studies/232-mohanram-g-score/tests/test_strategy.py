...
from mohanram_g_score import strategy as st  # noqa: E402
...
def test_hedge_positive_with_premium():
    """With a planted premium, the high-G quintile should beat the low."""
...
# ---------------------------------------------------------------------------
# Real cache test -- skip if absent
# ---------------------------------------------------------------------------
_CACHE_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))
_CACHE_PATHS = [
    os.path.join(_CACHE_ROOT, "_cache", f)
    for f in (
...
def test_real_panel_hedge_runs():
    """Real panel runs without errors and returns a non-empty hedge series."""
    from mohanram_g_score import data
