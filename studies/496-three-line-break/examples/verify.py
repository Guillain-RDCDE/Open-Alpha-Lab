"""Reproducible headline run for Study 496 — Three-Line-Break.
...
from three_line_break import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch reversal-vs-random test
    HAVE_SCIPY = True
...
print("# Three-Line-Break — mechanical 3-line up-reversal on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled TLB up-reversal vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'rev_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.reversal_entries(c, n_lines=NL)
            re = st.random_entries(c, max(len(e), 50), n_lines=NL, seed=7)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total up-reversals (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: reversal one-sample t and reversal-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.reversal_entries(c, n_lines=NL)
        re = st.random_entries(c, max(len(e), 50), n_lines=NL, seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} rev={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Shuffled-returns GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_returns_placebo(_load("SPY")["close"], 20, n_lines=NL, n_draws=500, seed=496)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => line-break geometry not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.reversal_entries(c, n_lines=NL), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  rev={s['mean_bps']:+7.1f}bps  "
