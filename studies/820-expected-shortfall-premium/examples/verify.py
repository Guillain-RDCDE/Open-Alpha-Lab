"""Reproducible headline run for Study 820 — Expected-Shortfall Premium.
...
print("# Expected-Shortfall Premium — do fat-left-tail (high-ES) names go on to earn more?")
...
for w in (252,):
    sp = st.es_spreads(ret, window=w, alpha=0.05, frac=0.3)
    h = st.es_stats(sp)
    print(f"\nsort: trailing-{w}d Expected Shortfall (5%), long top30% (high ES) / "
          f"short bottom30% (low ES), {h['n_days']} days (median {int(sp['n'].median())} names/day)")
    print("# THE HEADLINE — long-high-ES / short-low-ES spread")
    print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
          f"one-sample t = {h['t_1s']:+.2f}")
    print(f"  books : high-ES {h['hi_bps']:+.2f} vs low-ES {h['lo_bps']:+.2f} bps  "
          f"(Welch t = {h['welch_t']:+.2f})")
    sps = sp["spread"].to_numpy()
    print(f"  gross spread Sharpe (no cost): "
          f"{sps.mean() / sps.std(ddof=1) * np.sqrt(252):.2f}")

sp = st.es_spreads(ret, window=252, alpha=0.05, frac=0.3)
...
print(f"  observed sits {sig_from_mean:+.2f} sigma from the placebo mean")
...
print("\n# THE TIMER — long-high-ES / short-low-ES, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=820 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0024, seed=820, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0024, seed 820): spread NW t = {sy['t_nw']:+.2f}, "
