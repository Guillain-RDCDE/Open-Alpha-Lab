"""Data layer for Study 820 — Expected-Shortfall Premium.

The claim under test (a downside **tail-risk premium**): a stock whose recent daily
returns have a fat **left tail** — a large historical **Expected Shortfall** (CVaR)
at 5%, i.e. the mean of the *worst 5%* of its daily returns — is a riskier holding, so
if tail risk is *priced* it should earn **more**. Sort long **high-ES-magnitude** /
short **low-ES-magnitude**; the theory predicts a positive spread.
...
* **Synthetic world — the positive control.** A deterministic, seeded panel
  ...
  latent "tail loading" ``c_i[t]`` that both (a) fattens the **left tail** of its
  daily returns (so a trailing Expected-Shortfall sort proxies ``c_i``) and — only
  when ``edge > 0`` — *raises* its forward mean return. ``edge = 0`` is the null
  world: tail fatness still varies across names but carries **no** information about
  forward returns, and the sort must find nothing. ``edge > 0`` plants the priced
  downside-tail-risk premium (fat left tail earns more).
...
# --------------------------------------------------------------------------- #
# Synthetic world — planted priced downside-tail-risk premium (positive control)
# --------------------------------------------------------------------------- #
def synthetic_panel(
    edge: float = 0.0,
    seed: int = 820,
    ...
    """Deterministic seeded OHLC panel with a TUNABLE planted tail-risk->return relation.

    Each asset ``i`` carries a persistent latent "tail loading" ``c_i[t]`` — an AR(1)
    with autocorrelation ``factor_rho``. The loading fattens the **left tail** of the
    daily return via a signed quadratic-in-shock term (so a trailing Expected-Shortfall
    sort proxies ``c_i``), and — only when ``edge > 0`` — *raises* the **forward mean**:

        z ~ N(0,1)
        tail_shock = daily_vol * (z - tail_gain * c_i[t] * (z**2 - 1))
        r[i,t] = drift + edge * c_i[t] + tail_shock

    A high positive loading makes returns **left-fat** (large negative days -> a big
    Expected Shortfall) *and* (with ``edge > 0``) higher-mean — the priced downside-
    tail-risk premium: fat left tail, higher forward return. ``edge = 0`` is the null:
    tail fatness still varies across names but predicts nothing. Business-day index;
    span well below the pandas ns-timestamp horizon.
    """
    ...
    for i in range(n_assets):
        ...
        tail = daily_vol * (z - tail_gain * c * (z ** 2 - 1.0))
        r = drift + edge * c + tail
