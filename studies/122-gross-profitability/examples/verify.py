"""Real-panel verification — Study 122 (Gross-Profitability). Regenerates docs/results.md numbers.

Reads the desk's shared EDGAR caches (GrossProfit, Assets, yrret), runs the top-quintile /
bottom-quintile long-short with a one-year reporting lag, and reports vs the equal-weight
universe. Network is never touched; the EDGAR cache is maintained centrally.

    python studies/122-gross-profitability/examples/verify.py
"""
...
def main() -> None:
    ...
    print("Study 122 — Gross-Profitability — real EDGAR panel\n")
    fp_sig = data.fingerprint(signal)
    fp_fwd = data.fingerprint(fwd_ret)
    print(f"Signal panel (GrossProfit/Assets): {signal.shape}, fingerprint={fp_sig}")
    print(f"Forward returns panel:             {fwd_ret.shape}, fingerprint={fp_fwd}")
    print(f"Years covered: {signal.index.min()} – {signal.index.max()}")
    print(f"As-of: 2026-06-14 | Source: EDGAR XBRL (GrossProfit, Assets) + Yahoo monthly\n")
    ...
    print("=== Top-quintile GP/A vs Bottom-quintile GP/A (long-short hedge) ===")
    ...
    print(f"  High GP/A mean:  {s_high['mean']*100:+.2f}%/yr  (market: {s_mkt['mean']*100:+.2f}%/yr)")
    print(f"  Low GP/A mean:   {s_low['mean']*100:+.2f}%/yr")
