"""Reproducible headline run for Study 454 — Three-Outside-Up / Three-Outside-Down.
...
from three_outside import data, strategy as st  # noqa: E402
...
print("# Three-outside-up — mechanical engulf+confirm on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled three-outside-up vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.three_outside_entries(b, side=SIDE)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total three-outside-up entries (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: three-outside-up one-sample t and pattern-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        c = b["close"]
        e = st.three_outside_entries(b, side=SIDE)
        ...
        print(f"  {t}: ent={len(e):>3} patt={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Confirmation-shuffle placebo (SPY, H=20, 500 draws)")
    pl = st.confirm_shuffle_placebo(_load("SPY"), 20, side=SIDE, n_draws=500, seed=454)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => confirmation not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    ...
    e = st.three_outside_entries(px, side=SIDE)
