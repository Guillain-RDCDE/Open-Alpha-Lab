"""Data layer for Study 1009 — Sortino's Free Lunch.

A dozen assets chosen to span the skewness spectrum: equity indices, long bonds,
gold, REITs, utilities and technology. Skewness is the only thing that can make Sortino disagree
with Sharpe, so a panel with variation in it is the minimum needed to say anything.
...
CASH = "BIL"
BILLS = "SHY"
TICKERS = (EQUITY, NASDAQ, SMALL, EAFE, EM, BONDS, LONG_BONDS, GOLD, REITS,
           UTILITIES, TECH, CASH, BILLS)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call sortino.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1009) -> tuple[pd.DataFrame, dict]:
