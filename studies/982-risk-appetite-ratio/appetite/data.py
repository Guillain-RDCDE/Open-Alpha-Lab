"""Data layer for Study 982 — The Appetite Gauge.

SPHB and SPLV both launched in 2011, which caps this study at roughly fifteen years —
one full cycle and two crashes. That is stated up front because it is the binding constraint on
every *t*-statistic below, and no amount of daily data fixes a fifteen-year sample.
...
# The gauge itself, the market it is meant to describe, and the controls. SPHB and
# SPLV are the two halves of the S&P 500 sorted by beta and by volatility; their ratio is
# the most-watched risk-appetite proxy that does not require options data.
HIGH_BETA = "SPHB"
LOW_VOL = "SPLV"
MARKET = "SPY"
CONTROLS = ("IWM", "HYG", "LQD")     # small caps and the credit spread pair
CASH = "BIL"
TICKERS = (HIGH_BETA, LOW_VOL, MARKET) + CONTROLS + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call appetite.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 982) -> tuple[pd.DataFrame, dict]:
