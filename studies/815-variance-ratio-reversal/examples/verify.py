"""Reproducible headline run for Study 815 — Variance-Ratio Reversal.
...
print("# Variance-Ratio Reversal — do low-VR (mean-reverting) names offer a tradable reversal?")
...
W, Q = 120, 5
sp = st.vr_spreads(ret, window=W, q=Q, frac=0.3)
h = st.vr_stats(sp)
print(f"\nsort: trailing-{W}d VR(q={Q}), long bottom30% (low VR) / short top30% (high VR), "
      f"{h['n_days']} days (median {int(sp['n'].median())} names/day)")
print("# THE HEADLINE — long-low-VR / short-high-VR spread")
print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
      f"one-sample t = {h['t_1s']:+.2f}")
print(f"  books : low-VR {h['lo_bps']:+.2f} vs high-VR {h['hi_bps']:+.2f} bps  "
      f"(Welch t = {h['welch_t']:+.2f})")
sps = sp["spread"].to_numpy()
print(f"  gross spread Sharpe (no cost): {sps.mean() / sps.std(ddof=1) * np.sqrt(252):.2f}")

# cross-sectional VR snapshot — where do the names sit vs the random-walk line?
vr_last = st.trailing_vr(ret, window=W, q=Q).iloc[-1].dropna()
print(f"  VR(5) cross-section on last day: median {vr_last.median():.3f}, "
      f"{(vr_last < 1).mean() * 100:.0f}% below 1 (mean-reverting), "
      f"range [{vr_last.min():.3f}, {vr_last.max():.3f}]")

print("\n# PLACEBO — column-permute the forward returns (1,000 permutations)")
pl = st.placebo_pvalue(ret, window=W, q=Q, n_seeds=20, n_draws_per_seed=50)
...
if np.isfinite(pl["placebo_sd_bps"]) and pl["placebo_sd_bps"] > 0:
    print(f"  observed sits {pl['obs_bps'] / pl['placebo_sd_bps']:+.2f} sd from the null centre")
...
print("\n# ROBUSTNESS — alternate trailing windows")
for w in (63, 252):
    ts = st.vr_stats(st.vr_spreads(ret, window=w, q=Q, frac=0.3))
    print(f"  window={w}d: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-low-VR / short-high-VR, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=815 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0006, seed=815, n_assets=40, n_days=1600)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0006, seed 815): spread NW t = {sy['t_nw']:+.2f}, "
