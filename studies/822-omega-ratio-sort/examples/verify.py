"""Reproducible headline run for Study 822 — Omega-Ratio Sort.
...
print("# Omega-Ratio Sort — does a full gain/loss ratio beat plain trailing Sharpe?")
...
sp = st.omega_spreads(ret, lookback=252, skip=21, frac=0.3)
h = st.spread_stats(sp)
print(f"\nsort: trailing 12-1 Omega(0), long top30% / short bottom30%, "
      f"{h['n_days']} days (median {int(sp['n'].median())} names/day)")
print("# THE HEADLINE — long-high-Omega / short-low-Omega spread")
print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
      f"one-sample t = {h['t_1s']:+.2f}")
print(f"  books : high-Omega {h['hi_bps']:+.2f} vs low-Omega {h['lo_bps']:+.2f} bps  "
      f"(Welch t = {h['welch_t']:+.2f})")
print(f"  gross spread Sharpe (no cost): {h['gross_sharpe']:.2f}")

print("\n# THE HEAD-TO-HEAD — does Omega beat Sharpe? (same universe/dates/machinery)")
hh = st.head_to_head(ret, lookback=252, skip=21, frac=0.3)
print(f"  Omega  (long high / short low): {hh['omega_bps']:+.2f} bps  NW t = {hh['omega_t']:+.2f}")
print(f"  Sharpe (long high / short low): {hh['sharpe_bps']:+.2f} bps  NW t = {hh['sharpe_t']:+.2f}")
print(f"  low-vol(long low / short high): {hh['lowvol_bps']:+.2f} bps  NW t = {hh['lowvol_t']:+.2f}")
rc = st.signal_rank_corr(ret, lookback=252, skip=21)
print(f"  avg per-day rank corr  Omega~Sharpe = {rc['rho_omega_sharpe']:+.3f}  "
      f"Omega~(-vol) = {rc['rho_omega_negvol']:+.3f}")
...
print("\n# THE TIMER — long-high-Omega / short-low-Omega, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=822 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0016, seed=822, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0016, seed 822): spread NW t = {sy['t_nw']:+.2f}, "
