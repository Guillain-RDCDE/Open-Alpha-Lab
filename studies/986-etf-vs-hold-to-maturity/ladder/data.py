"""Data layer for Study 986 — The Rolling Ladder.

The rate series ``^TNX`` is quoted by Yahoo as ten times the yield in percent
(a print of 42.0 means 4.20%), and the data layer divides accordingly — a scaling error there
would silently multiply every duration calculation in this study by ten.
...
# Constant-maturity bond funds across the curve, plus cash and the equity market
# for scale. IEF and TLT roll; a bond does not.
SHORT = "SHY"           # 1-3 year Treasuries
INTERMEDIATE = "IEF"    # 7-10 year
LONG = "TLT"            # 20+ year
CORPORATE = "LQD"
CASH = "BIL"
EQUITY = "SPY"
YIELD_10Y = "^TNX"      # the 10-year yield, used as a rate series, not a holding
TICKERS = (SHORT, INTERMEDIATE, LONG, CORPORATE, CASH, EQUITY, YIELD_10Y)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call ladder.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 986) -> tuple[pd.DataFrame, dict]:
