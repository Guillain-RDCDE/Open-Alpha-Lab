"""Data layer for Study 1011 — The Half-Life of an Edge.

Fifty large-cap names and the market proxy. Signals are built cross-sectionally
from the names' own price histories, so the study needs breadth rather than exotic data — the
subject is the *decay profile* of a signal and what to do about it, which is a property of the
signal rather than of its ingredients.
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call halflife.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1011) -> tuple[pd.DataFrame, dict]:
