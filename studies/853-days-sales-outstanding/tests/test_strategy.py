...
the DSO long-short lights up *only when a red flag is planted* (low-buildup out-returns high)."""
...
from dso_signal import strategy as st  # noqa: E402
...
def test_calendar_ls_null_is_quiet(null_panel):
    ...
    assert abs(stt["t_nw"]) < 3.0            # no planted red flag -> no strong HAC t


def test_calendar_ls_recovers_planted_red_flag(planted_panel):
    prices, ev = planted_panel
    ls = st.calendar_ls(prices, ev, staleness_days=120)
    stt = st.calendar_ls_stats(ls)
    assert stt["mean_bps"] > 0               # long low-buildup / short high-buildup makes money
...
def test_hit_rate_ci_present(planted_panel):
    prices, ev = planted_panel
    stt = st.calendar_ls_stats(st.calendar_ls(prices, ev, staleness_days=120))
    assert stt["hit_lo"] <= stt["hit"] <= stt["hit_hi"]
...
# --------------------------------------------------------------------------- #
# warns-sales axis (channel-stuffing mechanism, expects NEGATIVE slope)
# --------------------------------------------------------------------------- #
def test_warns_sales_planted_negative_slope():
    import pandas as pd
    rng = np.random.default_rng(5)
    n = 400
    chg = rng.normal(0, 8, n)                        # DSO change in days
    fut = -0.01 * chg + rng.normal(0, 0.05, n)       # buildup truly precedes weaker sales
    ev = pd.DataFrame({"dso_yoy_chg": chg, "next_rev_yoy": fut})
    out = st.warns_sales(ev)
    assert out["slope"] < 0 and out["t"] < -5.0
    assert out["spread"] > 0                          # low-buildup tercile grows sales faster


def test_warns_sales_no_relation():
    import pandas as pd
    rng = np.random.default_rng(6)
    n = 400
    ev = pd.DataFrame({"dso_yoy_chg": rng.normal(0, 8, n),
                       "next_rev_yoy": rng.normal(0, 0.1, n)})
    out = st.warns_sales(ev)
