"""Reproducible headline run for Study 853 — Days-Sales-Outstanding Buildup.
...
from dso_signal import data, strategy as st  # noqa: E402

print("# Days-Sales-Outstanding Buildup — does rising DSO warn on future sales and returns?")
...
print("\n# PRIMARY — calendar-time tercile long-short on the DSO buildup")
print("#   (long the LOW-DSO-change third, short the HIGH-DSO-change third; earn next month; one lag)")
...
print(f"  Sharpe = {s['sharpe']:.2f}  hit = {s['hit']*100:.0f}% [{s['hit_lo']*100:.0f},{s['hit_hi']*100:.0f}]  "
      f"long {s['long_bps']:+.1f} / short {s['short_bps']:+.1f} bps/mo  turnover {s['avg_turnover']:.2f}")
ls120 = st.calendar_ls(px, ev, signal_col="dso_score", n_buckets=3, min_names=6, staleness_days=120)
...
print(f"  pct-change signal (dDSO/DSO): mean {s_pct['mean_bps']:+.1f} bps, "
      f"NW t = {s_pct['t_nw']:+.2f}, Sharpe {s_pct['sharpe']:.2f}")

print("\n# CROSS-CHECK — pooled event drift (tercile sort by signed score, 1-day-lag entry)")
...
print(f"  monotonicity (63d terciles high-buildup->low-buildup): {[f'{b*100:+.2f}%' for b in bm]}")

print("\n# ERA SPLIT — calendar long-short pre/post 2018")
for lbl, lo, hi in [("2010-2017", "2000-01-01", "2018-01-01"),
...
print("\n# THIRD AXIS — does DSO buildup WARN on future (next-quarter) sales? (expect slope < 0)")
q = st.warns_sales(ev)
print(f"  n={q['n']}  slope={q['slope']:+.4f}  t={q['t']:+.2f}  R2={q['r2']:.3f}  corr={q['corr']:+.3f}")
print(f"  future sales growth: low-buildup tercile {q['low_sales']*100:+.1f}%  "
      f"vs high-buildup {q['high_sales']*100:+.1f}%  ->  spread {q['spread']*100:+.1f} pp")
...
print("  planted red flag. Null checked over 12 seeds (never a single stream).")
null_ts = []
for s_ in range(12):
...
print(f"  planted edge=0.15 (seed 853): mean {pl['mean_bps']:+.1f} bps/mo, NW t = {pl['t_nw']:+.2f}")
