...
| Axis | Stamp | Why |
|---|---|---|
| **Signal** — is the carry premium real? | ![Weak](https://img.shields.io/badge/Weak-dab617?style=flat-square) | The literature says yes — UIP fails, so high-rate currencies earn a premium (LRV 2011) — but **this 23-year sample alone reads weak**: on the real G10 tape (2001–2024) the carry book earns Sharpe **+0.22** (high-minus-low **+3.0%/yr**) at only Lo *t* ≈ **1.0**, with a bootstrap 95% CI **[−0.17, +0.69]** that spans zero (14% of resamples negative) — under the desk's *t* ≥ 2 bar. The crash, at least, is unmistakable (skew **−0.70**, worst month **−10.6%**, Oct-2008). |
...
| **Combo diversifies the crash?** | ![Partial](https://img.shields.io/badge/Partial-dab617?style=flat-square) | **Mechanically yes, on Sharpe no.** On 2001–2024 the legs decorrelate (**+0.05**) so the combo *cushions* the steamroller (worst month **−10.6% → −6.4%**; in carry's worst 5 months **−7.6% → −3.0%**) — but **FX momentum lost money** this sample (Sharpe **−0.14**), so the combo **+0.06** can't beat carry **+0.22**. `PARTIAL`. |

> **In one sentence:** on the real 2001–2024 G10 tape the FX carry premium is thin rent for standing in front of a steamroller — and statistically `WEAK` on this sample alone (Sharpe **+0.22**, Lo *t* ≈ 1.0, CI spanning zero; the stamp leans on three decades of literature for existence; skew **−0.70**, worst month **−10.6%** in Oct-2008) — and the classic fix, the **carry⊕momentum combo**, *cushions* the crash exactly as designed (decorrelated legs **+0.05**; worst month **−10.6% → −6.4%**) but cannot lift the Sharpe because FX momentum itself decayed to **−0.14** over this sample, so the combo `PARTIAL`-ly delivers — it dulls the jump without ever pretending it's gone.
