"""Reproducible headline run for Study 476 — TD Sequential (DeMark 9-13).
...
from td_sequential import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch setup-vs-random test
    HAVE_SCIPY = True
...
print("# TD Sequential — mechanical buy setup-9 / countdown-13 on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled buy setup-9 vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'setup_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total buy setup-9 completions (H={st.HORIZONS[0]}): {total}")

    print("\n# Countdown-13 vs random baseline (pooled)")
    print(f"  {'H':>3} {'n':>5} {'cd_bps':>10} {'win%':>5} {'1samp_t':>8} {'rnd_bps':>8} {'delta':>7} {'welch_t':>8}")
    for h in st.HORIZONS:
        cc, rr = [], []
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.buy_countdown_entries(c, b["low"])
            re = st.random_entries(c, max(len(e), 50), seed=11)
            cc.append(st.forward_returns(c, e, h))
            rr.append(st.forward_returns(c, re, h))
        cc = np.concatenate(cc); rr = np.concatenate(rr)
        s = st.summarize(cc); sr = st.summarize(rr)
        wt = (stats.ttest_ind(cc, rr, equal_var=False)[0] if HAVE_SCIPY and cc.size else float("nan"))
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              f"{sr['mean_bps']:>8.1f} {s['mean_bps']-sr['mean_bps']:>7.1f} {wt:>8.2f}")

    print("\n# Per-ticker, H=20: setup-9 one-sample t and setup-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.buy_setup_entries(c)
        re = st.random_entries(c, max(len(e), 50), seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} setup={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Scrambled-lookback GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.scrambled_lookback_placebo(_load("SPY")["close"], 20, n_draws=500, seed=476)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => the 4-bar count not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.buy_setup_entries(c), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  setup={s['mean_bps']:+7.1f}bps  "
