"""Strategy + inference for Study 699 — Butterfly-Harmonic.

The claim: **the Butterfly harmonic predicts the reversal at point D.** Chartists
draw four alternating swing pivots X, A, B, C, then project a fifth point D such
that:

1. **AB retraces XA by 0.786** (the Butterfly's defining first ratio — deeper than
   Gartley's 0.618, shallower than nothing; it is the single number that tells a
   Butterfly apart from its siblings at a glance).
2. **BC retraces AB somewhere in 0.382-0.886** (a loose structural band shared by
   every XABCD pattern in the Carney taxonomy — not itself a hypothesis under
   test, just the geometry that keeps a candidate "well-formed").
3. **D extends 1.27-1.618x the ORIGINAL XA leg, BEYOND point X** — this is the
   Butterfly's real signature. Gartley/Bat/Crab all keep D somewhere *inside* the
   X-A range (a retracement); the Butterfly alone sends it *past* X. Believers
   read that overshoot as an exhaustion move: a fifth wave that "runs out of
   gas" exactly where a specific Fibonacci multiple says it should, and treat D
...
2. **XABCD candidates.** Every consecutive confirmed quadruple (X, A, B, C) whose
   AB/XA and BC/AB retracements sit inside the bands above, projecting
   **D = X - ext x (A - X)** with ``ext`` drawn from the 1.27-1.618 Butterfly band
   — a price target fully known the moment C confirms.
...
* **Forward returns at D vs a random-day BASE RATE, Bonferroni-corrected.** The
  ...
  directions as the real Butterfly touches — i.e. "what a random entry with the
  same directional tilt would have earned", the exact equity-drift confound a
  naive backtest would otherwise credit to the pattern. We test the pooled
  5-day headline *and* all six per-ticker splits (7 tests total) and apply a
...
* **A placebo arm (the third-axis myth-check).** Identical pivots, but each
  ...
  deterministic, seeded draw kept clear of the Butterfly's own bands — "would
  ANY extension-and-reversal projection have worked", not specifically the
  0.786 / 1.27-1.618 pair Carney trademarked.
"""
...
# --------------------------------------------------------------------------- #
# 2. XABCD candidates — Butterfly arm and its seeded placebo
# --------------------------------------------------------------------------- #
...
def find_butterfly(pivots: pd.DataFrame,
                   target_ab: float = 0.786, ab_tol: float = 0.06,
                   bc_lo: float = 0.382, bc_hi: float = 0.886,
                   target_ext: float = 1.444, ext_tol: float = 0.174,
                   placebo: bool = False, seed: int = 0) -> pd.DataFrame:
    """Scan consecutive confirmed pivot quadruples (X, A, B, C) for a Butterfly.

    Butterfly arm (``placebo=False``): a quadruple qualifies when (a) the AB
    retracement of XA is within ``ab_tol`` of ``target_ab`` (0.786) and (b) the BC
    retracement of AB lands in ``[bc_lo, bc_hi]`` (0.382-0.886, the shared XABCD
    structural band — not itself under test). The D projection extends the
    *original* XA leg past X: ``D = X - ext x (A - X)``, ``ext`` in the
    1.27-1.618 Butterfly band (``target_ext=1.444`` +/- ``ext_tol=0.174`` spans
    exactly that range). ``reversal_dir`` is +1 (expect a bounce UP off D) when
    XA rose (bullish Butterfly, D projects below X) and -1 otherwise.

    Placebo arm (``placebo=True``): identical pivots, but each candidate draws its
    *own* deterministic AB-retrace and D-extension targets, uniformly at random
    and kept away from the Butterfly bands under test — "would ANY
    extension-and-reversal projection have worked", not specifically 0.786 /
    1.27-1.618.
    """
    rows = []
    ...
    for k in range(len(P) - 3):
        ...
        if placebo:
            ...
            t_ab = float(rng.uniform(0.25, 0.72))
            while abs(t_ab - target_ab) < ab_tol:
                t_ab = float(rng.uniform(0.25, 0.72))
            t_ext = float(rng.uniform(0.85, 2.20))
            while abs(t_ext - target_ext) < ext_tol:
                t_ext = float(rng.uniform(0.85, 2.20))
        else:
            t_ab, t_ext = target_ab, target_ext

        if abs(retrace_ab - t_ab) > ab_tol:
            continue
        if not (bc_lo <= retrace_bc <= bc_hi):
            continue

        d_proj = float(X["price"] - t_ext * XA)         # D extends XA PAST point X
        ...
        rows.append({
            ...
            "ab_target": t_ab, "ext_target": t_ext,
            "D_proj": d_proj, "reversal_dir": reversal_dir,
        })
    cols = ["X_i", "A_i", "B_i", "C_i", "conf_i", "X_p", "A_p", "B_p", "C_p",
            "retrace_ab", "retrace_bc", "ab_target", "ext_target", "D_proj", "reversal_dir"]
...
def detect_and_scan(bars: pd.DataFrame, pct: float = 0.03, placebo: bool = False, seed: int = 0,
    ...
    cand = find_butterfly(piv, target_ab, ab_tol, bc_lo, bc_hi, target_ext, ext_tol,
...
def hac_mean_t(x: np.ndarray) -> float:
    ...
    time (overlapping Butterfly legs on the same tape)."""
...
def base_rate_ledger(bars: pd.DataFrame, n_events: int, dir_mix: float,
    ...
    (``dir_mix`` = share of ``reversal_dir == +1``) as the real Butterfly touches —
    the equity-drift confound a naive backtest would otherwise credit to the
    pattern. One draw per (ticker, seed); pool many seeds for the null distribution.
    """
    n = len(bars)
