"""Reproducible headline run for Study 699 — Butterfly-Harmonic.
...
from butterfly_harmonic import data, strategy as st  # noqa: E402
...
print("# Butterfly-Harmonic — does a 1.27-1.618 XA extension at D predict a reversal?")
...
def pooled(placebo: bool, cost: float = 0.0, seed: int = 699, horizons=HORIZONS):
...
print("\n# THE HEADLINE — Butterfly XABCD (AB retrace 0.786 +-0.06 of XA, BC retrace 0.382-0.886 "
      "of AB, D = X - ext*(A-X) with ext in [1.27, 1.618])")
fib_g, n_piv, n_cand_fib, fib_by_ticker = pooled(placebo=False, cost=0.0)
print(f"  pooled confirmed pivots: {n_piv:,}  |  Butterfly XABCD candidates: {n_cand_fib:,}  "
      f"|  D-touches: {len(fib_g):,}")
...
# --------------------------------------------------------------------------- #
print("\n# SIGNAL AXIS — Butterfly D-touch fade vs a random-day BASE RATE "
      f"(matched bullish/bearish mix, pooled over {BASE_SEEDS} seeds), Bonferroni-corrected")

dir_mix_all = float((fib_g["reversal_dir"] > 0).mean()) if not fib_g.empty else 0.5
...
base_g = base_rate_pool(fib_by_ticker)
tests = []  # (label, welch_t, n_fib, n_base)
for h in HORIZONS:
    sf = st.summarize(fib_g, f"ret_gross_{h}")
    sb = st.summarize(base_g, f"ret_gross_{h}")
    wt = st.welch_t(fib_g[f"ret_gross_{h}"], base_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: FIB mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
for t in data.BASKET:
    ...
    if n_ev == 0:
        ...
        continue
    mix = float((l_fib["reversal_dir"] > 0).mean())
    frames = [st.base_rate_ledger(bars[t], n_ev, mix, horizons=HORIZONS, cost_bps=0.0,
                                  seed=st._seed_from(699, t, s)) for s in range(BASE_SEEDS)]
    br_t = pd.concat(frames, ignore_index=True)
    sf = st.summarize(l_fib, "ret_gross_5")
    sb = st.summarize(br_t, "ret_gross_5")
    wt = st.welch_t(l_fib["ret_gross_5"], br_t["ret_gross_5"])
    tests.append((f"{t} (5-day)", wt))
    print(f"  {t:5s} FIB n={sf['n']:3d} mean={sf['mean_bps']:+7.2f}bps   |   "
...
for h in TIMER_HORIZONS:
    ...
    for c in (0.0, 5.0, 10.0):
        col = f"ret_gross_{h}" if c == 0.0 else None
...
# --------------------------------------------------------------------------- #
# Third axis (myth-check) — does the Butterfly beat a placebo extension zone?
# --------------------------------------------------------------------------- #
print("\n# THIRD AXIS (myth-check) — does the specific 0.786 / 1.27-1.618 pair beat "
      "a placebo extension-and-reversal projection built off the identical pivots?")
...
for h in HORIZONS:
    ...
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: BUTTERFLY mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
          f"PLACEBO mean={sp['mean_bps']:+7.2f}bps (n={sp['n']})   |   Welch t={wt:+.2f}{tag}")

n_beats = 0
print("\n  Per-instrument, 5-day, Butterfly vs placebo:")
for t in data.BASKET:
    l_fib = fib_by_ticker[t]
    ...
    n_beats += int(beats)
    print(f"  {t:5s} BUTTERFLY n={sf['n']:3d} mean={sf['mean_bps']:+7.2f}bps   |   "
          f"PLACEBO n={sp['n']:3d} mean={sp['mean_bps']:+7.2f}bps   |   "
          f"beats placebo: {'yes' if beats else 'no'}")
print(f"\n  Butterfly beats placebo on {n_beats}/{len(data.BASKET)} tickers (5-day)")
...
null_ts = []
for s_ in range(20):
    ledger = pooled_synth(0.0, 699 + s_)
...
print(f"  planted mean_rev=0.08 (seed 699): n={sp_planted['n']}  mean={sp_planted['mean_bps']:+.2f}bps  "
