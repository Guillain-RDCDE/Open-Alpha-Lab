"""Data layer for Study 967 — Window Shopping.

Sector SPDRs start on different dates (XLRE in 2015, XLC in 2018), so the panel is
deliberately ragged: a column is NaN before its own inception and every estimator here has to
cope with a cross-section whose width changes. Silently back-filling those columns would be
the single easiest way to fake a good answer.
...
# The eleven Select Sector SPDRs (a real cross-section a real portfolio holds), plus
# SPY as the market for the beta work and BIL as the cash leg.
SECTORS = ("XLB", "XLC", "XLE", "XLF", "XLI", "XLK", "XLP", "XLRE", "XLU", "XLV", "XLY")
MARKET = "SPY"
CASH = "BIL"
TICKERS = SECTORS + (MARKET, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call window_choice.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 967) -> tuple[pd.DataFrame, dict]:
