"""Data layer for Study 996 — The Palindrome Portfolio.

Nothing special is required of the data here — a long daily tape and a real trading
calendar. The point of the study is what happens when you search that calendar hard enough, so
the only property that matters is that the dates are genuine.
...
# A long single tape is all this needs; the extra names exist so the
# multiple-testing arithmetic has somewhere to grow.
EQUITY = "SPY"
NASDAQ = "QQQ"
SMALL = "IWM"
BONDS = "TLT"
...
EMERGING = "EEM"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, SMALL, BONDS, GOLD, EMERGING, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call palindrome.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 996) -> tuple[pd.DataFrame, dict]:
