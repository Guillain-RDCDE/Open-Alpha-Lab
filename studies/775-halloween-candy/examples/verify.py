"""Reproducible headline run for Study 775 — Halloween-Candy.

Prints every number quoted in docs/results.md and frozen into the ``R`` dict in
notebooks/build_notebooks.py. Deterministic; uses the cached HSY / SPY tapes under
...
print("# Halloween-Candy — is there a Hershey (HSY) Halloween seasonal run-up?")

print(f"calendar: {len(dt.EVENTS)} Halloweens {dt.EVENTS[0][0]}->{dt.EVENTS[-1][0]}, "
      f"hardcoded — Halloween is October 31 every year (fixed public holiday)")

if not dt.have_real():
    print("(cache miss — fetching HSY + SPY once)")
    dt.fetch()

prices = dt.load_real()
panel = pd.DataFrame({t: s for t, s in prices.items()}).dropna()
print(data_stamp("Halloween panel (HSY + SPY, adjusted/total-return)", panel, asof=dt.AS_OF))

ev = st.build_event_table(prices, cost_bps=5.0)
ev10 = st.build_event_table(prices, cost_bps=10.0)
inc, inc10 = ev[ev["included"]], ev10[ev10["included"]]
print(f"\nevents resolved: {len(inc)} of {len(dt.EVENTS)} Halloweens have HSY+SPY coverage "
      f"around the anchor")
for reason, n in ev[~ev["included"]]["reason"].value_counts().items():
    print(f"  excluded {n:2d}x {reason}")

print("\n# THE RUN-UP (the 'buy the candy season') — abnormal return, K sessions BEFORE "
      "Halloween, HSY minus SPY (gross)")
...
print("\n# THE FADE (the 'sell the season') — abnormal return, K sessions AFTER Halloween")
...
for base, label in (("pre_s", "2-week run-up"), ("pre_l", "1-month run-up")):
...
for bump in (0.01, 0.02):
    planted = st.synthetic_detect(bump=bump, seed=781, k=10)
    print(f"  planted run-up bump=+{bump*100:.1f}% (seed 781): mean AR {planted['mean']*100:+.3f}%  "
