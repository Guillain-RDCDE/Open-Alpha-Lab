"""Strategy + inference for Study 775 — Halloween-Candy.

The claim: **Hershey (HSY) enjoys a Halloween seasonal run-up.** Two windows, both
anchored on the fixed Halloween date (October 31, known decades ahead):

* **The run-up (the "buy the candy season").** HSY's abnormal return (HSY total-return
  minus the SPY total-return benchmark) over the K trading sessions ENDING on the last
  close on/before Oct 31. Tested at K = 10 (~2 weeks) and K = 21 (~1 month).

* **The fade (the "sell the season").** HSY's abnormal return over the K sessions STARTING
  from the Halloween-day close. Tested at K = 10 and K = 21. If the folklore is right the
  run-up is positive and the fade is negative; if both are noise, the whole "buy the candy
  season, sell after Halloween" story is busted.

Because each Halloween year is a single independent event (not a daily series), the primary
statistic is a **one-sample t** of the abnormal return across events (n = the number of
years with HSY+SPY coverage). A random-window placebo (drawing many random, non-Halloween
K-session windows from HSY's own history vs SPY) checks whether the observed mean sits
...
def build_event_table(prices: dict[str, pd.Series], cost_bps: float = COST_BPS
    ...
    """One row per Halloween year: run-up + post-holiday abnormal returns + inclusion.

    A row is INCLUDED only if HSY and SPY both have cached history covering
    ...
    for year, keydate in dt.EVENTS:
        row = dict(year=year, halloween=keydate)
        anchor_ts = pd.Timestamp(keydate)
        on_or_before = common[common <= anchor_ts]
        if len(on_or_before) == 0:
            row.update(included=False, reason="no HSY/SPY coverage at the anchor")
            rows.append(row)
            continue
        ...
        def ar(i_start, i_end):
            r_a = hsy.loc[common[i_end]] / hsy.loc[common[i_start]] - 1.0
...
def placebo_pvalue(events: pd.DataFrame, prices: dict[str, pd.Series], col: str,
                   ...
                   n_seeds: int = 20, n_draws_per_seed: int = 200, base_seed: int = 781,
                   tail: str = "right") -> dict:
    """For each INCLUDED event, redraw a random (non-Halloween) k-session window on HSY vs
    ...
    for s in range(n_seeds):
        ...
        for _ in range(n_draws_per_seed):
            ...
            for _e in range(n_events):
                ...
                r_a = hsy.loc[d_end] / hsy.loc[d_start] - 1.0
...
def car_path(events: pd.DataFrame, prices: dict[str, pd.Series],
    ...
    """Mean cumulative abnormal return (HSY - SPY) at each offset from -pre..+post
    relative to Halloween, normalised so offset 0 = 0%, averaged across all INCLUDED
    ...
    for _, row in inc.iterrows():
        ...
        base_a, base_s = hsy.loc[common[p]], spy.loc[common[p]]
        vals = []
        for o in offsets:
            d = common[p + o]
            r_a = hsy.loc[d] / base_a - 1.0
...
def synthetic_detect(bump: float, seed: int, k: int = PRE_SHORT_K,
    ...
    pre-Halloween bump. ``side='pre'`` measures the run-up window [p-k..p); ``side='post'``
    measures [p..p+k)."""
