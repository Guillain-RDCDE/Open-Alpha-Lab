"""Reproducible headline run for Study 480 — Darvas Box.
...
from darvas_box import data, strategy as st  # noqa: E402
...
print("# Darvas box — mechanical breakout on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled box breakout vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.breakout_entries(b, lookback=LB, min_box=MIN_BOX)
            re = st.random_entries(c, max(len(e), 50), lookback=LB, seed=7)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total box breakouts (H={st.HORIZONS[0]}): {total}")
    ...
    print("\n# Shuffled-box GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_box_placebo(_load("SPY"), 20, lookback=LB, min_box=MIN_BOX, n_draws=500, seed=480)
...
for edge in (0.0, 0.3):
    ...
    c = px["close"]
    e = st.breakout_entries(px, lookback=LB, min_box=MIN_BOX)
    s = st.summarize(st.forward_returns(c, e, 20))
