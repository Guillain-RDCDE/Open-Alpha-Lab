"""Data layer for Study 989 — The One-Way Beta.

Crypto trades 365 days a year and every annualisation here uses that calendar. The
sample starts in November 2017 because Yahoo's coverage of the smaller majors before then is
patchy, and a beta estimated on a series with silent gaps is a beta estimated on nothing.
...
# Bitcoin as the market, and the majors with enough history to say anything.
# Yahoo's crypto coverage before late 2017 is thin, which is why the sample starts there.
BENCHMARK = "BTC-USD"
ALTS = ("ETH-USD", "XRP-USD", "LTC-USD", "ADA-USD", "XLM-USD", "DOGE-USD")
EQUITY = "SPY"          # the same test on a market where the answer is known
CASH = "BIL"
TICKERS = (BENCHMARK,) + ALTS + (EQUITY, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call onewaybeta.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 989) -> tuple[pd.DataFrame, dict]:
