"""Tests for the strategy layer of Study 287 (Easter-Effect).
...
def _synth(premium_bps=0.0, seed=287, start=1900, end=2150):
    ...
    unc = df["ret"]
    # index the event series by year so subperiod_stats works
    ev.index = pd.Index(ev.index.year, name="year")
    return ev, unc
...
def test_event_stats_keys():
    ...
    required = {
        "n", "ev_mean", "ev_std", "uncond_mean", "uncond_std",
...
def test_null_has_no_signal():
    ...
    ev, unc = _synth(premium_bps=0.0, seed=287)
    r = st.event_stats(ev, unc, n_permutations=1000, seed=287)
...
def test_planted_premium_detectable():
    ...
    ev, unc = _synth(premium_bps=50.0, seed=287)
    r = st.event_stats(ev, unc, n_permutations=1000, seed=287)
...
def test_subperiod_stats_structure():
    ...
    sp = st.subperiod_stats(ev, unc, breaks=((1950, 1987), (1988, 2025)))
    assert list(sp.columns) == ["period", "n", "mean_bps", "excess_bps", "hac_t"]
    assert int(sp["n"].sum()) == 76


def test_subperiod_stats_null_quiet():
    """On the null tape, no sub-period should show a strong HAC t."""
    ev, unc = _synth(premium_bps=0.0, seed=287, start=1950, end=2025)
    sp = st.subperiod_stats(ev, unc, breaks=((1950, 1987), (1988, 2025)))
    assert (sp["hac_t"].abs() < 3.0).all()
...
def test_summarize_keys():
    ...
    required = {
        "n", "ev_mean_pct", "uncond_mean_pct", "excess_mean_bps",
