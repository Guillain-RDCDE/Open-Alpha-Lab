"""Data layer for Study 979 — The Prior Is the Portfolio.

Daily total-return closes. **No market-capitalisation data is used anywhere**, and
that is a deliberate design choice rather than a limitation: the textbook Black-Litterman prior
is the cap-weighted market portfolio, but the model's algebra does not care where the prior
came from, and pretending to know sector market caps from a price feed would be inventing
data. Three defensible priors are run instead, and the sensitivity to that choice is one of the
study's results.
...
# Ten multi-asset sleeves and the eleven sector SPDRs. Black-Litterman is an
# asset-allocation model, so the multi-asset panel is the headline; sectors are the check.
MULTI = ("SPY", "IWM", "EFA", "EEM", "TLT", "IEF", "LQD", "HYG", "GLD", "DBC")
SECTORS = ("XLB", "XLC", "XLE", "XLF", "XLI", "XLK", "XLP", "XLRE", "XLU", "XLV", "XLY")
CASH = "BIL"
TICKERS = MULTI + SECTORS + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call bl_prior.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 979) -> tuple[pd.DataFrame, dict]:
