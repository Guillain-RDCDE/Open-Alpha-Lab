"""Data layer for Study 991 — The Slow Bell.

Aggregating to a 250-day horizon from 8,000 daily observations leaves only 32
non-overlapping windows. Every statistic at long horizons is computed on a sample that small,
and the study reports overlapping and non-overlapping versions side by side because the
overlapping one looks far more precise than it is.
...
# A long equity history, plus assets whose tails converge at very different speeds.
EQUITY = "SPY"
NASDAQ = "QQQ"
SMALL = "IWM"
BONDS = "TLT"
...
CRYPTO = "BTC-USD"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, SMALL, BONDS, GOLD, EMERGING, CRYPTO, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call slowbell.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 991) -> tuple[pd.DataFrame, dict]:
