"""Data layer for Study 969 — Log or Simple.

A wide volatility range is the point of this universe: the gap between a log return
and a simple return is second-order in the return itself, so at 6% annualised volatility it is
invisible and at 80% it is the largest number on the page.
...
# Volatility is the whole story here, so the universe is chosen to span it: bills,
# bonds, equities, emerging markets, gold, a 3x leveraged fund and bitcoin.
TICKERS = ("BIL", "TLT", "SPY", "QQQ", "EEM", "GLD", "TQQQ", "BTC-USD")
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call log_vs_simple.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 969) -> tuple[pd.DataFrame, dict]:
