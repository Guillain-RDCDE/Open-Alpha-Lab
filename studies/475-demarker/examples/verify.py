"""Reproducible headline run for Study 475 — DeMarker.
...
from demarker import data, strategy as st  # noqa: E402
...
print("# DeMarker — mechanical oversold-rising long on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled DeMarker oversold-rising vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.oversold_rising_entries(b["high"], b["low"], period=PERIOD)
            re = st.random_entries(c, max(len(e), 50), period=PERIOD, seed=7)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total DeMarker oversold-rising entries (H={st.HORIZONS[0]}): {total}")
    ...
    print("\n# Phase-scramble TIMING placebo (SPY, H=20, 500 draws)")
    b = _load("SPY")
    pl = st.phase_scramble_placebo(b["high"], b["low"], b["close"], 20, period=PERIOD, n_draws=500, seed=475)
...
print("  edge=0 must NOT reach significance; planted rebound must light up.")
for edge in (0.0, 0.80):
    px, _ = data.synthetic_panel(edge=edge, seed=475, n_days=4000)
    e = st.oversold_rising_entries(px["high"], px["low"], period=PERIOD)
