"""Data layer for Study 408 — the Three Black Crows candlestick pattern.
...
* **Synthetic.** ``synthetic_panel`` — a deterministic, fixed-seed generator with a
  **planted-edge knob**. With ``edge = 0`` the post-pattern path is pure noise (a fair
  ...
  so that the day(s) *after* a real three-black-crows keep falling — the positive control
  proves the harness *can* bank a planted continuation/crash.
...
def _cache_path(ticker: str, cache_dir: str) -> str:
    ...
    return os.path.join(cache_dir, f"tbc_{safe}_1d.parquet")
...
def synthetic_panel(n_names: int = 30, n_days: int = 2200, edge: float = 0.0,
                    seed: int = 408, daily_vol: float = 0.013,
    ...
    high/low. After each bar we test whether the last three bars formed a textbook
    **three black crows** (see ``strategy.is_three_black_crows``); if ``edge != 0`` we add
    an *extra* drift of ``-edge`` to the **next** day's return — i.e. a real day-after
    ...
    for k in range(n_names):
        ...
        # a small overnight gap so the open can fall *inside* the prior body (the textbook
        # crow opens within the prior real body — a perfectly continuous open==prev_close
        # tape can never form the pattern, so we model a realistic overnight return)
        ...
        prev_close = 100.0
        pending = 0      # -1 if a three-black-crows just formed (applies to next bar)
        for i in range(n_days):
            ...
            hi[i] = max(o, c) + w
            lo[i] = min(o, c) - w
            # detect three black crows on bars (i-2, i-1, i) -> set pending for i+1
            pending = 0
            if i >= 2:
                trip = _tbc_from_arrays(open_, close, hi, lo, i)
                if trip == -1:
                    pending = -1
            prev_close = c
...
def _tbc_from_arrays(o: np.ndarray, c: np.ndarray, h: np.ndarray, l: np.ndarray,
                     i: int) -> int:
    """Inline three-black-crows test on bars (i-2, i-1, i); -1 if present else 0.

    Mirrors ``strategy.is_three_black_crows`` so the synthetic generator stays causal and
    self-contained (no circular import at module build time).
    """
    o0, o1, o2 = o[i - 2], o[i - 1], o[i]
    c0, c1, c2 = c[i - 2], c[i - 1], c[i]
    # three consecutive red bodies, each closing lower
    if not (c0 < o0 and c1 < o1 and c2 < o2):
        return 0
    if not (c2 < c1 < c0):
        return 0
    # each opens within the prior real body (the textbook "crow" stacking)
    if not (o0 >= o1 >= o2):
        return 0
