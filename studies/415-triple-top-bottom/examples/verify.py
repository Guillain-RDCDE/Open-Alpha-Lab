"""Reproducible headline run for Study 415 — Triple Top & Bottom.
...
print("# Triple Top & Bottom — objective three-tap detector on SPY + 29 large-caps (yfinance daily OHLC)")
if data.have_real():
    ...
    bt = st.collect_breakouts(panel, side="top")
    tot = sum(len(v) for v in bk.values())
    tott = sum(len(v) for v in bt.values())
    print(f"breakouts      : {tot} confirmed triple-BOTTOMS (SPY alone: {len(bk['SPY'])}); "
          f"{tott} triple-TOPS (myth check, SPY: {len(bt['SPY'])})")

    print("\n# TRIPLE BOTTOM (bullish, long) forward edge — EXCESS over each name's base rate")
    ...
    for h in st.HORIZONS:
        r = st.run_experiment(panel, horizon=h, side="bottom", n_draws=NDRAWS)
    ...
    print("\n# Myth check — TRIPLE TOP (bearish, short): does the figure reverse symmetrically?")
    for h in st.HORIZONS:
        r = st.run_experiment(panel, horizon=h, side="top", n_draws=NDRAWS)
        print(f"  TOP H={h:>2}: n={r['n_events']:>3}  excess={r['mean']*100:+.3f}%  "
              f"raw={r['raw_mean']*100:+.3f}%  win={r['win']*100:.0f}%  t={r['t']:+.2f}  "
              f"p={r['p_placebo']:.3f}")

    print("\n# Robustness — detector strictness (level_tol, min_bounce) at H=5")
    for lt, mb in [(0.03, 0.07), (0.04, 0.05), (0.06, 0.04)]:
        r = st.run_experiment(panel, horizon=5, side="bottom", n_draws=3000,
                              level_tol=lt, min_bounce=mb)
        print(f"  level_tol={lt:.2f} min_bounce={mb:.2f}: n={r['n_events']:>4}  "
              f"excess={r['mean']*100:+.3f}%  t={r['t']:+.2f}  p={r['p_placebo']:.3f}")

    print("\n# SPY-only triple bottom (the README hook) — beats SPY buy-and-hold?")
    for h in st.HORIZONS:
        r = st.run_experiment(panel, horizon=h, names=["SPY"], side="bottom", n_draws=NDRAWS)
        print(f"  SPY H={h:>2}: n={r['n_events']:>2}  excess={r['mean']*100:+.3f}%  "
              f"raw={r['raw_mean']*100:+.3f}%  t={r['t']:+.2f}  p={r['p_placebo']:.3f}")
else:
    print("(no _cache/triple_close.parquet — run data.fetch_panel() once to build the cache)")
...
for edge in (0.0, 0.20):
    panel, truth = data.synthetic_panel(edge=edge, seed=415, n_planted=8, daily_vol=0.011)
    r = st.run_experiment(panel, horizon=20, side="bottom", n_draws=1500)
