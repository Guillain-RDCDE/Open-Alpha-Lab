"""Data layer for Study 997 — The Rebalance Lottery.

Twenty years of daily closes for ten liquid ETFs. Nothing exotic is required — the
whole point is that a completely ordinary rule, on completely ordinary data, produces a wide
range of outcomes for reasons that have nothing to do with the rule.
...
# A plain 60/40 and a momentum sleeve — the two rule types where timing luck is
# largest, and a few extras so the momentum rank has something to choose between.
EQUITY = "SPY"
BONDS = "TLT"
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call lottery.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 997) -> tuple[pd.DataFrame, dict]:
