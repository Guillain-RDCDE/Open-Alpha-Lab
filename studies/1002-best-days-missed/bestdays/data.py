"""Data layer for Study 1002 — The Ten Best Days.

Total-return daily closes across five equity markets plus bonds and cash. The
statistic being examined is quoted for the S&P 500 almost exclusively; running it on six markets
at once is the cheapest available test of whether it is a fact about markets or a fact about one
sample.
...
BONDS = "TLT"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, SMALL, EAFE, EM, BONDS, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call bestdays.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1002) -> tuple[pd.DataFrame, dict]:
