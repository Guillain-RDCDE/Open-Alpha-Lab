"""Real-tape verification — Study 244 (Asset-Growth). Regenerates docs/results.md numbers.

Reads the EDGAR cache, computes the asset-growth signal, runs quintile sorts and the
...
— reads from the shared ``_cache/`` parquet files.
    ...
    python studies/244-asset-growth/examples/verify.py
"""
...
from asset_growth import data, strategy as st  # noqa: E402


def main() -> None:
    sig, fwd = data.fetch_panel()
    if sig.empty:
        print("ERROR: EDGAR cache not found. Place _edgar_Assets.parquet in _cache/.")
        sys.exit(1)

    print(f"Panel: {len(sig.columns)} tickers, {len(sig)} years "
          f"({sig.index.min()}–{sig.index.max()})")
    print(f"Fingerprint (AG): {data.fingerprint(sig)}")
    print()

    h = st.quintile_returns(sig, fwd, q=0.20)

    print("=== quintile returns (AG low->high, forward 1yr) ===")
    for q in ["q1", "q2", "q3", "q4", "q5", "market"]:
        s = st.summary(h[q])
        label = "low-AG" if q == "q1" else ("high-AG" if q == "q5" else
    ...
    print()
    print("=== long low-AG / short high-AG hedge ===")
    ...
    print(f"  Low-AG (q1) excess: mean={lo_excess['mean']*100:+.1f}%/yr  "
          f"HAC t={lo_excess['tstat']:+.2f}")
    print(f"  High-AG (q5) excess: mean={hi_excess['mean']*100:+.1f}%/yr  "
