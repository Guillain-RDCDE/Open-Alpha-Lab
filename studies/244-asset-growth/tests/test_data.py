...
_EDGAR_ASSETS = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "_cache", "_edgar_Assets.parquet"
)
_EDGAR_YRRET = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "_cache", "_edgar_yrret.parquet"
)
...
def test_premium_creates_ag_spread():
    """A negative premium plants a real high-AG underperformance."""
    from asset_growth import strategy as st
    sig_null, fwd_null, _ = data.synthetic_panel(n_firms=300, n_years=25, premium=0.0, seed=244)
    sig_live, fwd_live, _ = data.synthetic_panel(n_firms=300, n_years=25, premium=-0.12, seed=244)
    h_null = st.quintile_returns(sig_null, fwd_null, q=0.20)
    h_live = st.quintile_returns(sig_live, fwd_live, q=0.20)
    # With planted premium: low-AG minus high-AG hedge should be positive
