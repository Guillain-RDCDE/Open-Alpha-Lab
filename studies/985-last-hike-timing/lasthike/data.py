"""Data layer for Study 985 — The Last Hike.

The policy-rate path is **hard-coded** in ``strategy.FOMC_MOVES`` from the Fed's own
published record rather than fetched. Target-rate changes are public, dated, and never revised;
a vendor series adds a dependency and a revision risk for no gain. The market data is the usual
cached daily tape.
...
# What a "buy the last hike" investor is choosing between: stocks, long bonds,
# gold, and cash. The policy rate itself comes from the rate series below.
EQUITY = "^GSPC"
BONDS = "^TNX"          # 10-year yield, used as a rate series, not a holding
LONG_BOND = "TLT"
GOLD = "GLD"
SMALL = "^RUT"
TICKERS = (EQUITY, BONDS, LONG_BOND, GOLD, SMALL)

# The policy-rate history is hard-coded rather than fetched: the Fed's target changes are
# public record, they never revise, and no data vendor is needed to state them.
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call lasthike.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 985) -> tuple[pd.DataFrame, dict]:
