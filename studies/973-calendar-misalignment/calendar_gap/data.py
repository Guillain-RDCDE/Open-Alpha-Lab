"""Data layer for Study 973 — Different Holidays.

Every ticker here is a New-York-listed ETF, so all five close at 16:00 in the same
timezone on the same US calendar. That is deliberate: it removes the *quote*-time mismatch and
leaves only the mismatch that cannot be removed — the hours during which the **underlying**
market was open.
...
# Five markets whose holidays genuinely differ, all quoted in New York (so the *quote*
# times are synchronous even where the *underlying* markets are not — the distinction the
# whole study turns on), plus a same-timezone control.
US = "SPY"
FOREIGN = ("EWJ", "EWU", "EWG", "FXI")
CONTROL = "IWM"        # a US tape: same market, same calendar -- the null comparison
TICKERS = (US, CONTROL) + FOREIGN
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call calendar_gap.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 973) -> tuple[pd.DataFrame, dict]:
