"""Data layer for Study 992 — How Long Is a Storm?.

Every half-life below is quoted in **trading days of the asset's own calendar**.
Crypto trades 365 days a year and equities 252, so a "20-day half-life" means something 45%
longer in wall-clock time for Bitcoin — a conversion the study makes explicit rather than
...
# Assets whose storms are known to last very different lengths.
EQUITY = "SPY"
NASDAQ = "QQQ"
SMALL = "IWM"
BONDS = "TLT"
...
CURRENCY = "FXE"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, SMALL, BONDS, GOLD, CRYPTO, CURRENCY, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call storm.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 992) -> tuple[pd.DataFrame, dict]:
