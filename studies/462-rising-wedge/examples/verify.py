"""Reproducible headline run for Study 462 — Rising Wedge.
...
The rising-wedge rule is bearish: a SHORT fires on the first close below the rising support of
an active rising wedge, entered at the next close. A short's forward return is the *negative* of
the underlying move (a price fall is a gain).
...
from rising_wedge import data, strategy as st  # noqa: E402
...
print("# Rising wedge — mechanical support-break SHORT on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled wedge-break SHORT vs drift-matched random baseline (short returns)")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            c = _load(t)["close"]
            e = st.wedge_break_entries(c, k=K)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total wedge-break shorts (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: break-down short one-sample t and break-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.wedge_break_entries(c, k=K)
        re = st.random_entries(c, max(len(e), 50), k=K, seed=7)
    ...
    print("\n# Slope-scramble GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.slope_scramble_placebo(_load("SPY")["close"], 20, k=K, n_draws=500, seed=462)
...
print("  edge=0 must NOT reach significance; planted break-down must light up.")
for edge in (0.0, 0.50):
    px, _ = data.synthetic_panel(edge=edge, seed=462, n_days=4000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.wedge_break_entries(c, k=K), 20))
