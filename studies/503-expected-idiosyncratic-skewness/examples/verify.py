"""Reproducible headline run for Study 503 — Expected Idiosyncratic Skewness.
...
print("# Expected idiosyncratic skewness — quintile sort on a fixed S&P-100-style large-cap basket")
if data.have_real():
    ...
    print("\n# Idio-skew quintiles — next-month return, equal-weight (Q1 = LOW skew … Q5 = HIGH skew)")
    ...
    print("\n# The trade — long LOW-skew (Q1), short HIGH-skew (Q5), monthly")
...
print("  edge=0 must NOT manufacture significance; a planted skew penalty must light up.")
for edge in (0.0, 0.004):
    syn = data.synthetic_panel(edge=edge, seed=503)
