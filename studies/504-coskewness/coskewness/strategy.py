"""Strategy + inference for Study 504 — Coskewness.

The claim (Harvey & Siddique, 2000, *Conditional Skewness in Asset Pricing Tests*): a stock's
**coskewness** with the market — its contribution to the *market's* own skewness — is a priced,
systematic risk. Names with **low (more negative)** coskewness fall hardest precisely when the
market is most volatile / falling, so they are *bad* hedges and must offer a **premium**;
high-coskewness names behave like insurance and earn less. We measure coskewness as Harvey-
Siddique's direct standardised statistic over a trailing 12-month daily window and trade **long
the low-coskewness quintile, short the high-coskewness quintile**, rebalanced monthly.

This module:

  * sorts the monthly coskewness panel into **quintiles** each month and earns each quintile's
    *next-month* return (the panel is already lagged: coskewness at month-end pairs with month
    t+1's return — one execution lag, documented);
  * forms the **long-short** spread Q1 (low coskew) − Q5 (high coskew);
  * tests its mean with a **Newey-West (HAC) t-stat** and a **placebo / sign-flip null** (the
    ...
    **one-way costs × turnover** plus a **short borrow** on the high-coskew leg.

The decisive object is the HAC t of the long-short on the *real* tape: literature support for
the CRSP cross-sectional factor does not certify a tradable large-cap spread (the desk's t ≥ 2
...
def quintile_returns(panel: dict, n_q: int = 5) -> pd.DataFrame:
    """Equal-weight next-month return of each coskewness quintile, month by month.

    For each month with at least ``2*n_q`` valid (coskewness, fwd_ret) pairs, rank names by
    coskewness, split into ``n_q`` quantiles (Q1 = lowest coskew … Qn = highest coskew), and
    average each quantile's forward return. Returns a DataFrame indexed by month with columns
    ``Q1..Qn``.
    """
    ck = panel["coskew"]
    fwd = panel["fwd_ret"]
    ...
    for dt in ck.index:
        s = ck.loc[dt]
        f = fwd.loc[dt]
        both = pd.concat([s.rename("coskew"), f.rename("fwd")], axis=1).dropna()
        if len(both) < 2 * n_q:
            continue
        try:
            q = pd.qcut(both["coskew"].rank(method="first"), n_q, labels=False)
        except ValueError:
            continue
...
def long_short(qret: pd.DataFrame, low: str = "Q1", high: str = "Q5",
    ...
    """Long low-coskew (``low``) / short high-coskew (``high``) monthly spread, net of frictions.

    Gross spread is ``qret[low] - qret[high]`` (buy the names that pay the systematic-skew
    premium, short the insurance-like names). Costs: a one-way ``cost_bps`` × ``turnover`` charge
    is applied to *each* of the two legs every rebalance (so a full two-sided round-trip is
    ``2 * cost_bps * turnover`` per month, the conservative monthly-rebalance case).
    ``borrow_ann_bps`` charges short financing on the high-coskew leg (annual, pro-rated monthly).
    ``turnover`` defaults to 1.0 (full monthly turnover — quintile membership churns for a monthly
    signal).
    """
...
def placebo_pvalue(spread: pd.Series, n_draws: int = 20_000, seed: int = 504) -> dict:
    ...
    low-coskew beats high-coskew, so the edge should be *positive*). Returns the observed mean,
    the placebo mean (~0), and the empirical p-value.
    """
