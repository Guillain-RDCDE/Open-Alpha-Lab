"""Reproducible headline run for Study 463 — Bear-Flag.
...
from bear_flag import data, strategy as st  # noqa: E402
...
print("# Bear-flag — mechanical breakdown-short on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled breakdown-short vs drift-matched random baseline (SHORT side)")
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>9.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total bear-flag breakdowns (H={st.HORIZONS[0]}): {total}")
    ...
    print("\n# Shuffled-flag GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_flag_placebo(_load("SPY")["close"], 20, n_draws=500, seed=463)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => flag geometry not load-bearing)")
else:
...
for edge in (0.0, 0.80):
    ...
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.breakdown_entries(c), 20))
