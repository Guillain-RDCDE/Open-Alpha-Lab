"""Reproducible headline run for Study 812 — Corwin-Schultz Spread.
...
print("# Corwin-Schultz Spread — do high-estimated-spread (illiquid) names earn a premium?")
...
# Cross-sectional CS spread magnitudes (a sanity readout of the estimator itself).
S = st.daily_cs_spread(panel)
print(f"[spread] median daily CS spread across names ~ {np.nanmedian(S.to_numpy())*1e4:.1f} bps "
      f"(cross-name range {np.nanmedian(S, axis=0).min()*1e4:.1f}-{np.nanmedian(S, axis=0).max()*1e4:.1f} bps)")

for w in (21,):
    sp = st.cs_spreads(panel, window=w, frac=0.3)
    h = st.cs_stats(sp)
    print(f"\nsort: trailing-{w}d Corwin-Schultz spread, long top30% (illiquid) / short "
          f"bottom30% (liquid), {h['n_days']} days (median {int(sp['n'].median())} names/day)")
    print("# THE HEADLINE — long-high-spread / short-low-spread")
    print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
          f"one-sample t = {h['t_1s']:+.2f}")
    print(f"  books : high-spread {h['long_bps']:+.2f} vs low-spread {h['short_bps']:+.2f} bps  "
          f"(Welch t = {h['welch_t']:+.2f})")
    sps = sp["spread"].to_numpy()
    print(f"  gross spread Sharpe (no cost): "
          f"{sps.mean() / sps.std(ddof=1) * np.sqrt(252):.2f}")

sp = st.cs_spreads(panel, window=21, frac=0.3)
...
sig_units = (pl['obs_bps'] - pl['placebo_mean_bps']) / pl['placebo_sd_bps']
print(f"  observed is {sig_units:+.2f} sigma from the permutation null centre")
...
print("\n# THE TIMER — long-high-spread / short-low-spread, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=812 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.08, seed=812, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.08, seed 812): spread NW t = {sy['t_nw']:+.2f}, "
