"""Tests for the data layer (curated SOPR table, synthetic series, cache guard)."""
...
# Curated SOPR series
# ---------------------------------------------------------------------------
def test_sopr_series_monotone_index():
    sp = data.sopr_series()
    assert sp.index.is_monotonic_increasing
    assert len(sp) > 100, "Expected >100 monthly SOPR observations"


def test_sopr_series_month_end():
    sp = data.sopr_series()
    for ts in sp.index[:6]:
        assert ts == ts + pd.offsets.MonthEnd(0), f"{ts} is not a month-end date"


def test_sopr_series_oscillates_around_one():
    sp = data.sopr_series()
    assert (sp > 0).all(), "SOPR must be strictly positive"
    # Regime band: at least one capitulation month under 1 and one greed month above 1
    assert sp.min() < 0.98, "Expected a capitulation month (SOPR < 0.98)"
    assert sp.max() > 1.02, "Expected a greed month (SOPR > 1.02)"
    # SOPR is a tight ratio around 1 -- sanity-bound it
    assert 0.80 < sp.min() and sp.max() < 1.20
...
def test_synthetic_series_shape():
    df, truth = data.synthetic_series(n_months=120, beta=2.0, seed=1)
    assert df.shape == (120, 2)
    assert list(df.columns) == ["sopr", "price"]
    assert truth["n_months"] == 120


def test_synthetic_series_positive():
    df, _ = data.synthetic_series(n_months=120, beta=2.0, seed=2)
    assert (df["sopr"] > 0).all()
    assert (df["price"] > 0).all()


def test_synthetic_series_deterministic():
    a, _ = data.synthetic_series(n_months=120, beta=2.0, seed=42)
    b, _ = data.synthetic_series(n_months=120, beta=2.0, seed=42)
...
def test_fingerprint_length():
    sp = data.sopr_series()
    fp = data.fingerprint(sp)
    assert isinstance(fp, str) and len(fp) == 12


def test_fingerprint_changes_with_data():
    a, _ = data.synthetic_series(n_months=60, beta=0.5, seed=7)
    b, _ = data.synthetic_series(n_months=60, beta=3.0, seed=7)
