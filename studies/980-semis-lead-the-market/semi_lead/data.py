"""Data layer for Study 980 — The Silicon Canary.

Two semiconductor ETFs rather than one: SMH and SOXX track different indices with
different weighting rules, and a leading-indicator claim that holds for one and not the other
is a statement about an index committee, not about the economy.
...
# The canary, its cousins, and the birds it is supposed to warn. XLK is in the list
# because "semis lead" is often really "tech leads", and a study that cannot separate the
# two is measuring a sector rotation, not a leading indicator.
CANARY = "SMH"
PEERS = ("SOXX", "XLK")
MARKET = "SPY"
OTHERS = ("QQQ", "IWM")
CASH = "BIL"
TICKERS = (CANARY,) + PEERS + (MARKET,) + OTHERS + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call semi_lead.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 980) -> tuple[pd.DataFrame, dict]:
