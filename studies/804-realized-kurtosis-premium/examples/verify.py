"""Reproducible headline run for Study 804 — Realized-Kurtosis Premium.
...
print("# Realized-Kurtosis Premium — do fat-tailed names earn a cross-sectional premium?")
...
for w in (21,):
    sp = st.kurt_spreads(ret, window=w, frac=0.3)
    h = st.kurt_stats(sp)
    print(f"\nsort: trailing-{w}d realized kurtosis, long top30% / short bottom30%, "
          f"{h['n_days']} days (median {int(sp['n'].median())} names/day)")
    print("# THE HEADLINE — long-high-kurt / short-low-kurt spread")
    print(f"  spread: {h['spread_bps']:+.2f} bps/day  NW(10) t = {h['t_nw']:+.2f}  "
          f"one-sample t = {h['t_1s']:+.2f}")
    print(f"  books : high-kurt {h['hi_bps']:+.2f} vs low-kurt {h['lo_bps']:+.2f} bps  "
          f"(Welch t = {h['welch_t']:+.2f})")
    sps = sp["spread"].to_numpy()
    print(f"  gross spread Sharpe (no cost): "
          f"{sps.mean() / sps.std(ddof=1) * np.sqrt(252):.2f}")

sp = st.kurt_spreads(ret, window=21, frac=0.3)
...
for lo, hi, lbl in [("2010-01-01", "2018-01-01", "2010-2017"),
    ...
    ts = st.kurt_stats(sub)
    print(f"  {lbl}: n={ts['n_days']}  spread {ts['spread_bps']:+.2f} bps "
          f"(NW t={ts['t_nw']:+.2f})")

print("\n# THE TIMER — long-high-kurt / short-low-kurt, costed")
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=804 + s_, n_assets=40, n_days=1500)
...
p1 = data.synthetic_panel(edge=0.010, seed=804, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.010, seed 804): spread NW t = {sy['t_nw']:+.2f}, "
