"""Data layer for Study 1008 — The Start-Date Lottery.

Daily total returns for US large and small caps, developed international,
aggregate and long bonds, gold and short Treasuries.

Thirty-three years supports many overlapping thirty-year paths and roughly one independent one.
Every dispersion figure here therefore carries an effective-sample count, and the synthetic
control exists so that the *mechanisms* — sequence risk, contribution timing, glide paths — can
be measured on samples long enough for the answers to mean something.
...
CASH = "BIL"
BILLS = "SHY"
TICKERS = (EQUITY, SMALL, EAFE, BONDS, LONG_BONDS, GOLD, CASH, BILLS)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call startdate.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1008) -> tuple[pd.DataFrame, dict]:
