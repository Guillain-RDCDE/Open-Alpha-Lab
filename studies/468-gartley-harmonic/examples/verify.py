"""Reproducible headline run for Study 468 — Gartley / AB=CD harmonic patterns.
...
from gartley_harmonic import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch D-vs-random test
    HAVE_SCIPY = True
...
print("# Gartley harmonic — mechanical XABCD D-point long on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled D-point long vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'D_bps':>10} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total D-point completions (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: D one-sample t and D-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        c = _load(t)["close"]
        e = st.detect_completions(c, k=K)
        re = st.random_entries(c, max(len(e), 50), k=K, seed=7)
        ...
        print(f"  {t}: ent={len(e):>3} D={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Ratio-grid placebo (SPY, H=20, 500 draws)")
    pl = st.ratio_grid_placebo(_load("SPY")["close"], 20, k=K, n_draws=500, seed=468)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => Fibonacci ratios not load-bearing)")
else:
...
print("  edge=0 must NOT reach significance; planted Gartley bounce must light up.")
for edge in (0.0, 0.40):
    px, tr = data.synthetic_panel(edge=edge, seed=468, n_days=4000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.detect_completions(c, k=K), 20))
    print(f"  edge={edge:.2f}: planted={tr['n_planted']:>3}  n={s['n']:>4}  "
          f"D={s['mean_bps']:+7.1f}bps  win={s['win']*100:.0f}%  t={s['t']:+.2f}")
