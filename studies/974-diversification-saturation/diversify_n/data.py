"""Data layer for Study 974 — The Nth Asset.

Twelve asset classes rather than twelve tickers: US large and small cap, developed and
emerging international, long and intermediate Treasuries, investment-grade and high-yield
credit, gold, broad commodities and two property sleeves. The two property funds are in on
purpose — they are near-duplicates, and a diversification study that cannot see a duplicate is
not measuring diversification.
...
# Twelve liquid, genuinely different exposures — the sleeve a multi-asset portfolio is
# actually built from, not twelve flavours of US equity.
UNIVERSE = ("SPY", "IWM", "EFA", "EEM", "TLT", "IEF", "LQD", "HYG",
            "GLD", "DBC", "XLRE", "VNQ")
CASH = "BIL"
TICKERS = UNIVERSE + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call diversify_n.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 974) -> tuple[pd.DataFrame, dict]:
