"""Reproducible headline run for Study 860 — Accounting Conservatism (C-Score).
...
from conservatism import data, strategy as st  # noqa: E402

print("# Accounting Conservatism (C-Score) — does reserve-intensity predict forward returns?")
...
print("\n# PRIMARY — calendar-time tercile long-short on the C-score (reserves/Assets)")
print("#   (rank fresh signals monthly, long high-conservatism / short low, earn next month; one lag)")
...
ls120 = st.calendar_ls(px, ev, signal_col="cscore", n_buckets=3, min_names=6, staleness_days=120)
...
print(f"  NOA-scaled signal (reserves/NOA): n={s_noa['n_months']}, mean {s_noa['mean_bps']:+.1f} bps, "
      f"NW t = {s_noa['t_nw']:+.2f}, Sharpe {s_noa['sharpe']:.2f}")

print("\n# CROSS-CHECK — pooled event drift (tercile sort by C-score, 1-day-lag entry)")
...
bm126 = st.bucket_means(st.event_drift_frame(px, ev, horizon=126), 3)
print(f"  monotonicity (126d terciles low->high): {[f'{b*100:+.2f}%' for b in bm126]}")

print("\n# ERA SPLIT — calendar long-short pre/post 2016")
for lbl, lo, hi in [("2009-2015", "2000-01-01", "2016-01-01"),
                    ("2016-2026", "2016-01-01", "2100-01-01")]:
    sub = ls[(ls.index >= lo) & (ls.index < hi)]["ls"].to_numpy()
    print(f"  {lbl}: n={len(sub)}  mean {sub.mean()*1e4:+.1f} bps  NW t = {st.newey_west_t(sub):+.2f}")

print("\n# THIRD AXIS — is the accounting actually conservative? (Basu asymmetric timeliness)")
q = st.basu_asymmetry(ev)
print(f"  n={q['n']}  good-news slope b1={q['b_good']:+.3f}  asymmetry b3={q['b_asym']:+.3f}  "
      f"t(b3)={q['t_asym']:+.2f}  R2={q['r2']:.3f}")
...
print(f"  planted edge=0.15 (seed 860): mean {pl['mean_bps']:+.1f} bps/mo, NW t = {pl['t_nw']:+.2f}")
