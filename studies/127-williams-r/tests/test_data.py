...
def test_synthetic_is_deterministic():
    a, _ = data.synthetic_daily(n_days=100, mean_rev=0.1, seed=5)
    b, _ = data.synthetic_daily(n_days=100, mean_rev=0.1, seed=5)
    assert np.allclose(a["close"].to_numpy(), b["close"].to_numpy())
    c, _ = data.synthetic_daily(n_days=100, mean_rev=0.1, seed=6)
    assert not np.allclose(a["close"].to_numpy(), c["close"].to_numpy())


def test_mean_rev_creates_negative_autocorrelation():
    """The mean_rev knob really induces negative serial correlation (and 0 induces ~none)."""
    flat, _ = data.synthetic_daily(n_days=800, mean_rev=0.0, seed=127)
    rev, _ = data.synthetic_daily(n_days=800, mean_rev=0.30, seed=127)
