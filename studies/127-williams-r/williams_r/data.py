"""Data layer for Study 127 (Williams-R).
...
- ``synthetic_daily`` — a *deterministic, offline* generator.  A bar-level AR(1) knob
  (``mean_rev``) lets us dial the only thing Williams %R can possibly harvest: short-term
  mean-reversion in price.  ``mean_rev = 0`` is a pure random walk with no predictable
  structure; ``mean_rev > 0`` plants oversold/overbought bounces the %R rule can ride.
...
Williams %R (14-period) is defined as:
    %R(t) = -100 * (highest_high(14) - close(t)) / (highest_high(14) - lowest_low(14))

so it runs from 0 (close at the 14-day high = overbought) to -100 (close at the 14-day low
= oversold).  The folk rule:
  - %R < -80 → "oversold" → *buy* next open (mean-reversion bet)
  - %R > -20 → "overbought" → *sell* / short next open (mean-reversion bet)
...
def synthetic_daily(
    ...
    autocorrelation (positive ``mean_rev``) is the structure that Williams %R's
    oversold/overbought rule is designed to harvest — extended runs in one direction
    should get pulled back.

    - ``mean_rev = 0``   → a martingale; the %R signal is a fair coin.
