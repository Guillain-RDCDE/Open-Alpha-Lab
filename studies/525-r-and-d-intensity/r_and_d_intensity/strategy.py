"""Strategy + inference for Study 525 — R-And-D-Intensity (Chan-Lakonishok-Sougiannis 2001).

The rule under test (the believers' / academic version):

    Each month, rank the field by **R&D-to-market-cap** (R&D / ME). Hold the **top tertile
    equal-weight (long the high-R&D/ME names)** and **short the bottom tertile** for the next
    month; rebalance monthly. The long-high / short-low spread is the claimed *CLS R&D premium*.

CLS's decisive finding is that the premium lives in R&D scaled by **market value** (a value /
mispricing signal — high R&D/ME ≈ a lot of intangible building bought cheaply), and is far weaker
in R&D scaled by **sales** (pure spending intensity, study 400). So the study's third axis is the
**denominator contrast**: does R&D/ME beat R&D/sales, as CLS predict?

We measure it honestly:

  * **One execution lag, exact.** The signal at month-end ``t`` (R&D from the most recent reported
    fiscal year, price as of ``t``) selects the book entered at the **next** month's open and held
    through month ``t+1``. No same-bar fill, no look-ahead in the accounting (1-year reporting lag).
  ...
  * **Costs + borrow.** One-way turnover × NAV at ``cost_bps`` per rebalance (monthly turnover is
    real here — the market-cap denominator moves the ranking every month), plus an annual
    **borrow** fee on the short leg (you pay to be short the low-R&D/ME names).
"""
...
def _leg_returns(signal: pd.DataFrame, returns: pd.DataFrame, frac: float,
    ...
    for i in range(len(sig_months) - 1):
        ...
        cur = set(members)
        # one-way turnover = fraction of weight traded = |new ∪ old changes| / size, equal-weight
        if prev_set:
...
def placebo_null(signal: pd.DataFrame, returns: pd.DataFrame, frac: float = 1 / 3,
                 n_shuffles: int = 400, seed: int = 525) -> np.ndarray:
...
def race(real: dict, signals: dict, frac: float = 1 / 3, cost_bps: float = 10.0,
         borrow_bps: float = 100.0, n_shuffles: int = 400, seed: int = 525) -> dict:
    """The full teardown: R&D/ME long-short vs SPY vs a label-shuffle placebo, plus the R&D/sales
    contrast (the CLS denominator question).

    ``signals`` is the dict from :func:`data.build_signals` (keys ``rd_me`` and ``rd_sales``).
    """
    rets, spy = real["returns"], real["spy"]
    me_books = signal_books(signals["rd_me"], rets, frac=frac, cost_bps=cost_bps,
                            borrow_bps=borrow_bps)
    sales_books = signal_books(signals["rd_sales"], rets, frac=frac, cost_bps=cost_bps,
                              borrow_bps=borrow_bps)
    null = placebo_null(signals["rd_me"], rets, frac=frac, n_shuffles=n_shuffles, seed=seed)
    ls_mean_ann = me_books["long_short"].mean() * MONTHS_PER_YEAR
    return {
        **{k: me_books[k] for k in ("long", "short", "long_short", "long_net", "ls_net")},
        "avg_turnover": me_books["avg_turnover"],
        "spy": spy,
        "test_ls": hac_tstat(me_books["long_short"]),
        "test_long_vs_spy": hac_tstat_diff(me_books["long"], spy),
        "test_short_vs_spy": hac_tstat_diff(me_books["short"], spy),
        "test_ls_net": hac_tstat(me_books["ls_net"]),
        # the CLS contrast: R&D/sales (study-400 intensity)
        "sales_long_short": sales_books["long_short"],
        "test_sales_ls": hac_tstat(sales_books["long_short"]),
        # placebo
        "placebo_null": null,
