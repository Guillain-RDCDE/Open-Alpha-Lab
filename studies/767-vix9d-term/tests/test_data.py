...
def test_synthetic_shape_and_columns(null_tape):
    ...
    assert set(["VIX9D", "VIX", "slope", "SPY_close", "SPY_ret"]).issubset(df.columns)
    assert (df["VIX9D"] > 0).all()
    assert (df["VIX"] > 0).all()
    assert (df["SPY_close"] > 0).all()


def test_synthetic_is_deterministic():
    a, _ = data.synthetic_daily(n_days=500, contango_signal=0.0, seed=767)
    b, _ = data.synthetic_daily(n_days=500, contango_signal=0.0, seed=767)
...
def test_slope_definition(null_tape):
    """slope = log(VIX / VIX9D) should match the computed column."""
    df, _ = null_tape
    expected = np.log(df["VIX"] / df["VIX9D"])
    assert np.allclose(df["slope"].to_numpy(), expected.to_numpy(), atol=1e-9)


def test_contango_is_majority_state(null_tape):
    """Natural front-end dynamics produce more contango than backwardation."""
    df, _ = null_tape
    frac_contango = (df["slope"] > 0).mean()
    assert frac_contango > 0.55, f"Expected >55% contango, got {frac_contango:.2%}"


def test_backwardation_is_more_frequent_than_long_end():
    """The 9-day tenor is twitchier: it inverts more than a smoother long-end curve would.

    Sanity check that the synthetic front end spends a non-trivial share of time in
    backwardation (unlike the ~10% of Study 111's 3-month slope)."""
    df, _ = data.synthetic_daily(n_days=3000, contango_signal=0.0, seed=767)
    frac_back = (df["slope"] < 0).mean()
    assert frac_back > 0.15, f"Expected >15% backwardation on the twitchy front end, got {frac_back:.2%}"
...
def test_signal_knob_has_effect():
    ...
    null, _ = data.synthetic_daily(n_days=2000, contango_signal=0.0, seed=767)
    sig, _ = data.synthetic_daily(n_days=2000, contango_signal=0.05, seed=767)
