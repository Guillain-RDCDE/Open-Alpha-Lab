"""Data layer for Study 975 — Shrink the Matrix.

Forty single names are in this universe for one reason: the sector sleeve everyone
demonstrates shrinkage on has more rows than parameters, which is precisely the case where
shrinkage does not matter. The stock cross-section has 820 covariance parameters and a
one-year window offers 252 observations to fit them — that is where an estimator earns its
keep, and where a study that only tested eleven sectors would report nothing.
...
# Two cross-sections on purpose. The eleven sector SPDRs are the benign case (66
# parameters, plenty of rows). The forty single names are where the arithmetic bites: 820
# parameters, and a one-year window has 252 rows to estimate them from.
...
BENCH = "SPY"
CASH = "BIL"
TICKERS = SECTORS + NAMES + (BENCH, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call shrinkage.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 975) -> tuple[pd.DataFrame, dict]:
