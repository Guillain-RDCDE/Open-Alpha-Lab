"""Reproducible headline run for Study 687 — Ladder Bottom.
...
from ladder_bottom import data, strategy as st  # noqa: E402

print("# Ladder Bottom — does the 5-candle ladder (4 declining candles, then a bullish "
      "reversal) end a downtrend?")
...
print("\n# THE DETECTOR — four declining bearish candles in a downtrend, then a bullish break")
res = st.run_experiment(panel)
print(f"  loose cut  (4 declining candles + downtrend context + bullish break): "
      f"n = {res['n_loose']}")
print(f"  strict cut (+ near-low rungs, warning wick, gap-up reversal): "
      f"n = {res['n_strict']}  <- the literature-closer reading, and it is genuinely rare")
...
print(f"  {'H':>3} {'n':>5} {'ladder_bps':>10} {'win%':>5} {'base_bps':>9} {'delta_bps':>10} "
...
for h in st.HORIZONS:
    ...
    print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win_rate']*100:>5.1f} "
          f"{b['mean_bps']:>9.1f} {ph['delta_bps']:>10.1f} {wt_s:>8} {ph['placebo_p']:>10.3f} "
          f"{s['net_bps']:>8.1f}")

print("\n# STRICT cut — the literature-closer ladder bottom")
print(f"  {'H':>3} {'n':>5} {'ladder_bps':>10} {'win%':>5} {'delta_bps':>10} "
      f"{'welch_t':>8} {'placebo_p':>10} {'net_bps':>8}")
for h in st.HORIZONS:
...
print("  a planted ladder-bottom bounce. Null checked over 20 seeds (never a single stream).")
null_ts = []
for s_ in range(20):
    d, _truth = data.synthetic_panel(edge=0.0, seed=687 + s_)
...
for edge in (0.02, 0.04):
    d, truth = data.synthetic_panel(edge=edge, seed=687)
    r = st.synthetic_detect(d, horizon=20, seed=687)
    print(f"  planted edge={edge:.2f} (seed 687): n={r['n']}  ladder {r['mean_bps']:+.1f} bps "
