"""Data layer for Study 981 — The Price of Waiting.

Daily total-return closes. The cash leg matters here more than in most studies: a
confirmation rule spends measurable time *out* of the market by construction, so the return it
gives up depends on what the sidelines pay, and 2007-2026 contains both zero-rate and
five-percent regimes.
...
# Four tapes with different trend characters: US large cap, US small cap, long
# Treasuries and gold. A confirmation rule that only helps on the one asset that trended
# cleanly is a description of that asset, not of the rule.
RISKY = ("SPY", "IWM", "TLT", "GLD")
CASH = "BIL"
TICKERS = RISKY + (CASH,)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call confirm_delay.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 981) -> tuple[pd.DataFrame, dict]:
