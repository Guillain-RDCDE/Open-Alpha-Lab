"""Reproducible headline run for Study 776 — Valentine-Sparkle.

Prints every number quoted in docs/results.md and frozen into the ``R`` dict in
notebooks/build_notebooks.py. Deterministic; uses the cached SIG / SPY tapes under
...
print("# Valentine-Sparkle — does Signet (SIG) rally into Valentine's Day?")

print(f"calendar: {len(dt.EVENTS)} Valentine's Days {dt.EVENTS[0][0]}->{dt.EVENTS[-1][0]}, "
      f"hardcoded as February 14 each year (anchored on last close on/before the 14th)")

if not dt.have_real():
    print("(cache miss — fetching SIG + SPY once)")
    dt.fetch()

prices = dt.load_real()
panel = pd.DataFrame({t: s for t, s in prices.items()}).dropna()
print(data_stamp("Valentine panel (SIG + SPY, adjusted/total-return)", panel, asof=dt.AS_OF))

ev = st.build_event_table(prices, cost_bps=5.0)
ev10 = st.build_event_table(prices, cost_bps=10.0)
inc, inc10 = ev[ev["included"]], ev10[ev10["included"]]
print(f"\nevents resolved: {len(inc)} of {len(dt.EVENTS)} Valentine's Days have SIG+SPY "
      f"coverage around the anchor")
for reason, n in ev[~ev["included"]]["reason"].value_counts().items():
    print(f"  excluded {n:2d}x {reason}")

print("\n# THE RUN-UP (the 'buy the gifting quarter') — abnormal return, K sessions BEFORE "
      "Valentine's, SIG minus SPY (gross)")
...
print("\n# THE FADE (the 'sell the holiday') — abnormal return, K sessions AFTER Valentine's")
...
for bump in (0.01, 0.02):
    planted = st.synthetic_detect(bump=bump, seed=782, k=10)
    print(f"  planted run-up bump=+{bump*100:.1f}% (seed 782): mean AR {planted['mean']*100:+.3f}%  "
