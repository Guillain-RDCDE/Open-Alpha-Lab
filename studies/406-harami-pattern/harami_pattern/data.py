"""Data layer for Study 406 — the Harami candlestick pattern.
...
* **Synthetic.** ``synthetic_panel`` — a deterministic, fixed-seed generator with a
  **planted-edge knob**. With ``edge = 0`` the post-harami path is pure noise (a fair
  coin: the pattern must NOT manufacture an edge); with ``edge > 0`` the bars are nudged
  so that the day *after* a real bullish harami drifts up (and after a bearish one, down)
  — the positive control proves the harness *can* bank a planted reversal.

The harami ("pregnant" in Japanese) is the geometric inverse of the engulfing pattern:
a *large* real body followed by a *small* one whose body sits *inside* the prior body.
The folk rule reads it as a reversal — a bullish harami (big red, small green inside)
after a downtrend signals a bottom; a bearish harami (big green, small red inside) after
an uptrend signals a top.
...
def _cache_path(ticker: str, cache_dir: str) -> str:
    ...
    return os.path.join(cache_dir, f"harami_{safe}_1d.parquet")
...
def synthetic_panel(n_names: int = 30, n_days: int = 2200, edge: float = 0.0,
                    seed: int = 406, daily_vol: float = 0.013,
                    start: str = "2010-01-04") -> tuple[dict[str, pd.DataFrame], dict]:
    """Deterministic OHLCV panel with a PLANTED post-harami reversal (knob ``edge``).
    ...
    high/low. After each bar we test whether it formed a textbook **harami** candle
    (large prior body, small current body sitting inside it — see ``strategy.is_harami``);
    if ``edge != 0`` we add an *extra* drift of ``edge * direction`` to the **next** day's
    return — i.e. a real day-after reversal in the direction the pattern predicts (up
    after bullish, down after bearish).
    ...
    for k in range(n_names):
        ...
        # Build bars incrementally so the planted drift is causal (no look-ahead): we
        # inject the edge on the day AFTER a detected harami.
        log_ret = eps.copy()
        ...
        prev_close = 100.0
        pending = 0  # direction of the just-formed harami; applies to the next bar
        for i in range(n_days):
            ...
            hi[i] = max(o, c) + w
            lo[i] = min(o, c) - w
            # detect harami on the bar just formed (uses i-1, i) -> set pending for i+1
            pending = 0
            if i >= 1:
                ...
                body0 = abs(pc0 - po0)
                body1 = abs(pc1 - po1)
                # current body strictly inside the prior body, prior body strictly larger
                if body0 > 0 and body1 < body0:
                    hi0, lo0 = max(po0, pc0), min(po0, pc0)
                    hi1, lo1 = max(po1, pc1), min(po1, pc1)
                    if lo1 >= lo0 and hi1 <= hi0:
                        # bullish: prior red (big down), current green (small up) inside
                        if (pc0 < po0) and (pc1 > po1):
                            pending = +1
                        # bearish: prior green (big up), current red (small down) inside
                        elif (pc0 > po0) and (pc1 < po1):
                            pending = -1
            prev_close = c
