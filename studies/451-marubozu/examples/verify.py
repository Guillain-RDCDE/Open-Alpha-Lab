"""Reproducible headline run for Study 451 — Marubozu.
...
from marubozu import data, strategy as st  # noqa: E402

try:
    from scipy import stats  # for the Welch marubozu-vs-random test
    HAVE_SCIPY = True
...
print("# Marubozu — mechanical wickless-up-bar continuation on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    print("\n# Pooled bullish-marubozu vs drift-matched random baseline")
    print(f"  {'H':>3} {'n':>5} {'maru_bps':>9} {'win%':>5} {'1samp_t':>8} "
    ...
    for h in st.HORIZONS:
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>9.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total bullish marubozus (H={st.HORIZONS[0]}): {total}")

    print("\n# Per-ticker, H=20: marubozu one-sample t and marubozu-minus-random delta")
    for t in data.DEFAULT_TICKERS:
        b = _load(t)
        c = b["close"]
        e = st.marubozu_entries(b)
        ...
        print(f"  {t}: ent={len(e):>3} maru={s['mean_bps']:+7.1f}bps t={s['t']:+5.2f} "
              f"rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Body-shuffle GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.body_shuffle_placebo(_load("SPY"), 20, n_draws=500, seed=451)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => no-wick body not load-bearing)")
else:
...
for edge in (0.0, 0.60):
    px, tr = data.synthetic_panel(edge=edge, seed=451, n_days=4000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.marubozu_entries(px), 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  maru={s['mean_bps']:+7.1f}bps  "
          f"win={s['win']*100:.0f}%  t={s['t']:+.2f}  (planted marubozus={tr['planted_maru']})")
