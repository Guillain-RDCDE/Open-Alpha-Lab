"""Fully-offline, deterministic tests for Study 846 — Blockbuster Game-Launch Drift.
...
def test_calendar_shape():
    # 37 marquee launches: 33 on the four listed publishers + 4 ATVI (delisted).
    assert len(dt.EVENTS) == 37
    pubs = [p for _, p, _ in dt.EVENTS]
    for p in dt.PUBLISHERS + dt.DELISTED:
        assert pubs.count(p) >= 1
    assert pubs.count("ATVI") == 4          # the delisted, tape-less publisher
    # every publisher tag is one of the known tickers
    assert set(pubs) <= set(dt.PUBLISHERS + dt.DELISTED)
    dates = [pd.Timestamp(d) for d, _, _ in dt.EVENTS]
    years = {d.year for d in dates}
    assert min(years) == 2013 and max(years) == 2024
    # every event carries a non-empty title
    assert all(title.strip() for _, _, title in dt.EVENTS)
    # ATVI is NOT in the fetchable ticker set (no tape)
    assert "ATVI" not in dt.all_tickers()
    assert set(dt.all_tickers()) == set(dt.PUBLISHERS) | {dt.BENCHMARK}
...
def test_wilson_interval_brackets_point_estimate():
    lo, hi = st.wilson_interval(13, 33)
    assert lo < 13 / 33 < hi
...
def test_synthetic_world_deterministic(null_world):
    a, b, keys = null_world
    a2, b2, keys2 = dt.synthetic_world(drift=0.0, seed=846)
    assert keys == keys2
    assert np.allclose(a.values, a2.values) and np.allclose(b.values, b2.values)
    assert len(keys) == 32  # 32 synthetic launches, ~mirroring the resolvable calendar size


def test_synthetic_null_is_quiet():
    # drift = 0 -> the launch-drift detector should not fire; mean |t| small across seeds
    ts = np.array([st.synthetic_detect(drift=0.0, seed=846 + s, k=20)["t"] for s in range(20)])
    assert abs(ts.mean()) < 0.75
    # honest small-sample FPR bound at n=32 events with a 20-day AR
    assert (np.abs(ts) >= 2).sum() <= 4


def test_synthetic_planted_drift_lifts_t_monotonically():
    t0 = st.synthetic_detect(drift=0.0, seed=846, k=20)["t"]
    t1 = st.synthetic_detect(drift=0.02, seed=846, k=20)["t"]
    t2 = st.synthetic_detect(drift=0.04, seed=846, k=20)["t"]
    assert t0 < t1 < t2
...
def test_synthetic_planted_drift_recovered_unit_slope():
    ...
    m0 = st.synthetic_detect(drift=0.0, seed=846, k=20)["mean"]
    m4 = st.synthetic_detect(drift=0.04, seed=846, k=20)["mean"]
...
def _synthetic_price_panel(post_drift: float = 0.0, seed: int = 7) -> dict[str, pd.Series]:
    """Build TTWO/EA/NTDOY/UBSFY/SPY adjusted-close Series on a business-day index spanning
    the real event calendar, optionally injecting a positive publisher AR in the sessions
    after each launch anchor (to prove build_event_table's post-window AR picks it up).
    ATVI is intentionally absent — its launches must resolve to excluded."""
    idx = pd.bdate_range("2012-06-01", periods=3800)  # ~2012-06 -> ~2027, safe horizon
    ...
    if post_drift != 0.0:
        for launch, publisher, _ in dt.EVENTS:
            if publisher not in prices:
                continue
            common = prices[publisher].index
            pos = common.searchsorted(pd.Timestamp(launch), side="right") - 1
            # bump the publisher's price path for 20 sessions after the anchor
            prices[publisher].iloc[pos + 1:pos + 21] *= (1.0 + post_drift)
    return prices


def test_build_event_table_resolves_listed_events_offline():
    prices = _synthetic_price_panel(post_drift=0.0)
    ev = st.build_event_table(prices)
    assert len(ev) == len(dt.EVENTS)
    inc = ev[ev["included"]]
    # the 33 non-ATVI launches resolve; the 4 ATVI launches are excluded for lack of a tape
    assert len(inc) == 33
    assert set(inc["publisher"]) == set(dt.PUBLISHERS)
    excl = ev[~ev["included"]]
    assert (excl["publisher"] == "ATVI").all() and len(excl) == 4
...
def test_event_table_picks_up_planted_post_drift():
    ...
    # a planted +3% post-launch bump lifts the mean 20-day post-window AR clearly
    assert st.one_sample_t(u_inc["post_l"].values)["mean"] > \
        st.one_sample_t(b_inc["post_l"].values)["mean"] + 0.02


def test_net_is_below_gross():
    ev = st.build_event_table(_synthetic_price_panel(post_drift=0.01), cost_bps=10.0)
    inc = ev[ev["included"]]
    assert (inc["post_l_net"] < inc["post_l"]).all()
...
def test_placebo_runs_and_pvalue_in_range():
    prices = _synthetic_price_panel(post_drift=0.0)
    ev = st.build_event_table(prices)
    pl = st.placebo_pvalue(ev, prices, "post_l", k=20, n_seeds=3, n_draws_per_seed=50,
                           tail="right")
...
def test_real_panel_resolves_listed_launches():
    prices = dt.load_real()
    ev = st.build_event_table(prices)
    # every non-ATVI launch resolves; ATVI (delisted) stays excluded
    assert int(ev["included"].sum()) == 33
    assert "ATVI" not in prices
