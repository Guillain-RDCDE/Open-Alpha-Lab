"""Strategy + inference for Study 846 — Blockbuster Game-Launch Drift.

The claim: **"buy the hype into a marquee game launch,"** then either ride the momentum or
**"sell the news."** The finance transplant asks whether the *publisher* (TTWO / EA /
NTDOY / UBSFY) earns an abnormal return around a blockbuster's ship date. Two windows, both
anchored on the real launch date and measured against SPY:

* **The run-up (the "buy the hype").** The publisher's abnormal return (publisher
  total-return minus the SPY total-return benchmark) over the K trading sessions ENDING on
  the last close on/before the launch. Tested at K = 5 (~1 week) and K = 10 (~2 weeks).

* **The post-launch drift (the "ride / sell the news").** The publisher's abnormal return
  over the K sessions STARTING from the launch-day close. Tested at K = 5 (~1 week) and the
  headline **K = 20 (~1 month) post-drift**. If the folklore is right the run-up is
  positive and the drift keeps ripping (momentum); a negative drift is "sell the news."

Each launch is a single, non-overlapping event pooled across the four listed publishers (a
Rockstar/2K title → TTWO, an EA title → EA, a Nintendo title → NTDOY, a Ubisoft title →
UBSFY), so the primary statistic is a **one-sample t** of the abnormal return across events
(n = the number of launches with publisher+SPY coverage). A **Newey-West (HAC) t** on the
date-ordered per-event series is reported as a robustness cross-check; with well-spaced
events it should land close to the one-sample t. A **random-window placebo** (drawing many
random, non-launch K-session windows on the *same* publisher vs SPY) checks whether the
observed mean sits inside or outside the stock's ordinary tracking noise. With only ~32
resolvable events, power is low — a mild lean will not clear |t| ≥ 2, and we say so.

Survivorship note (Signal axis): TTWO, EA, NTDOY and UBSFY are all **survivors** — large,
still-listed publishers — so there is no delisting bias in the panel, but by the same token
the sample cannot speak to a publisher that failed. The five **ATVI** launches carry NO
tape (Activision Blizzard delisted 2023-10-13 after the Microsoft buyout) and resolve to
EXCLUDED in the funnel — a documented gap, not a silent drop.
"""
...
POST_SHORT_K = 5     # ~1 week post-launch drift
POST_LONG_K = 20     # ~1 month post-launch drift (the headline window)
COST_BPS = 5.0       # one-way, per leg
EXEC_LAG = 0         # launch date is calendar-known months/years ahead; no announcement lag
...
def build_event_table(prices: dict[str, pd.Series], cost_bps: float = COST_BPS
    ...
    Each event is anchored to *its own* publisher (a Rockstar/2K title → TTWO, an EA title
    → EA, etc.) versus SPY. A row is INCLUDED only if the publisher and SPY both have cached
    history covering [anchor − PRE_LONG_K .. anchor + POST_LONG_K] for that event. Launches
    whose publisher has no tape at all (ATVI, delisted) are excluded with a reason, so the
    funnel is auditable.
    """
    spy = prices[dt.BENCHMARK]
    rt = 2.0 * cost_bps / 1e4    # round-trip cost (one-way x 2), applied to the net leg
    rows = []
    for launch, publisher, title in dt.EVENTS:
        row = dict(launch=launch, publisher=publisher, title=title)
        if publisher not in prices:
            row.update(included=False,
                       reason=f"{publisher} tape unavailable (delisted; no history)")
            rows.append(row)
            continue
...
def placebo_pvalue(events: pd.DataFrame, prices: dict[str, pd.Series], col: str,
                   ...
                   n_seeds: int = 20, n_draws_per_seed: int = 200, base_seed: int = 846,
                   tail: str = "right") -> dict:
    """For each INCLUDED event, redraw a random (non-launch) k-session window on the event's
    OWN publisher vs SPY and recompute the abnormal return; average across the same n events;
    ...
    # Precompute, per publisher present in the panel, the aligned (pub, spy) arrays.
    pub_arrays = {}
...
# --------------------------------------------------------------------------- #
def synthetic_detect(drift: float, seed: int, k: int = POST_LONG_K,
                     side: str = "post") -> dict:
    """Run the one-sample-t detector on a synthetic paired world with a planted launch
    drift. ``side='post'`` measures the launch window [p..p+k); ``side='pre'`` measures the
    run-up [p-k..p)."""
