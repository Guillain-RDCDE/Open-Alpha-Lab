"""Strategy + inference for Study 700 — Bat-Harmonic.

The claim: **the Bat harmonic predicts the reversal at point D, and — per Scott
Carney — is the *most reliable* member of the harmonic-pattern family.** Chartists
draw four alternating swing pivots X, A, B, C, then project a fifth point D such
that:

1. **AB retraces XA by 0.382-0.50** — a *shallower* pullback than Gartley's 0.618
   or Butterfly's 0.786. This is a structural band (every XABCD pattern needs a
   well-formed B leg); it is not itself the hypothesis under test.
...
3. **D retraces 0.886x of the ORIGINAL XA leg** — this is the Bat's real
   signature, and the specific number the brief asks us to test. Unlike
   Butterfly/Crab, D stays *inside* the X-A range (a deep retracement, not an
   extension past X) — Carney's stated reason the Bat is "the most reliable":
   the reversal zone sits close to, but never beyond, the pattern's origin, so a
   failed Bat is cheap to invalidate (a stop just past X). Believers read the
   0.886 level as an unusually precise support/resistance confluence and treat D
...
2. **XABCD candidates.** Every consecutive confirmed quadruple (X, A, B, C) whose
   ...
   **D = A - retrace_d x (A - X)** with ``retrace_d = 0.886`` — a price target
...
* **Forward returns at D vs a random-day BASE RATE, Bonferroni-corrected.** The
  ...
  directions as the real Bat touches — i.e. "what a random entry with the same
...
* **A placebo arm (the third-axis myth-check).** Identical pivots, but each
  ...
  kept clear of the Bat's own 0.886 band — "would ANY deep retracement/extension
  zone near X have worked", not specifically the 0.886 ratio Carney trademarked.
"""
...
# --------------------------------------------------------------------------- #
# 2. XABCD candidates — Bat arm and its seeded placebo
# --------------------------------------------------------------------------- #
...
def find_bat(pivots: pd.DataFrame,
            ab_lo: float = 0.382, ab_hi: float = 0.50,
            bc_lo: float = 0.382, bc_hi: float = 0.886,
            target_d: float = 0.886, d_tol: float = 0.05,
            placebo: bool = False, seed: int = 0) -> pd.DataFrame:
    """Scan consecutive confirmed pivot quadruples (X, A, B, C) for a Bat.
    ...
    ``[ab_lo, ab_hi]`` (0.382-0.50 — the Bat's characteristically shallow B,
    versus Gartley's 0.618 or Butterfly's 0.786) and the BC retracement of AB
    must sit in ``[bc_lo, bc_hi]`` (0.382-0.886, the shared XABCD band).

    Bat arm (``placebo=False``): D is projected as a retracement of the
    *original* XA leg, ``D = A - target_d x (A - X)`` with ``target_d = 0.886``
    — the Bat's defining signature, deep but always **inside** [X, A] (never
    beyond X, unlike Butterfly/Crab). ``reversal_dir`` is +1 (expect a bounce UP
    off D) when XA rose (bullish Bat, D projects below A near X) and -1
    otherwise.

    Placebo arm (``placebo=True``): identical structural pivots, but each
    candidate draws its *own* deterministic D-retracement target, uniformly at
    random and kept away from the Bat's 0.886 band under test — "would ANY deep
    retracement/extension zone near X have worked", not specifically 0.886.
    """
    rows = []
    ...
    for k in range(len(P) - 3):
        ...
        if placebo:
            ...
            t_d = float(rng.uniform(0.55, 1.20))
            while abs(t_d - target_d) < d_tol:
                t_d = float(rng.uniform(0.55, 1.20))
        else:
            t_d = target_d

        d_proj = float(A["price"] - t_d * XA)              # D retraces the ORIGINAL XA leg
        ...
        rows.append({
            ...
            "d_target": t_d, "D_proj": d_proj, "reversal_dir": reversal_dir,
        })
    cols = ["X_i", "A_i", "B_i", "C_i", "conf_i", "X_p", "A_p", "B_p", "C_p",
            "retrace_ab", "retrace_bc", "d_target", "D_proj", "reversal_dir"]
...
def scan_touches(bars: pd.DataFrame, candidates: pd.DataFrame, max_fwd_days: int = 120,
    ...
    for _, r in candidates.iterrows():
        ...
        rec = {
            ...
            "d_target": r["d_target"], "reversal_dir": d, "entry": entry_px,
            "entry_i": bar_i,
        }
        for hh in horizons:
    ...
    cols = (["touch_date", "X_i", "A_i", "B_i", "C_i", "retrace_ab", "retrace_bc",
             "d_target", "reversal_dir", "entry", "entry_i"]
...
def detect_and_scan(bars: pd.DataFrame, pct: float = 0.03, placebo: bool = False, seed: int = 0,
    ...
    cand = find_bat(piv, ab_lo, ab_hi, bc_lo, bc_hi, target_d, d_tol,
...
def hac_mean_t(x: np.ndarray) -> float:
    ...
    time (overlapping Bat legs on the same tape)."""
...
def base_rate_ledger(bars: pd.DataFrame, n_events: int, dir_mix: float,
    ...
    (``dir_mix`` = share of ``reversal_dir == +1``) as the real Bat touches — the
