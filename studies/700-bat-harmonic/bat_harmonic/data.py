"""Data layer for Study 700 — Bat-Harmonic.
...
* **Synthetic world.** A deterministic, seeded daily price path with a TUNABLE
  ...
  (the null the Bat detector must NOT manufacture significance from); at
  ``mean_rev > 0`` price is nudged back toward a slow moving average, giving *any*
  well-formed retracement-and-reversal structure (Bat-labelled or not) genuine
...
def _cache_path(ticker: str) -> str:
    return os.path.join(CACHE_DIR, f"bat_{ticker.lower()}.csv")
...
def synthetic_world(mean_rev: float = 0.0, n_days: int = 2500, annual_vol: float = 0.22,
                    start: str = "2010-01-04", seed: int = 700) -> pd.DataFrame:
    ...
    anchor" mechanism that would make ANY retracement-and-reversal projection
    (Bat-labelled or not) show a genuine edge once ``mean_rev > 0``.
    ``mean_rev = 0`` is the null: the Bat detector's split must NOT reach
