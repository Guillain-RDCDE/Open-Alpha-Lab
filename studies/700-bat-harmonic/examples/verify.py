"""Reproducible headline run for Study 700 — Bat-Harmonic.
...
from bat_harmonic import data, strategy as st  # noqa: E402
...
print("# Bat-Harmonic — does a 0.886 XA retracement at D predict a reversal?")
...
def pooled(placebo: bool, cost: float = 0.0, seed: int = 700, horizons=HORIZONS):
...
print("\n# THE HEADLINE — Bat XABCD (AB retrace 0.382-0.50 of XA, BC retrace 0.382-0.886 "
      "of AB, D = A - 0.886*(A-X), a deep retracement that stays INSIDE [X, A])")
bat_g, n_piv, n_cand_bat, bat_by_ticker = pooled(placebo=False, cost=0.0)
print(f"  pooled confirmed pivots: {n_piv:,}  |  Bat XABCD candidates: {n_cand_bat:,}  "
      f"|  D-touches: {len(bat_g):,}")

for h in HORIZONS:
    sf = st.summarize(bat_g, f"ret_gross_{h}")
...
# --------------------------------------------------------------------------- #
print("\n# SIGNAL AXIS — Bat D-touch fade vs a random-day BASE RATE "
      f"(matched bullish/bearish mix, pooled over {BASE_SEEDS} seeds), Bonferroni-corrected")

dir_mix_all = float((bat_g["reversal_dir"] > 0).mean()) if not bat_g.empty else 0.5
...
base_g = base_rate_pool(bat_by_ticker)
tests = []  # (label, welch_t, n_bat, n_base)
for h in HORIZONS:
    sf = st.summarize(bat_g, f"ret_gross_{h}")
    sb = st.summarize(base_g, f"ret_gross_{h}")
    wt = st.welch_t(bat_g[f"ret_gross_{h}"], base_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: BAT mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
for t in data.BASKET:
    ...
    if n_ev == 0:
        ...
        continue
    mix = float((l_bat["reversal_dir"] > 0).mean())
    frames = [st.base_rate_ledger(bars[t], n_ev, mix, horizons=HORIZONS, cost_bps=0.0,
                                  seed=st._seed_from(700, t, s)) for s in range(BASE_SEEDS)]
    br_t = pd.concat(frames, ignore_index=True)
    sf = st.summarize(l_bat, "ret_gross_5")
    sb = st.summarize(br_t, "ret_gross_5")
    wt = st.welch_t(l_bat["ret_gross_5"], br_t["ret_gross_5"])
    tests.append((f"{t} (5-day)", wt))
    print(f"  {t:5s} BAT n={sf['n']:3d} mean={sf['mean_bps']:+7.2f}bps   |   "
...
# --------------------------------------------------------------------------- #
# Third axis (myth-check) — does the specific 0.886 ratio beat a placebo zone?
# --------------------------------------------------------------------------- #
print("\n# THIRD AXIS (myth-check) — does the specific 0.886 XA retracement beat "
      "a placebo retracement/extension zone built off the identical pivots?")
...
for h in HORIZONS:
    ...
    wt = st.welch_t(bat_g[f"ret_gross_{h}"], plac_g[f"ret_gross_{h}"])
    tag = "  <-- headline horizon" if h == 5 else ""
    print(f"  {h:2d}-day: BAT mean={sf['mean_bps']:+7.2f}bps (n={sf['n']})   |   "
...
print(f"\n  Bat beats placebo on {n_beats}/{len(data.BASKET)} tickers (5-day)")
...
null_ts = []
for s_ in range(20):
    ledger = pooled_synth(0.0, 700 + s_)
...
planted = pooled_synth(0.12, 700)
sp_planted = st.summarize(planted, "ret_gross_5")
print(f"  planted mean_rev=0.12 (seed 700): n={sp_planted['n']}  mean={sp_planted['mean_bps']:+.2f}bps  "
