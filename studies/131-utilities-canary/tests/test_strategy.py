"""Signal construction, quintile logic, timing overlay, and the study's spine:
the canary only outperforms buy-and-hold when a signal is planted, and reads ~0 without."""
...
from utilities_canary import strategy as st  # noqa: E402
...
def test_rs_momentum_is_20day_diff(null_tape):
    ...
    expected = rs.diff(20)
    result = st.rs_momentum(df, window=20)
...
def test_quintile_table_counts_sum_to_sample(null_tape):
    ...
    # The total should be close to len(df) minus warmup and last observation
    assert total > 4000
...
def test_canary_signal_detected_with_planted_effect(canary_tape, null_tape):
    """The spine: a planted negative canary signal produces a negative Q5−Q1 spread
    (canary alert days have lower forward returns); on the null tape the spread is ~0."""
    df_c, _ = canary_tape
    df_n, _ = null_tape
    spread_c = st.top_minus_bottom(df_c, horizon=5)["spread_bps"]
    ...
    assert abs(spread_n) < abs(spread_c)
...
def test_timing_overlay_flat_when_alert(null_tape):
    df, _ = null_tape
    overlay = st.timing_overlay(df, cost_bps=0.0)
    # When alert, r_active should be 0 (cash, no SPY return earned)
...
def test_regime_stats_three_rows(null_tape):
    ...
    assert len(rs) == 3
    assert set(rs["regime"]) == {"full sample", "calm (RS Q1-Q2)", "alert (RS Q3-Q5)"}
