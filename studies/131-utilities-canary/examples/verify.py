"""Real-tape verification -- Study 131 (Utilities-Canary). Regenerates docs/results.md numbers.

Fetches (or reads from cache) the XLU/SPY daily tape, runs the utilities-canary analysis:
quintile-sorted forward returns, top-minus-bottom spreads at 1d/5d/21d, regime stats,
and a timing-overlay cost sweep. Network is touched only with --fetch.

    python studies/131-utilities-canary/examples/verify.py            # cache-only
    python studies/131-utilities-canary/examples/verify.py --fetch    # refresh the tape
"""
...
def main(fetch: bool) -> None:
    ...
    if fetch:
        ...
        print(f"XLU/SPY {df.index[0].date()} .. {df.index[-1].date()}  fp={data.fingerprint(df)}")
        print(f"n = {len(df)}")
        print()
    else:
    ...
    print("=== Quintile table (Q1=SPY outperforming/calm, Q5=XLU outperforming/alert) ===")
    for h in [1, 5, 21]:
        sub = qt[qt["horizon"] == h]
        print(f"\nhorizon={h}d:")
        for _, r in sub.iterrows():
            print(f"  Q{r['quintile']}: n={r['n']:5.0f}  mean={r['mean_bps']:+7.2f}bps  t={r['t_stat']:+.2f}")
    ...
    # Regime stats
    print()
    print("=== Regime stats (prior-day RS > median = alert) ===")
    ...
    # Timing overlay
    print()
    print("=== Timing overlay (go to cash when top 20% RS alert) ===")
