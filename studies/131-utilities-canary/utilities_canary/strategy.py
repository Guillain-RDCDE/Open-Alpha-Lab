"""The strategy and its honest controls — Study 131 (Utilities-Canary).

The folk recipe: when XLU outperforms SPY on a relative-strength basis (rising
XLU/SPY RS or its momentum), exit or reduce equity exposure because a drawdown is
coming.  We test this three ways:

1. **Quintile-sorted forward returns** — sort days by the rolling percentile rank of
   RS momentum into five buckets and compute forward 1-day / 5-day / 21-day SPY log-
   returns per bucket.  The canary claim predicts a *monotone descent* from bottom
   (SPY outperforming, risk-on) to top (XLU outperforming, risk-off → bad outcomes).
...
2. **Binary timing overlay** — a "risk-off" signal that is *flat* (cash) when the
   prior-day RS momentum is in the top quintile (XLU strongly outperforming), and
   long SPY otherwise.  The fair baseline is unconditional buy-and-hold.  Inference:
   HAC t-stat on the active-minus-passive daily return spread.

3. **Regime stats** — SPY statistics conditional on being in a "canary alert" regime
   (RS momentum > rolling median) vs a "calm" regime.

No look-ahead: the RS momentum at close of day *t* forms the signal; the forward
return starts at the close of day *t+1*.
"""
...
def rs_momentum(df: pd.DataFrame, window: int = 20) -> pd.Series:
    """20-day change in the log(XLU/SPY) relative strength ratio.

    Positive means XLU has been outperforming SPY over the past ``window`` days —
    the "canary is singing" state the folk signal uses.
    """
    rs = np.log(df["XLU_close"]) - np.log(df["SPY_close"])
...
def rs_momentum_rank(
    ...
    """Rolling out-of-sample percentile rank of RS momentum (0–1).

    A high rank (close to 1) means XLU has been outperforming SPY more aggressively
    than at almost any point in the prior year — the "strong canary" state.
    """
...
def forward_return(spy_close: pd.Series, horizon: int = 1) -> pd.Series:
    ...
    Signal at day *t* uses the RS momentum known at the close of *t*; the forward
...
def canary_quintile(
    ...
    """Rolling canary quintile rank (1 = SPY outperforming, 5 = XLU outperforming strongly).
...
def quintile_table(
    ...
    """Forward-return statistics for each canary quintile, across multiple horizons.

    Parameters
    ----------
    df : DataFrame with columns ``[XLU_close, SPY_close]`` and a DatetimeIndex.
    horizons : list of forward-return horizons in calendar days (default [1, 5, 21]).
    rs_window : lookback window for RS momentum (default 20 trading days).
    ...
    Quintile 1 = SPY outperforming (risk-on); quintile 5 = XLU outperforming (risk-off).
    The folk claim predicts mean_bps[Q5] < mean_bps[Q1] and a monotone descent.
    """
    if horizons is None:
        horizons = [1, 5, 21]
    rs_mom = rs_momentum(df, window=rs_window)
    q = canary_quintile(rs_mom, window=rank_window, min_periods=min_periods)
    rows = []
    for h in horizons:
...
def top_minus_bottom(
    ...
    """HAC t-stat on the spread: mean(fwd | Q1 calm) − mean(fwd | Q5 canary alert).

    This is the central test.  If utilities leading down genuinely forecasts weak SPY
    returns, the Q5 ("strong canary") forward returns should be materially lower than
    Q1 ("SPY leading, risk-on") forward returns.

    Note the sign convention: Q1 − Q5 (positive means the calm regime outperforms
    the alert regime, consistent with the folk claim).

    Returns a dict with keys
    ``{n_top, n_bot, mean_top_bps, mean_bot_bps, spread_bps, t_spread, t_top, t_bot}``.
    Here "top" = Q5 (XLU outperforming) and "bot" = Q1 (SPY outperforming).
    """
    rs_mom = rs_momentum(df, window=rs_window)
    q = canary_quintile(rs_mom, window=rank_window, min_periods=min_periods)
    fwd = forward_return(df["SPY_close"], horizon=horizon)

    # Q5 = "canary alert" (XLU strongly outperforming)
...
# ---------------------------------------------------------------------------
# Binary timing overlay: go to cash when canary is sounding
# ---------------------------------------------------------------------------
...
def timing_overlay(
    ...
    """Daily return of the utilities-canary timer vs unconditional buy-and-hold.

    The timer is *flat* (in cash, earns 0) when the prior-day RS momentum rank exceeds
    ``top_pct`` (default 80th percentile — "strong canary alert"), and *long SPY*
    otherwise.  The baseline is always long SPY.  The 'active minus passive' spread is
    the edge to test.  ``cost_bps`` is charged on each *switch* as a one-way round-trip.
...
def regime_stats(
    ...
    Alert = prior-day RS momentum rank > ``threshold`` (default 50th percentile).
    ...
    for label, mask in [
        ("full sample", valid),
        ("calm (RS Q1-Q2)", valid & ~signal.fillna(True)),
        ("alert (RS Q3-Q5)", valid & signal.fillna(False)),
    ]:
