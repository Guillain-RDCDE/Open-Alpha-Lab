"""Data layer for Study 963 — The Half Day.

The tape has to carry **volume**, not just prices: the whole premise of this study
is that an early close is a session with the liquidity taken out of it, and the only way
to see that in a daily bar file — and the only way to *confirm* that a date really was a
1 p.m. close rather than a date the calendar rule guessed — is the volume column.
...
# Five liquid tapes that all trade the shortened session: US large cap, US tech,
# US small cap, long Treasuries and gold. Small caps and gold are here because the
# "nobody is trading" story should bite *hardest* where depth is thinnest.
...
def load_ohlc(ticker: str, cache_dir: str = DEFAULT_CACHE,
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            ...
            f"Call half_day.data.fetch() once to populate the shared cache."
        )
