"""Reproducible headline run for Study 803 — Realized-Skewness Reversal.
...
print("# Realized-Skewness Reversal — do high-realized-skew names go on to earn less?")
...
for w in (21,):
    sp = st.skew_spreads(ret, window=w, frac=0.3)
    h = st.skew_stats(sp)
    print(f"\nsort: trailing-{w}d realized skew, long bottom30% / short top30%, "
...
sp = st.skew_spreads(ret, window=21, frac=0.3)
...
null_t = []
for s_ in range(20):
    p0 = data.synthetic_panel(edge=0.0, seed=803 + s_, n_assets=40, n_days=1200)
...
p1 = data.synthetic_panel(edge=0.0016, seed=803, n_assets=40, n_days=1500)
sy = st.synthetic_detect(p1)
print(f"  planted (edge=0.0016, seed 803): spread NW t = {sy['t_nw']:+.2f}, "
