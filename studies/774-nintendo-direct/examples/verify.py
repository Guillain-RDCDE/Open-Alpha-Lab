"""Reproducible headline run for Study 774 — Nintendo-Direct.

Prints every number quoted in docs/results.md and frozen into the ``R`` dict in
notebooks/build_notebooks.py. Deterministic; uses the cached NTDOY / SPY tapes under
...
print("# Nintendo-Direct — does NTDOY rally into a Nintendo Direct broadcast?")

print(f"calendar: {len(dt.EVENTS)} flagship general/E3 Nintendo Directs "
      f"{dt.EVENTS[0][0][:4]}->{dt.EVENTS[-1][0][:4]}, hardcoded from Wikipedia / Nintendo "
      f"Life (2020 absent: no general Direct that year)")

if not dt.have_real():
    print("(cache miss — fetching NTDOY + SPY once)")
    dt.fetch()

prices = dt.load_real()
panel = pd.DataFrame({t: s for t, s in prices.items()}).dropna()
print(data_stamp("Nintendo-Direct panel (NTDOY + SPY, adjusted/total-return)", panel, asof=dt.AS_OF))

ev = st.build_event_table(prices, cost_bps=5.0)
ev10 = st.build_event_table(prices, cost_bps=10.0)
inc, inc10 = ev[ev["included"]], ev10[ev10["included"]]
print(f"\nevents resolved: {len(inc)} of {len(dt.EVENTS)} Directs have NTDOY+SPY coverage "
      f"around the anchor")
...
print("\n# THE FADE (the 'sell the news') — abnormal return, K sessions AFTER the Direct")
...
print("\n# TRADABILITY — the run-up net of costs (NB: Directs announced ~1-3 days ahead, "
      "so the exact entry date is NOT truly known 2 weeks out)")
...
for bump in (0.01, 0.02):
    planted = st.synthetic_detect(bump=bump, seed=778, k=10)
    print(f"  planted run-up bump=+{bump*100:.1f}% (seed 778): mean AR {planted['mean']*100:+.3f}%  "
