"""Tests for strategy.py — HAC inference, baseline construction, positive control,
...
# ---------------------------------------------------------------------------
# Helpers — build a tiny synthetic price panel for baseline tests
# ---------------------------------------------------------------------------

def _make_minimal_price_panel(seed: int = 0) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    idx = pd.bdate_range("2018-01-03", periods=1000, name="date")
    prices = {}
    for tkr in ("AAA", "BBB", "CCC"):
        log_ret = rng.normal(0.0, 0.015, len(idx))
        prices[tkr] = pd.Series(50.0 * np.exp(np.cumsum(log_ret)), index=idx)
...
def test_summarize_horizon_win_rate_in_range(null_events):
    events, _ = null_events
...
# ---------------------------------------------------------------------------
# Positive control — negative drift is planted and recovered
# ---------------------------------------------------------------------------

def test_negative_drift_shows_lower_returns(null_events, planted_events):
    """drift_bps=-20 should produce materially lower 3m mean return than drift=0."""
...
def test_rs_vs_random_edge_planted_below_null():
    ...
    p_mean, n_mean = st.rs_vs_random_edge(ev_planted, ev_null, col="ret_3m")
...
def test_baseline_has_is_rs_false():
    prices = _make_minimal_price_panel()
    events = _make_events_for_panel(prices)
    base = st.build_random_baseline(events, prices, horizon_days=(21, 63, 126, 252))
    if not base.empty:
        assert (base["is_rs"] == False).all()  # noqa: E712


def test_baseline_ret_columns_present():
    prices = _make_minimal_price_panel()
    events = _make_events_for_panel(prices)
    base = st.build_random_baseline(events, prices)
...
def test_compare_horizons_output_shape(null_events, planted_events):
    ...
    assert len(result) == 4
    assert "tstat_rs" in result.columns
...
def test_cost_adjusted_return_reduces_net():
    ...
    cost_net = st.cost_adjusted_return(0.05, 63, cost_bps=10.0)
    assert cost_net < gross_net


def test_cost_adjusted_negative_gross_is_negative():
    """A large negative gross return (typical for RS names) should stay negative."""
    net = st.cost_adjusted_return(-0.20, 126, cost_bps=10.0)
    assert net < 0
