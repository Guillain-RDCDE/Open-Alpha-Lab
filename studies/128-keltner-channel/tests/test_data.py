...
def test_synthetic_is_business_days(flat):
    bars, _ = flat
    # All index entries must be business-day (weekday 0-4).
...
def test_momentum_creates_autocorrelation():
    ...
    flat, _ = data.synthetic_daily(n_days=1000, momentum=0.0, seed=128)
    trend, _ = data.synthetic_daily(n_days=1000, momentum=0.30, seed=128)
    ac = lambda s: np.corrcoef(s[:-1], s[1:])[0, 1]
    r_flat = np.diff(np.log(flat["close"].to_numpy()))
    r_trend = np.diff(np.log(trend["close"].to_numpy()))
    assert abs(ac(r_flat)) < 0.07
