"""Data for the January-Barometer study — an offline synthetic world, and the cached S&P tape.
  ...
  * :func:`fetch_index` — the real **S&P 500** (^GSPC), daily from Yahoo resampled to month-end total
    return, **cache-first** (network only on ``fetch=True``). The fingerprinted real run is in
    ``docs/results.md``.
"""
...
def fetch_index(cache_dir: str = DEFAULT_CACHE, fetch: bool = False, start_year: int = 1950) -> pd.Series:
    """Monthly total-return of the S&P 500 (^GSPC), cache-first.

    **Cache-only** unless ``fetch=True`` (downloads daily from Yahoo and resamples to month-end —
    daily ^GSPC reaches back to 1927, unlike the monthly endpoint). Returns a monthly simple-return
    Series from ``start_year``, or an empty Series on a cache miss with ``fetch=False``.
    """
