"""Reproducible headline run for Study 455 — Rising/Falling Three Methods.
...
from three_methods import data, strategy as st  # noqa: E402
...
print("# Three Methods — mechanical rising/falling continuation on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    ...
    # pooled long_share for the matched random baseline
    all_dirs = []
    for t in data.DEFAULT_TICKERS:
        _, d = st.pattern_entries(_load(t))
        all_dirs.append(d)
    all_dirs = np.concatenate(all_dirs) if all_dirs else np.array([])
    long_share = float((all_dirs > 0).mean()) if all_dirs.size else 1.0
    print(f"pooled long share: {long_share:.2f}  (rising vs falling mix)")

    print("\n# Pooled three-methods vs drift-matched random baseline (signed by direction)")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            ...
            rr.append(st.forward_returns(b, rd, rdirs, h))
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total three-methods patterns (H={st.HORIZONS[0]}): {total}")
    ...
    for t in data.DEFAULT_TICKERS:
        ...
        sr = st.summarize(st.forward_returns(b, rd, rdirs, 20))
        nlong = int((dirs > 0).sum()); nshort = int((dirs < 0).sum())
        print(f"  {t}: ent={len(e):>3} ({nlong}L/{nshort}S) patt={s['mean_bps']:+7.1f}bps "
              f"t={s['t']:+5.2f} rnd={sr['mean_bps']:+7.1f}bps delta={s['mean_bps']-sr['mean_bps']:+7.1f}")

    print("\n# Shuffled-date GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.shuffled_body_placebo(_load("SPY"), 20, n_draws=500, seed=455)
...
for edge in (0.0, 1.00):
    ...
    e, dirs = st.pattern_entries(px)
    s = st.summarize(st.forward_returns(px, e, dirs, 20))
    print(f"  edge={edge:.2f}: n={s['n']:>4}  patt={s['mean_bps']:+7.1f}bps  "
