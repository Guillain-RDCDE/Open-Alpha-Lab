...
the deferred-revenue long-short lights up *only when a lead is planted*."""
...
from deferred_revenue import strategy as st  # noqa: E402
...
def test_newey_west_reduces_to_iid_at_zero_lags():
    ...
    # with lags=0 the HAC se is just gamma0/n -> matches a population-sd one-sample t
...
def test_newey_west_widens_se_under_positive_autocorr():
    ...
    for i in range(1, 2000):
        x[i] = 0.6 * x[i - 1] + e[i]      # strongly autocorrelated
    x += 0.1
...
def test_calendar_ls_recovers_planted_lead(planted_panel):
    ...
    ls = st.calendar_ls(prices, ev, staleness_days=120)
    stt = st.calendar_ls_stats(ls)
    assert stt["mean_bps"] > 0               # high-growth beats low-growth
...
# leads-sales axis
# --------------------------------------------------------------------------- #
def test_leads_sales_planted_correlation():
    import pandas as pd
    rng = np.random.default_rng(5)
    n = 400
    g = rng.normal(0, 0.25, n)
    fut = 0.8 * g + rng.normal(0, 0.05, n)          # deferred growth truly leads sales
    ev = pd.DataFrame({"defrev_yoy": g, "next_rev_yoy": fut})
    out = st.leads_sales(ev)
    assert out["slope"] > 0 and out["t"] > 5.0
    assert out["spread"] > 0


def test_leads_sales_no_relation():
    import pandas as pd
    rng = np.random.default_rng(6)
    n = 400
    ev = pd.DataFrame({"defrev_yoy": rng.normal(0, 0.25, n),
