"""Strategy + inference for Study 798 — Deferred-Revenue Signal.

The claim: **growth in deferred revenue / contract liabilities leads future sales and forward
returns.** Deferred revenue is billed-but-unrecognised subscription revenue sitting on the
balance sheet; a swelling balance is supposed to front-run the income statement and the stock.
...
1. **Forward returns — the calendar-time long-short (PRIMARY).** Each month-end, rank the names
   that carry a fresh deferred-revenue-YoY-growth signal into terciles; go long the top, short
...
2. **Forward returns — the pooled event drift (CROSS-CHECK).** The 534-style pooled sort: bucket
   all (ticker, filing) events by the signal, measure the top-minus-bottom forward drift over a
   horizon, one-sample t + a label-shuffle placebo.

3. **Leads future sales? (3rd axis).** Does this quarter's deferred-revenue growth predict NEXT
   quarter's YoY revenue growth? A pooled OLS slope with a t-stat, plus the tercile spread in
   future sales growth. This is the mechanism check: even if the stock doesn't move, does the
   accounting lead actually exist?
...
(current-survivors basket). Given the thin, uneven panel, expect the honest answer to be modest.
"""
...
def calendar_ls(prices: pd.DataFrame, events: pd.DataFrame, signal_col: str = "defrev_yoy",
...
def event_drift_frame(prices: pd.DataFrame, events: pd.DataFrame,
    ...
    for _, r in events.iterrows():
        ...
        if pos <= 0 or exit_ >= len(idx):
            continue
        out.append({"ticker": tk, "filed": idx[pos], "signal": float(r["defrev_yoy"]),
...
# --------------------------------------------------------------------------- #
# 3rd axis — does deferred-revenue growth lead future sales?
# --------------------------------------------------------------------------- #
def leads_sales(events: pd.DataFrame) -> dict:
    """Does this quarter's deferred-revenue YoY growth predict NEXT quarter's revenue YoY growth?

    Pooled OLS: next_rev_yoy = a + b · defrev_yoy. Reports the slope b, its (iid) t, the R²,
    the Pearson correlation, and the future-sales-growth spread between the top and bottom
    deferred-growth terciles. A positive, significant b is the accounting lead the whole thesis
    rests on. (t is iid-pooled — events cluster by quarter, so read it as suggestive, not a
    calendar-robust HAC statistic.)
    """
    fr = events.dropna(subset=["defrev_yoy", "next_rev_yoy"]).copy()
    x = fr["defrev_yoy"].to_numpy(); y = fr["next_rev_yoy"].to_numpy()
    n = len(x)
    if n < 10:
...
def synthetic_detect(prices: pd.DataFrame, events: pd.DataFrame) -> dict:
    ...
    ls = calendar_ls(prices, events, signal_col="defrev_yoy", n_buckets=3, min_names=6,
                     staleness_days=120)
