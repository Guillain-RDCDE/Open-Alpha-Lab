"""Reproducible headline run for Study 798 — Deferred-Revenue Signal.
...
from deferred_revenue import data, strategy as st  # noqa: E402

print("# Deferred-Revenue Signal — does deferred-revenue growth lead future sales and returns?")
...
print("\n# PRIMARY — calendar-time tercile long-short on deferred-revenue YoY growth")
...
ls120 = st.calendar_ls(px, ev, signal_col="defrev_yoy", n_buckets=3, min_names=6, staleness_days=120)
...
print(f"  asset-scaled signal (dDefRev/Assets): mean {s_sc['mean_bps']:+.1f} bps, "
...
print("\n# ERA SPLIT — calendar long-short pre/post 2019")
for lbl, lo, hi in [("2010-2018", "2000-01-01", "2019-01-01"),
                    ("2019-2026", "2019-01-01", "2100-01-01")]:
    sub = ls[(ls.index >= lo) & (ls.index < hi)]["ls"].to_numpy()
    print(f"  {lbl}: n={len(sub)}  mean {sub.mean()*1e4:+.1f} bps  NW t = {st.newey_west_t(sub):+.2f}")

print("\n# THIRD AXIS — does deferred-revenue growth LEAD future (next-quarter) sales?")
q = st.leads_sales(ev)
print(f"  n={q['n']}  slope={q['slope']:+.3f}  t={q['t']:+.2f}  R2={q['r2']:.3f}  corr={q['corr']:+.3f}")
print(f"  future sales growth: top deferred-growth tercile {q['top_sales']*100:+.1f}%  "
      f"vs bottom {q['bot_sales']*100:+.1f}%  ->  spread {q['spread']*100:+.1f} pp")
...
print(f"  planted edge=0.15 (seed 798): mean {pl['mean_bps']:+.1f} bps/mo, NW t = {pl['t_nw']:+.2f}")
