...
def test_synthetic_uses_business_days(coin):
    bars, _ = coin
    # All days are weekdays (Mon=0 .. Fri=4); no Saturday (5) or Sunday (6).
...
def test_momentum_creates_autocorrelation():
    ...
    flat, _ = data.synthetic_daily(n_days=800, momentum=0.0, seed=125)
    trend, _ = data.synthetic_daily(n_days=800, momentum=0.20, seed=125)
