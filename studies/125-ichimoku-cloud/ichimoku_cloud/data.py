...
- ``fetch_daily`` — the real Yahoo! daily tape (``yfinance``), cache-only by default so
  ...
  long (years of data), giving the Ichimoku system's Tenkan(9)/Kijun(26) enough
  warmup and enough independent flips for meaningful inference.

Ichimoku Kinko Hyo uses standard settings: Tenkan-sen period 9, Kijun-sen period 26,
Senkou Span B period 52, and a 26-period forward displacement for the Kumo (cloud).
The *no look-ahead* discipline: the cloud at bar *t* is computed only from data up
to and including bar *t* (the forward-displaced cloud is shifted back conceptually —
we never use the cloud position at bar *t+1*+26; we use the cloud value that was
published 26 bars ago, i.e. computed from bar *t-26*). See ``strategy.py``.
...
def synthetic_daily(
    ...
    - ``momentum = 0``   → a martingale; the Ichimoku signal is a fair coin.
    - ``momentum > 0``   → return persistence the trend system can ride.
    - ``momentum < 0``   → mean reversion the Ichimoku is on the *wrong* side of.
    ...
    The synthetic tape is intentionally long (n_days=1000 by default) because the
    Ichimoku system needs at least 52 bars of warmup (Senkou Span B) plus the 26-bar
    cloud-displacement lag before a usable signal can fire.
    """
    ...
    vol = rng.integers(500_000, 50_000_000, close.size).astype(float)
...
def fetch_daily(
    ...
    as a parquet under ``_cache/``). The daily tape is long — ten years of history
    by default — giving the Ichimoku system's 52-bar warmup and 26-bar cloud
    displacement lag enough history before the first usable signal can fire.
    """
