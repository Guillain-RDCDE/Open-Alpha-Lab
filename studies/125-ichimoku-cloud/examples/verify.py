"""Real-tape verification — Study 125 (Ichimoku-Cloud). Regenerates docs/results.md numbers.

Fetches (or reads from cache) the four daily tapes, runs the Ichimoku Kinko Hyo composite
signal (Tenkan 9 / Kijun 26 / Senkou B 52 / displacement 26) — bullish when price is above
the cloud and Tenkan > Kijun, bearish when below the cloud and Tenkan < Kijun — with symmetric
±1 ATR barriers against a random-direction control, sweeps costs, and shows the fixed-day
forward return at 5/20 day horizons. Network only with --fetch.

    python studies/125-ichimoku-cloud/examples/verify.py            # cache-only
    python studies/125-ichimoku-cloud/examples/verify.py --fetch    # refresh the tapes
"""
...
def main(fetch: bool) -> None:
    ...
    rand = st.summarize(_pool(False, 1, 1, 0, seed=125), "ret_gross")
    print("=== honest symmetric ±1 ATR (gross) ===")
    print(f"SIGNAL n={cross['n_trades']} win={cross['win_rate']:.3f} "
    ...
    print(f"Delta (signal - random): {cross['mean_bps'] - rand['mean_bps']:+.2f} bps")

    print("\n=== per-instrument breakdown (signal, symmetric, gross) ===")
    for t in TICKERS:
    ...
    print("\n=== cost sweep (signal arm, symmetric ±1 ATR, net) ===")
    ...
    print("\n=== fixed-day forward returns (d5 / d20, signal gross) ===")
    for hd in (5, 20):
        frames = []
        for t in TICKERS:
            bars = data.fetch_daily(t, fetch=False)
