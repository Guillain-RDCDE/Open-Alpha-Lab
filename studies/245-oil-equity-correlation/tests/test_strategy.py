...
from oil_equity_correlation import strategy as st  # noqa: E402
...
def test_compute_features_no_lookahead(null_tape):
    ...
    assert not feat["ret_oil"].isnull().all()
    assert len(feat) > 10


def test_compute_features_weekly_vs_monthly(null_tape):
    daily, _ = null_tape
...
def test_oos_r2_positive_when_signal_planted(signal_tape):
    ...
    result_sig = st.oos_r2(feat_sig["ret_oil"], feat_sig["fwd_equity"], min_train=52)
    ...
    result_null = st.oos_r2(feat_null["ret_oil"], feat_null["fwd_equity"], min_train=52)
...
def test_oos_r2_near_zero_on_null(null_tape):
    ...
    daily, _ = null_tape
    feat = st.compute_features(daily, horizon="W")
    result = st.oos_r2(feat["ret_oil"], feat["fwd_equity"], min_train=52)
...
def test_summarize_returns_all_keys(null_tape):
    ...
    for key in ("is_equity", "oos_equity", "contemp", "n_periods"):
        assert key in res
...
def test_engine_finds_signal_but_not_noise():
    ...
    signal, _ = data.synthetic_daily(n_years=20, pred_r=0.8, seed=245)

    feat_null = st.compute_features(null, horizon="W")
    feat_sig = st.compute_features(signal, horizon="W")

    res_null = st.regression_is(feat_null["ret_oil"], feat_null["fwd_equity"])
    res_sig = st.regression_is(feat_sig["ret_oil"], feat_sig["fwd_equity"])
