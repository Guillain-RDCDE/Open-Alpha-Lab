"""Tests for the strategy layer of Study 284 (Equinox-Effect).

All tests are offline and deterministic — they use the synthetic generator and
the hardcoded equinox/solstice table only.
"""
...
def test_event_day_returns_shape():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    evd = st.event_day_returns(df["ret"], ev)
    assert set(["date", "event_date", "kind", "season", "ret"]).issubset(evd.columns)
    # Most events should land inside the synthetic window.
    assert len(evd) >= len(ev) - 4


def test_event_date_is_on_or_after_event():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    evd = st.event_day_returns(df["ret"], ev, lag=0)
    assert (evd["event_date"].values >= evd["date"].values).all()
...
def test_event_window_matrix_shape():
    ...
    ev = data.equinox_table()
    win = 4
    mat = st.event_window_matrix(df["ret"], ev, win=win)
    assert list(mat.columns) == list(range(-win, win + 1))
    assert len(mat) >= len(ev) - 4
...
def test_event_stats_keys():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    r = st.event_stats(df["ret"], ev, n_permutations=200, seed=99)
    required = {
...
def test_event_stats_perm_p_is_probability():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    r = st.event_stats(df["ret"], ev, n_permutations=500, seed=7)
...
def test_event_stats_win_rate_in_range():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    r = st.event_stats(df["ret"], ev, n_permutations=200, seed=1)
...
def test_permutation_deterministic():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    r1 = st.event_stats(df["ret"], ev, n_permutations=300, seed=42)
    r2 = st.event_stats(df["ret"], ev, n_permutations=300, seed=42)
...
def test_null_has_no_signal():
    """On the null tape, the event-day abnormal return is tiny and t is small."""
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    r = st.event_stats(df["ret"], ev, n_permutations=1000, seed=284)
    assert abs(r["mean_abn_bps"]) < 30.0
    assert abs(r["t_hac"]) < 2.0


def test_planted_drag_detectable():
    """A large planted event drag is detected: negative mean, big |t|, tiny p."""
    df, _ = data.synthetic_panel(signal_bps=-300.0, seed=284)
    ev = data.equinox_table()
    r = st.event_stats(df["ret"], ev, n_permutations=1000, seed=284)
...
def test_planted_premium_detectable():
    ...
    df, _ = data.synthetic_panel(signal_bps=250.0, seed=284)
    ev = data.equinox_table()
    r = st.event_stats(df["ret"], ev, n_permutations=1000, seed=284)
...
def test_car_profile_shape():
    ...
    ev = data.equinox_table()
    win = 5
    car = st.car_profile(df["ret"], ev, win=win)
...
def test_car_is_cumsum_of_aar():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    car = st.car_profile(df["ret"], ev, win=3)
...
def test_mc_summary_shape():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    mc = st.multiple_comparisons_summary(df["ret"], ev, n_permutations=200, seed=1)
    assert "bonferroni_perm_p" in mc.columns
    # all + 2 kinds + 4 seasons = 7 slices
    assert len(mc) == 7
...
def test_summarize_keys():
    df, _ = data.synthetic_panel(signal_bps=0.0, seed=284)
    ev = data.equinox_table()
    s = st.summarize(df["ret"], ev, n_permutations=200, seed=1)
    required = {
        "n_all", "n_eqx", "n_sol", "n_season", "uncond_bps",
        "all_mean_abn_bps", "all_t_hac", "all_perm_p", "all_win_rate_pct",
        "autumn_mean_abn_bps", "autumn_t_hac", "car_window_bps",
    }
