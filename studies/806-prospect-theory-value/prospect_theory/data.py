"""Data layer for Study 806 — Prospect-Theory Value.

The claim under test (Nicholas **Barberis, Abhiroop Mukherjee & Baolian Wang** 2016,
*"Prospect Theory and Stock Returns: An Empirical Test"*): the
**cumulative-prospect-theory (TK) value** of a stock's *past* return distribution
predicts its future return **negatively**. A stock whose recent return distribution
looks like an attractive *gamble* under Tversky-Kahneman prospect theory — a
right-skewed, lottery-like tape — has a **high** TK value; prospect-theory investors
overweight that good gamble, bid the name up, and it goes on to earn **less**. A long
**low-TK** / short **high-TK** book should therefore earn a positive spread.
...
* **Synthetic world — the positive control.** A deterministic, seeded panel
  ...
  latent "gamble tilt" ``c_i`` that both (a) shapes the *skewness* of its daily
  returns — a right-skewed, lottery-like tape carries a **high** TK value — and (b)
  — only when ``edge > 0`` — depresses its forward mean return. ``edge = 0`` is the
  null world: the TK value still varies across names but carries **no** information
  about forward returns, and the sort must find nothing. ``edge > 0`` plants the
  Barberis-Mukherjee-Wang negative TK->return relation.
...
# --------------------------------------------------------------------------- #
# Synthetic world — planted negative TK->return relation (the positive control)
# --------------------------------------------------------------------------- #
def synthetic_panel(
    edge: float = 0.0,
    seed: int = 806,
    ...
    """Deterministic seeded OHLC panel with a TUNABLE planted TK->return relation.

    Each asset ``i`` carries a persistent latent "gamble tilt" ``c_i[t]`` — an AR(1)
    with autocorrelation ``factor_rho``. The tilt shapes the **skewness** of the daily
    return via a quadratic-in-shock term, so a right-skewed / lottery-like tape (high
    ``c_i``) carries a **high** cumulative-prospect-theory (TK) value; and — only when
    ``edge > 0`` — the tilt depresses the **forward mean**:

        z ~ N(0,1)
        skewed_shock = daily_vol * (z + skew_gain * c_i[t] * (z**2 - 1))
        r[i,t] = drift - edge * c_i[t] + skewed_shock

    So a high positive tilt makes returns **right-skewed / lottery-like** (a high TK
    value) *and* (with ``edge > 0``) lower-mean — the Barberis-Mukherjee-Wang pattern:
    high TK value, low forward return. ``edge = 0`` is the null: the TK value still
    varies across names but predicts nothing. Business-day index; span well below the
