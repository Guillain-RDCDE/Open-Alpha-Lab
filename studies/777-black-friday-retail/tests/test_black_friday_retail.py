"""Fully-offline, deterministic tests for Study 777 — Black-Friday-Retail.
...
def test_calendar_shape():
    # 20 Black Fridays 2006->2025, each the day after the 4th Thursday of November.
    assert len(dt.EVENTS) == 20
    years = [y for y, d in dt.EVENTS]
    assert years == list(range(2006, 2026))
    # every event is in November and lands on a Friday (weekday() == 4)
    import datetime as _dt
    for _y, d in dt.EVENTS:
        dd = _dt.date.fromisoformat(d)
        assert dd.month == 11
        assert dd.weekday() == 4          # Friday
        # it is the day after the 4th Thursday: the Thursday before it is the 4th Thursday
        thu = dd - _dt.timedelta(days=1)
        assert thu.weekday() == 3         # Thursday
        assert (thu.day - 1) // 7 == 3    # 4th Thursday of the month
...
def test_wilson_interval_brackets_point_estimate():
    lo, hi = st.wilson_interval(11, 20)
    assert lo < 11 / 20 < hi
...
def test_synthetic_null_is_quiet():
    ...
    ts = np.array([st.synthetic_detect(bump=0.0, seed=783 + s, k=10)["t"] for s in range(20)])
    assert abs(ts.mean()) < 0.75
    # honest FPR bound: at n=20 events and a 10-day AR the small-sample t is noisy, so we
...
def test_synthetic_planted_bump_lifts_t_monotonically():
    t0 = st.synthetic_detect(bump=0.0, seed=783, k=10)["t"]
    t1 = st.synthetic_detect(bump=0.01, seed=783, k=10)["t"]
    t2 = st.synthetic_detect(bump=0.02, seed=783, k=10)["t"]
    assert t0 < t1 < t2
...
def test_synthetic_fade_shows_up_post_event():
    ...
    r0 = st.synthetic_detect(bump=0.0, seed=783, k=10, side="post")
    a, b, keys = dt.synthetic_world(bump=0.0, fade=0.02, seed=783)
