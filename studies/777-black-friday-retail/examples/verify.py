"""Reproducible headline run for Study 777 — Black-Friday-Retail.

Prints every number quoted in docs/results.md and frozen into the ``R`` dict in
notebooks/build_notebooks.py. Deterministic; uses the cached XRT / SPY tapes under
...
from black_friday_retail import data as dt, strategy as st  # noqa: E402

print("# Black-Friday-Retail — does retail (XRT) rally into Black-Friday week?")

print(f"calendar: {len(dt.EVENTS)} Black Fridays {dt.EVENTS[0][0]}->{dt.EVENTS[-1][0]}, "
      f"hardcoded (day after the 4th Thursday of November; XRT began trading 2006-06-19)")

if not dt.have_real():
    print("(cache miss — fetching XRT + SPY once)")
    dt.fetch()

prices = dt.load_real()
panel = pd.DataFrame({t: s for t, s in prices.items()}).dropna()
print(data_stamp("Black-Friday panel (XRT + SPY, adjusted/total-return)", panel, asof=dt.AS_OF))

ev = st.build_event_table(prices, cost_bps=5.0)
ev10 = st.build_event_table(prices, cost_bps=10.0)
inc, inc10 = ev[ev["included"]], ev10[ev10["included"]]
print(f"\nevents resolved: {len(inc)} of {len(dt.EVENTS)} Black Fridays have XRT+SPY coverage "
      f"around the anchor")
for reason, n in ev[~ev["included"]]["reason"].value_counts().items():
    print(f"  excluded {n:2d}x {reason}")

print("\n# THE RUN-UP (the 'buy the holiday-sales trade') — abnormal return, K sessions BEFORE "
      "Black Friday, XRT minus SPY (gross)")
...
print("\n# THE FADE (the 'sell the news') — abnormal return, K sessions AFTER Black Friday")
...
for bump in (0.01, 0.02):
    planted = st.synthetic_detect(bump=bump, seed=783, k=10)
    print(f"  planted run-up bump=+{bump*100:.1f}% (seed 783): mean AR {planted['mean']*100:+.3f}%  "
