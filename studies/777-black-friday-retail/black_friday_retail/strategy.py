"""Strategy + inference for Study 777 — Black-Friday-Retail.

The claim: **retail (XRT) rallies into Black-Friday week** and fades after. Two windows,
both anchored on the real Black-Friday date (calendar-known — fixed by statute):

* **The run-up (the "buy the holiday-sales trade").** XRT's abnormal return (XRT
  total-return minus the SPY total-return benchmark) over the K trading sessions ENDING
  on the last close on/before Black Friday. Tested at K = 10 (~2 weeks) and K = 21
  (~1 month).

* **The fade (the "sell the news").** XRT's abnormal return over the K sessions STARTING
  from the Black-Friday close. Tested at K = 10 and K = 21. If the folklore is right the
  run-up is positive and the fade is negative; if both are noise, the whole "buy retail
  into Black Friday, sell the news" story is busted.

Because each Black-Friday year is a single independent event (not a daily series), the
primary statistic is a **one-sample t** of the abnormal return across events (n = the
number of years with XRT+SPY coverage). A random-window placebo (drawing many random,
non-Black-Friday K-session windows from XRT's own history vs SPY) checks whether the
observed mean sits inside or outside the sector's ordinary tracking noise.
"""
...
def build_event_table(prices: dict[str, pd.Series], cost_bps: float = COST_BPS
    ...
    """One row per Black-Friday year: run-up + post-event abnormal returns + inclusion.

    A row is INCLUDED only if XRT and SPY both have cached history covering
    ...
    for year, keydate in dt.EVENTS:
        row = dict(year=year, black_friday=keydate)
        anchor_ts = pd.Timestamp(keydate)
        on_or_before = common[common <= anchor_ts]
        if len(on_or_before) == 0:
            row.update(included=False, reason="no XRT/SPY coverage at the anchor")
            rows.append(row)
            continue
        ...
        def ar(i_start, i_end):
            r_a = xrt.loc[common[i_end]] / xrt.loc[common[i_start]] - 1.0
...
def placebo_pvalue(events: pd.DataFrame, prices: dict[str, pd.Series], col: str,
                   ...
                   n_seeds: int = 20, n_draws_per_seed: int = 200, base_seed: int = 783,
                   tail: str = "right") -> dict:
    """For each INCLUDED event, redraw a random (non-Black-Friday) k-session window on XRT
    ...
    for s in range(n_seeds):
        ...
        for _ in range(n_draws_per_seed):
            ...
            for _e in range(n_events):
                ...
                r_a = xrt.loc[d_end] / xrt.loc[d_start] - 1.0
...
def car_path(events: pd.DataFrame, prices: dict[str, pd.Series],
    ...
    """Mean cumulative abnormal return (XRT - SPY) at each offset from -pre..+post
    relative to Black Friday, normalised so offset 0 = 0%, averaged across all INCLUDED
    ...
    for _, row in inc.iterrows():
        ...
        base_a, base_s = xrt.loc[common[p]], spy.loc[common[p]]
        vals = []
        for o in offsets:
            d = common[p + o]
            r_a = xrt.loc[d] / base_a - 1.0
...
def synthetic_detect(bump: float, seed: int, k: int = PRE_SHORT_K,
    ...
    pre-Black-Friday bump. ``side='pre'`` measures the run-up window [p-k..p); ``side='post'``
    measures [p..p+k)."""
