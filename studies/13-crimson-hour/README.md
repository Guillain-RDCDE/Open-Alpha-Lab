...
| **Tradability** — could you get paid? | 🔴 `MIRAGE` | The only real edge is ~6 pp of directional tilt on a full-afternoon hold — a *bias on the close's sign*, not an entry. It doesn't survive a round-trip spread plus a sensible target. The newsletter's own hedge — *"bias, not a trade"* — is the honest read. |
...
| **[02_for_the_quants](notebooks/02_for_the_quants.ipynb)** | quants | the full machinery: the mechanical/forecast decomposition, the two-proportion + Fisher test of the IB increment, Wilson + beta-binomial on 22/25, and the forking-paths Monte-Carlo |
