"""The strategy and its honest controls — Study 111 (VIX-Term-Structure).

The claim: the VIX term-structure slope (log(VIX3M / VIX)) predicts SPY forward returns.
Contango (slope > 0, VIX < VIX3M) signals a calm, risk-on market; backwardation
(slope < 0) signals stress and predicts weaker forward returns.  We test this in three
dimensions:
...
def contango_mask(slope: pd.Series) -> pd.Series:
    """Boolean mask: True when the term structure is in contango (slope > 0)."""
    return slope > 0
...
def top_minus_bottom(
    ...
    This is the central test of the claim.  If VIX term-structure slope predicts
    returns, Q5 (steepest contango) should deliver better forward returns than Q1
    (deepest backwardation).  Returns a dict with keys
...
def timing_overlay(
    ...
    """Daily return of the VIX-slope timer vs unconditional buy-and-hold.
