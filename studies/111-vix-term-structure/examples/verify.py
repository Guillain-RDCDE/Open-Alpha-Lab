"""Real-tape verification — Study 111 (VIX-Term-Structure). Regenerates docs/results.md numbers.

Fetches (or reads from cache) the VIX, VIX3M, and SPY daily tape, computes the quintile
...
Network is touched only with --fetch.
    ...
    python studies/111-vix-term-structure/examples/verify.py            # cache-only
    python studies/111-vix-term-structure/examples/verify.py --fetch    # refresh the tape
"""
...
def main(fetch: bool) -> None:
    ...
    if fetch:
        print(f"^VIX/^VIX3M/SPY  {df.index[0].date()}..{df.index[-1].date()}  fp={data.fingerprint(df)}")
        print()
