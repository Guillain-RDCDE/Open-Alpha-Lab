"""Shareholder-yield long-short, scored annually.

Shareholder yield = dividend yield + net buyback yield.
The claim (Faber 2007; Boudoukh et al. 2007): high total yield predicts outperformance.
We sort on total yield, go long the top-quintile and short the bottom-quintile, and measure the
annual hedge vs next-year returns.
"""
...
def quantile_hedge(signal: pd.DataFrame, fwd_ret: pd.DataFrame,
                   q: float = 0.2, long_high: bool = True) -> pd.DataFrame:
    """Annual long-``q`` / short-``q`` hedge on a (year x ticker) ``signal`` vs next-year returns.

    For shareholder yield use ``long_high=True`` (long the HIGH-yield names).
    Returns a DataFrame with columns: ['high', 'low', 'hedge'] indexed by year.
    """
    rows = {}
    for y in signal.index:
        ...
        hi_idx = s[s >= s.quantile(1.0 - q)].index
        lo_idx = s[s <= s.quantile(q)].index
        rh = nxt.reindex(hi_idx).dropna().mean()
        rl = nxt.reindex(lo_idx).dropna().mean()
...
def summary(annual_returns: pd.Series) -> dict:
    """Key statistics for a series of annual hedge (or leg) returns."""
