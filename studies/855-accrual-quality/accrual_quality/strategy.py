"""Strategy + inference for Study 855 — Accrual Quality (Dechow-Dichev).

The claim: earnings whose accruals map **poorly** into operating cash flows are low-quality —
less persistent, and (the strong trading form) discounted, so a book **long high-quality (low
DD residual vol) / short low-quality (high residual vol)** should earn a positive spread.

We test three things on the point-in-time accrual-quality signal (known at the 10-Q/10-K
filing date; the DD rolling window never uses a lead-CFO term that is not yet public — zero
look-ahead):

1. **Forward returns — the calendar-time long-short (PRIMARY).** Each month-end, rank the names
   that carry a fresh accrual-quality signal into terciles by ``quality`` (= -residual vol); go
   long the top (high quality), short the bottom (low quality), equal-weight; earn the NEXT
   month's return (one execution lag). The decisive number is the **Newey-West (HAC) t** of that
   monthly long-short return series.

2. **Forward returns — the pooled event drift (CROSS-CHECK).** Bucket all (ticker, filing)
   events by ``quality``, measure top-minus-bottom forward drift over a horizon, one-sample t +
   a label-shuffle placebo, and the tercile monotonicity picture.

3. **Does poor accrual quality flag less-persistent earnings? (3rd axis).** DD's core validity
   claim: low-quality accruals go with noisier, less-persistent earnings. We compare the
   ROA persistence slope (next-quarter ROA on current ROA) and the earnings volatility between
   the high-quality and low-quality terciles. This is the mechanism check — it can hold even
...
(current-survivors basket). Given the thin, uneven panel, expect a modest honest answer.
"""
...
def calendar_ls(prices: pd.DataFrame, events: pd.DataFrame, signal_col: str = "quality",
    ...
    top (high ``quality`` = low residual vol), short the bottom (equal-weight). The position is
...
def event_drift_frame(prices: pd.DataFrame, events: pd.DataFrame,
                      horizon: int, lag: int = 1, signal_col: str = "quality") -> pd.DataFrame:
...
# --------------------------------------------------------------------------- #
# 3rd axis — does poor accrual quality flag less-persistent, noisier earnings?
# --------------------------------------------------------------------------- #
def _persistence_slope(x: np.ndarray, y: np.ndarray) -> tuple[float, float]:
    """OLS slope + iid t of y = a + b·x (b = ROA persistence)."""
    ...
    return float(beta[1]), t


def persistence_by_quality(events: pd.DataFrame, n_buckets: int = 3) -> dict:
    """Do low-quality (high residual-vol) names have less-persistent, noisier earnings?

    Split events into terciles by ``aq_vol`` (high vol = poor quality). Within the *good-quality*
    (low-vol) and *poor-quality* (high-vol) terciles, regress next-quarter ROA on current ROA —
    the persistence slope b. DD's validity claim: good-quality earnings persist more
    (higher b) and are less volatile. Also reports mean earnings volatility per tercile.
    """
    fr = events.dropna(subset=["aq_vol", "roa", "roa_next"]).copy()
    n = len(fr)
    if n < 30:
        return {"n": n, "good_slope": float("nan"), "good_t": float("nan"),
                "poor_slope": float("nan"), "poor_t": float("nan"),
                "good_earn_vol": float("nan"), "poor_earn_vol": float("nan"),
                "slope_gap": float("nan")}
    b = _bucketize(fr["aq_vol"].to_numpy(), n_buckets)
    good = fr[b == 0]                # lowest residual vol = best quality
    poor = fr[b == n_buckets - 1]    # highest residual vol = worst quality
    gs, gt = _persistence_slope(good["roa"].to_numpy(), good["roa_next"].to_numpy())
    ps, pt = _persistence_slope(poor["roa"].to_numpy(), poor["roa_next"].to_numpy())
    gev = float(np.nanmean(good["earn_vol"].to_numpy()))
    pev = float(np.nanmean(poor["earn_vol"].to_numpy()))
    return {"n": n, "good_slope": gs, "good_t": gt, "poor_slope": ps, "poor_t": pt,
            "good_earn_vol": gev, "poor_earn_vol": pev, "slope_gap": gs - ps}
...
def synthetic_detect(prices: pd.DataFrame, events: pd.DataFrame) -> dict:
    ...
    ls = calendar_ls(prices, events, signal_col="quality", n_buckets=3, min_names=6,
                     staleness_days=120)
