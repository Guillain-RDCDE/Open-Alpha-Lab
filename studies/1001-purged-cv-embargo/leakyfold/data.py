"""Data layer for Study 1001 — The Leaky Fold.

A long daily tape is all this needs. The features are built from the price series
itself, deliberately: the leakage this study measures does not require exotic data, only
overlapping labels and a validation scheme that ignores time.
...
# One long tape is enough — the subject is the validation scheme, not the asset.
EQUITY = "SPY"
NASDAQ = "QQQ"
BONDS = "TLT"
GOLD = "GLD"
CASH = "BIL"
TICKERS = (EQUITY, NASDAQ, BONDS, GOLD, CASH)
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call leakyfold.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1001) -> tuple[pd.DataFrame, dict]:
