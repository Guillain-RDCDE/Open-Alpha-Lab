"""Reproducible headline run for Study 473 — Balance of Power.
...
from balance_of_power import data, strategy as st  # noqa: E402
...
print("# Balance of Power — mechanical smoothed-BOP up-cross on 5 indices/ETFs (yfinance daily)")
if data.have_real():
    spy = _load("SPY")
    span = (spy.index[-1] - spy.index[0]).days / 365.25
    print(f"window   : {spy.index[0].date()} -> {spy.index[-1].date()}  ({span:.1f} years)")
    print(f"SPY fp   : {data.fingerprint(spy)}   tickers: {data.DEFAULT_TICKERS}   smooth={SMOOTH}")

    print("\n# Pooled smoothed-BOP up-cross vs drift-matched random baseline")
    ...
    for h in st.HORIZONS:
        ...
        for t in data.DEFAULT_TICKERS:
            b = _load(t)
            c = b["close"]
            e = st.bop_cross_entries(b, smooth=SMOOTH)
        ...
        print(f"  {h:>3} {s['n']:>5} {s['mean_bps']:>10.1f} {s['win']*100:>5.0f} {s['t']:>8.2f} "
              ...
              f"{wt:>8.2f} {wp:>6.3f}")
    print(f"  total smoothed-BOP up-crosses (H={st.HORIZONS[0]}): {total}")
    ...
    print("\n# Sign-scramble GEOMETRY placebo (SPY, H=20, 500 draws)")
    pl = st.scramble_placebo(_load("SPY"), 20, smooth=SMOOTH, n_draws=500, seed=473)
    print(f"  observed {pl['obs']*1e4:+.1f} bps   placebo p={pl['p_value']:.3f}  "
          f"(>0.05 => BOP ordering not load-bearing)")
else:
...
print("  edge=0 must NOT reach significance; planted BOP-lead must light up.")
for edge in (0.0, 1.5):
    px, _ = data.synthetic_panel(edge=edge, seed=473, n_days=4000)
    c = px["close"]
    s = st.summarize(st.forward_returns(c, st.bop_cross_entries(px, smooth=SMOOTH), 20))
