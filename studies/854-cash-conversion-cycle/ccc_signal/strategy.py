"""Strategy + inference for Study 854 — Cash Conversion Cycle.

The claim (operations-finance folklore, e.g. the CFO-magazine "working-capital scorecard"
tradition): **shortening your Cash Conversion Cycle is good, bloating it is bad.** A firm
that frees cash trapped in receivables and inventory — and stretches its payables float —
has more capital to redeploy and should out-earn; a firm whose CCC is rising is a
working-capital drag. The tradeable form is **long the CCC-shorteners, short the
CCC-bloaters**, ranked on the year-over-year change in CCC.

The ranking column ``ccc_score = −ccc_yoy_chg`` is signed so that HIGH score = FALLING CCC =
the attractive (long) side. Every function below then goes **long the top score / short the
bottom** with no special-casing, and a *wrong-sign* result (bloating CCC that predicts
*higher* returns) surfaces as a *negative* long-short — exactly what the desk's honesty rail
wants.

We test three things, each on the point-in-time signal (known at the 10-Q/10-K filing date,
so zero look-ahead):

1. **Forward returns — the calendar-time long-short (PRIMARY).** Each month-end, rank the
   names carrying a fresh CCC-change signal into terciles; long the falling-CCC third, short
   the rising-CCC third, equal-weight; earn the NEXT month's return (one execution lag). The
   decisive number is the **Newey-West (HAC) t** of that monthly long-short return series.

2. **Forward returns — the pooled event drift (CROSS-CHECK).** Bucket all (ticker, filing)
   events by the signal, measure top-minus-bottom forward drift over a horizon, one-sample t
   + a label-shuffle placebo.

3. **Does shortening the CCC precede a better margin? (3rd axis).** Does this quarter's CCC
   change predict NEXT quarter's gross-margin change? The "frees cash → out-earns" story
   implies a *negative* slope — a falling CCC (freed working capital, cheaper operations)
   precedes a widening gross margin. This is the mechanism check.
...
survivors basket). Given the thin, uneven five-fact panel, expect the honest answer to be
modest.
"""
...
SIGNAL_COL = "ccc_score"             # −(YoY change in CCC): high score = falling CCC = long
...
def calendar_ls(prices: pd.DataFrame, events: pd.DataFrame, signal_col: str = SIGNAL_COL,
    ...
    the TOP score (= falling CCC), short the BOTTOM (= rising CCC), equal-weight. The position
    is held over month M+1 (return realised then) — one documented execution lag, signal known
    at M's close. Requires ≥ ``min_names`` in the cross-section. Returns a frame indexed by the
    RETURN month with columns: ls, long, short, n, turnover.
    """
...
def calendar_ls_net(ls: pd.DataFrame, cost_bps: float = 20.0, borrow_bps_ann: float = 100.0,
    ...
    ``cost_bps`` one-way on both legs costs ``2 × cost_bps × turnover`` per month, and the
...
def event_drift_frame(prices: pd.DataFrame, events: pd.DataFrame,
    ...
    ``signal`` carries ``signal_col`` (default the signed CCC score: high = falling CCC)."""
    out = []
...
def long_short_drift(frame: pd.DataFrame, n_buckets: int = 3) -> dict:
    ...
    ``signal`` is the signed score (high = falling CCC = long), so top−bottom is the claimed
    long-shorteners / short-bloaters spread."""
...
def bucket_means(frame: pd.DataFrame, n_buckets: int = 3) -> np.ndarray:
    """Mean forward drift in each signal bucket (low score → high score, i.e. rising CCC →
    falling CCC) — the monotonicity picture."""
    s = frame["signal"].to_numpy(); d = frame["drift"].to_numpy()
    b = _bucketize(s, n_buckets)
...
# --------------------------------------------------------------------------- #
# 3rd axis — does CCC shortening precede a BETTER margin?
# --------------------------------------------------------------------------- #
def improves_margin(events: pd.DataFrame) -> dict:
    """Does this quarter's CCC change predict NEXT quarter's gross-margin change?

    Pooled OLS: next_gm_chg = a + b · ccc_yoy_chg. The "frees cash → out-earns" story predicts
    b < 0 — a falling CCC (freed working capital) precedes a *widening* gross margin. Reports
    the slope b, its (iid) t, the R², the Pearson correlation, and the future-margin-change
    spread between the SHORTENING (low CCC change) and BLOATING (high CCC change) terciles.
    (t is iid-pooled — events cluster by quarter, so read it as suggestive, not a calendar-
    robust HAC statistic.)
    """
    fr = events.dropna(subset=["ccc_yoy_chg", "next_gm_chg"]).copy()
    x = fr["ccc_yoy_chg"].to_numpy(); y = fr["next_gm_chg"].to_numpy()
    n = len(x)
    if n < 10:
        return {"n": n, "slope": float("nan"), "t": float("nan"), "r2": float("nan"),
                "corr": float("nan"), "low_margin": float("nan"), "high_margin": float("nan"),
                "spread": float("nan")}
    ...
    b = _bucketize(x, 3)                       # bucket on RAW change: 0 = shortening, 2 = bloating
    low_margin = float(y[b == 0].mean()); high_margin = float(y[b == 2].mean())
    return {"n": n, "slope": float(beta[1]), "t": t, "r2": r2, "corr": corr,
            "low_margin": low_margin, "high_margin": high_margin,
            "spread": low_margin - high_margin}
