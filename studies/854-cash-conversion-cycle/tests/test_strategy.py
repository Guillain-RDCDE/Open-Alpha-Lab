...
the CCC long-short lights up *only when a lead is planted* (falling CCC out-returns rising)."""
...
from ccc_signal import strategy as st  # noqa: E402
...
def test_calendar_ls_recovers_planted_edge(planted_panel):
    prices, ev = planted_panel
    ls = st.calendar_ls(prices, ev, staleness_days=120)
    stt = st.calendar_ls_stats(ls)
    assert stt["mean_bps"] > 0               # long falling-CCC / short rising-CCC makes money
...
# --------------------------------------------------------------------------- #
# improves-margin axis (frees-cash mechanism, expects NEGATIVE slope)
# --------------------------------------------------------------------------- #
def test_improves_margin_planted_negative_slope():
    import pandas as pd
    rng = np.random.default_rng(5)
    n = 400
    chg = rng.normal(0, 12, n)                       # CCC change in days
    fut = -0.0008 * chg + rng.normal(0, 0.01, n)     # shortening truly precedes a wider margin
    ev = pd.DataFrame({"ccc_yoy_chg": chg, "next_gm_chg": fut})
    out = st.improves_margin(ev)
    assert out["slope"] < 0 and out["t"] < -5.0
    assert out["spread"] > 0                          # shortening tercile widens margin more


def test_improves_margin_no_relation():
    import pandas as pd
    rng = np.random.default_rng(6)
    n = 400
    ev = pd.DataFrame({"ccc_yoy_chg": rng.normal(0, 12, n),
                       "next_gm_chg": rng.normal(0, 0.02, n)})
    out = st.improves_margin(ev)
