"""Reproducible headline run for Study 854 — Cash Conversion Cycle.
...
from ccc_signal import data, strategy as st  # noqa: E402

print("# Cash Conversion Cycle — does a shortening CCC out-earn, and can you trade it?")
...
print("\n# PRIMARY — calendar-time tercile long-short on the YoY change in CCC")
print("#   (long the FALLING-CCC third, short the RISING-CCC third; earn next month; one lag)")
...
ls120 = st.calendar_ls(px, ev, signal_col="ccc_score", n_buckets=3, min_names=6, staleness_days=120)
...
print(f"  pct-change signal (dCCC/CCC): mean {s_pct['mean_bps']:+.1f} bps, "
...
print(f"  monotonicity (63d terciles rising-CCC->falling-CCC): {[f'{b*100:+.2f}%' for b in bm]}")
...
print("\n# THIRD AXIS — does CCC shortening precede a BETTER margin? (expect slope < 0)")
q = st.improves_margin(ev)
print(f"  n={q['n']}  slope={q['slope']:+.5f}  t={q['t']:+.2f}  R2={q['r2']:.3f}  corr={q['corr']:+.3f}")
print(f"  next-q gross-margin change: shortening tercile {q['low_margin']*100:+.2f}pp  "
      f"vs bloating {q['high_margin']*100:+.2f}pp  ->  spread {q['spread']*100:+.2f} pp")
...
print(f"  planted edge=0.15 (seed 854): mean {pl['mean_bps']:+.1f} bps/mo, NW t = {pl['t_nw']:+.2f}")
