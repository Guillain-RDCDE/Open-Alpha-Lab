"""Strategy + inference for Study 746 — HQ-Relocation (event-study abnormal returns).

The claim: a **headquarters-relocation announcement** — especially a tax/incentive-driven
one (an inversion abroad, or a jump to a lower-tax U.S. state) — is *information*. Believers
split two ways: the *signal* camp says the market prices in the tax saving and cost cut (buy
it); the *distraction* camp says a splashy address change is management theatre that masks a
weak business (fade it). We pin both down with a textbook short-window **event study**:

    For each dated HQ-move announcement, fit a **market model** (stock = alpha + beta*SPY)
    on a clean PRE-event estimation window [-est-gap, -gap], then compute the **abnormal
    return** AR_t = r_stock,t - (alpha + beta*r_SPY,t) on the event window and cumulate it
    into a **CAR** (cumulative abnormal return).

We then ask, with a 1-day execution lag for any tradable variant:

  * Is the CAR different from zero, overall and in the **tax** vs **other** bucket?
  * Is **tax - other** different from zero? (does the *reason* for the move matter?)
  * Is there a post-announcement **drift** (the signal camp) that a lagged trade could
    harvest? (a long [+1, +N] window, entered the day after the headline)
  * A **placebo** null: random non-event windows on the same names, to size the
    small-sample sampling distribution of a CAR;
  * a **win-rate** (P[CAR > 0]) vs the 50/50 base rate;
  * **one-way costs** on the tradable drift variant.

The decisive number is not the sign of any point estimate (the folklore itself can't decide
signal vs distraction) but the **sample size** and the **drift**: with ~a dozen events per
bucket the announcement CAR is inside its own noise, and the holdable drift is statistically
zero — an HQ move is, on the tape, a **non-event**.
"""
...
DRIFT_WINDOW = (1, 63)    # post-announcement drift [+1, +63] (~one quarter), lagged entry
...
def event_car(prices: pd.DataFrame, ticker: str, announce_date: pd.Timestamp,
              ...
              lag: int = 0) -> float:
    """Market-model cumulative abnormal return (CAR) for one HQ-move event.
    ...
    (inclusive, in trading days). ``lag`` shifts the event window forward by ``lag``
    days to model a one-day execution delay (no look-ahead). Returns NaN if the windows
    do not fit the available history.
    """
...
def car_panel(prices: pd.DataFrame, events: list[dict],
    ...
    """CAR for every event in the table; returns a tidy frame with the ``tax`` flag."""
    rows = []
    for e in events:
        car = event_car(prices, e["ticker"], e["announce_date"], window=window, lag=lag)
        if not np.isfinite(car):
            continue
        rows.append({"ticker": e["ticker"], "announce_date": e["announce_date"],
                     "tax": bool(e["tax"]), "car": car})
...
def placebo_car_dist(prices: pd.DataFrame, tickers: list[str], k: int,
                     window: tuple[int, int] = DEFAULT_WINDOW,
                     n_draws: int = 20_000, seed: int = 746) -> np.ndarray:
    ...
    each one's market-model CAR over the same window, and records the mean. This is the
    honest small-sample yardstick: "could a dozen random dates on these names have
    produced this CAR?"
    """
...
def summarize(panel: pd.DataFrame, prices: pd.DataFrame | None = None,
    ...
    """Headline event-study stats: tax / other / all buckets, the tax-minus-other
    difference (Welch t), and a placebo p-value for the all-events bucket if prices given."""
    tax = panel.loc[panel["tax"], "car"].to_numpy(float)
    other = panel.loc[~panel["tax"], "car"].to_numpy(float)
    allc = panel["car"].to_numpy(float)
    out = {
        "tax": summarize_bucket(tax),
        "other": summarize_bucket(other),
        "all": summarize_bucket(allc),
        "diff_pct": float(np.nanmean(tax) - np.nanmean(other)) * 100
        if len(tax) and len(other) else float("nan"),
        "diff_t": welch_t(tax, other),
    }
...
def drift_panel(prices: pd.DataFrame, events: list[dict],
                window: tuple[int, int] = DRIFT_WINDOW) -> pd.DataFrame:
    """Post-announcement drift CAR, entered the day after the headline (window already
    starts at +1). This is the *tradable* leg the 'signal' camp needs to exist."""
    return car_panel(prices, events, window=window, lag=0)


def net_of_costs(car_mean: float, cost_bps: float = 10.0) -> dict:
    """One-way round-trip cost on a 'buy the day after the announcement, hold the window'
    trade. A single entry + single exit; 10 bps round-trip covers a liquid large-cap desk.
    Returns gross/net mean CAR. (As with the rest of the study, cost is not the binding
    constraint — the drift simply isn't there.)"""
    c = cost_bps / 1e4
