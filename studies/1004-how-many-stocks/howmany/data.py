"""Data layer for Study 1004 — How Many Stocks.

Forty large-capitalisation US names with long histories, plus the cap-weighted and
equal-weighted index products they aggregate towards.

**This basket survived.** Every name in it is still listed in 2026, which is precisely the
selection an investor could not have made in 1993. The study therefore does not claim to measure
the level of diversification benefit — it measures *how the benefit scales with the number of
holdings*, which is far less sensitive to survivorship, and it reports the direction of the bias
...
# A cross-section broad enough to sample from, plus the index it aggregates to.
MARKET = "SPY"
EQUAL_WEIGHT = "RSP"
...
CASH = "BIL"
TICKERS = (MARKET, EQUAL_WEIGHT, CASH) + NAMES
    ...
    for tk in tickers:
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                f"Call howmany.data.fetch() once to populate the shared cache."
            )
...
def synthetic_daily(n_years: int = 20, signal_strength: float = 1.0,
                    seed: int = 1004) -> tuple[pd.DataFrame, dict]:
