"""Strategy + inference for Study 365 — the Lottery / MAX effect.

The claim (Bali, Cakici & Whitelaw, 2011, *Maxing out*): stocks with a big recent one-day
pop — high **MAX** = highest daily return over the prior month — are "lottery tickets" that
investors over-pay for, and they *underperform* going forward. The trade is **long the
low-MAX quintile, short the high-MAX quintile**, rebalanced monthly.

This module:

  * sorts the monthly MAX panel into **quintiles** each month and earns each quintile's
    *next-month* return (the panel is already lagged: MAX at month-end pairs with month t+1's
    return — one execution lag, documented);
  * forms the **long-short** spread Q1 (low MAX) − Q5 (high MAX);
  * tests its mean with a **Newey-West (HAC) t-stat** and a **placebo / sign-flip null**
    (the small-sample workhorse), reports the **monthly win-rate** vs a 50% coin, and charges
    **one-way costs × turnover** plus a **short borrow** on the high-MAX leg.

The decisive object is the HAC t of the long-short on the *real* tape: literature support for
the CRSP small-cap anomaly does not certify a tradable large-cap spread (the desk's t ≥ 2 bar,
on the tape the study actually ran).
"""
...
def quintile_returns(panel: dict, n_q: int = 5) -> pd.DataFrame:
    """Equal-weight next-month return of each MAX quintile, month by month.

    For each month with at least ``2*n_q`` valid (MAX, fwd_ret) pairs, rank names by MAX,
    split into ``n_q`` quantiles (Q1 = lowest MAX … Qn = highest MAX), and average each
    quantile's forward return. Returns a DataFrame indexed by month with columns
    ``Q1..Qn`` (and the dropped tail rows excluded).
    """
    mx = panel["max"]
    fwd = panel["fwd_ret"]
    rows = {}
    for dt in mx.index:
        s = mx.loc[dt]
        f = fwd.loc[dt]
        both = pd.concat([s.rename("max"), f.rename("fwd")], axis=1).dropna()
        if len(both) < 2 * n_q:
            continue
        try:
            q = pd.qcut(both["max"].rank(method="first"), n_q, labels=False)
        except ValueError:
            continue
...
def long_short(qret: pd.DataFrame, low: str = "Q1", high: str = "Q5",
    ...
    high-MAX leg (annual, pro-rated monthly) exactly once. ``turnover`` defaults to 1.0
    (full monthly turnover — quintile membership churns hard for a one-month signal).
    """
...
def placebo_pvalue(spread: pd.Series, n_draws: int = 20_000, seed: int = 365) -> dict:
    ...
    often the resampled mean is **>=** the observed mean (a two-sided idea kept one-sided to
    the claim's direction: low-MAX beats high-MAX, so the edge should be *positive*). Returns
    the observed mean, the placebo mean (~0), and the empirical p-value.
    """
