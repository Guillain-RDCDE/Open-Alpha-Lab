"""Reproducible headline run for Study 411 — Ascending Triangle.
...
print("# Ascending Triangle — objective detector on SPY + 29 large-caps (yfinance daily OHLC)")
if data.have_real():
    ...
    print("\n# Myth check — DOWN-break of the same figure (does it break UP far more than down?)")
    for h in st.HORIZONS:
    ...
    print("\n# Robustness — detector strictness (flat_tol, min_rise) at H=20")
    for ft, mr in [(0.03, 0.07), (0.04, 0.05), (0.06, 0.04)]:
        r = st.run_experiment(panel, horizon=20, n_draws=3000, flat_tol=ft, min_rise=mr)
        print(f"  flat_tol={ft:.2f} min_rise={mr:.2f}: n={r['n_events']:>4}  "
    ...
    print("(no _cache/triangle_close.parquet — run data.fetch_panel() once to build the cache)")
...
for edge in (0.0, 0.20):
    panel, truth = data.synthetic_panel(edge=edge, seed=411, n_planted=8, daily_vol=0.011)
